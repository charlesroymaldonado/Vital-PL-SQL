 ...
 * %v 3/12/2020 - LM  seteo uxb=1 cuando no es indivisible y obligo a dividir por el uxb al buscar el restante
 * %v 08/03/2021- ChM   agrupo por caso de articulos mismo pedido dos lineas BTO y UN
                        solo aquí el grupo porque aplica a porcentaje de distribución
 *****************************************************************************************************/
 ...
 for conformado in(
                   select 
                          pe.idpedido,
                          dpe.cdarticulo,
                          sum(dpe.qtunidadmedidabase) necesita,
                    ...
                    where pe.idpedido = dpe.idpedido
                      ...
                      and cp.idconsolidadocomi is null
                      and cp.idconsolidadopedido = p_IdPedidos
                      --agrupo por caso de articulos mismo pedido dos lineas BTO y UN
                      --solo aquí el grupo porque aplica a porcentaje de distribución
                 group by 
                          pe.idpedido,
                          dpe.cdarticulo,
                 ...
                 -- ordenados por el que menos le faltó en artículo
                 order by dpe.cdarticulo,
                          necesita)--ordeno por articulo para mejorar la distribución
     loop
   ...
   insert into DETPEDIDOCONFORMADO
      ...
      select count(*)
       ...
       where conforma.idconsolidadopedido = consopedido.idconsolidadopedido
         ...
         and case
            --valida pesables
            when conforma.qtpiezas = 0 and conforma.qtbase-consopedido.qtbaseP <> 0 then 1
            when conforma.qtpiezas <> 0 and conforma.qtpiezas- consopedido.qtpiezasP <> 0 then 1
              else 0
            end = 1;
     if v_ped <> 0 then
        n_pkg_vitalpos_log_general.write(2,
