  ...
  --  v_valida           integer:=0;
   ...
   --verifica si esta iniciado Control
    ...
    OPEN p_Cursor FOR                
             ...
             from ( WITH 
                    --listo todo lo de controlremito asi no este en remito,
                    --en esta instancia todo lo del remito debe estar en control, aún sin contó
                    control AS(  
                         select crd.cdarticulo,                          
                                --valida pesables
                                decode(sum(nvl(crd.qtpiezaspicking,0)),0,
     ...
     begin
       ...
       select count(*)
        ...
        where crd.idcontrolremito = v_idcontrol
          ...
          and (crd.qtdiferenciaunidadmbase+nvl(crd.qtunidadmedidabasepicking,0) <> 0
           or crd.qtdiferenciapiezas+nvl(crd.qtpiezaspicking,0) <> 0);       
       exception 
         when no_data_found then
          v_cantidad:=0;
       end;
