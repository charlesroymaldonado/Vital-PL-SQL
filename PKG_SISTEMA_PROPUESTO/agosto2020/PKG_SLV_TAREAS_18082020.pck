...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  * %v 05/08/2020 - ChM  ajustes para ubicación de remitos (p_Ubicacion)
  *****************************************************************************************************/
  ...
  BEGIN
   ...
   loop
     ...
     if nvl(p_idpersonamanual,'X')='X' then  --es picking hh
       PKG_SLV_TAREAS.SetRegistrarPicking(p_idpersona,v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, r_artbolsin.cdarticulo, p_idtarea,r_artbolsin.idpedido, p_Fintarea,p_ubicacion, p_ok, p_error); 
     else
       PKG_SLV_TAREAS.SetRegistrarPickingM(v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, p_IdTarea,p_IdPersonaManual, 1,r_artbolsin.idpedido, p_Fintarea,p_ubicacion, p_ok, p_error);
     end if;
     if (p_ok=0) then
       Rollback;
       Exit;
