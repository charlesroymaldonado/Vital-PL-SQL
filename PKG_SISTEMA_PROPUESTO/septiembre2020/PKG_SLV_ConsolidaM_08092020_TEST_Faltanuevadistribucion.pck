...
/****************************************************************************************************
  ...
  * %v 08/09/2020 - ChM  agrego validacion de no MIx Hijos con padres de comi o reparto  
  *****************************************************************************************************/
  PROCEDURE GetPreVisualizarPedidos(p_QtBtoConsolidar IN  NUMBER,
                                    p_TransId         IN  arr_TransId,
                                    --p_IdComisionista  IN  arr_IdentidadComi,
           ...
           SELECT seq_consolidadopedidorel.nextval,
             ...
             FROM (select p.idpedido
                    ...
                    where m.transid = p.transid
                      ...
                      and m.idcanal = PEDIDO.ID_CANAL --valida Canal
                      and trunc(p.dtaplicacion) = PEDIDO.DTAPLICACION --por fecha
                      and p.cdtipodireccion = PEDIDO.CDTIPODIRECCION --direccion
                      and p.sqdireccion = PEDIDO.SQDIRECCION --direccion
                      and m.idpersona = p_idPersona
                      and m.idcomisionista is null
                      and m.idcanal <> 'CO'
