CREATE OR REPLACE PACKAGE PKG_SLV_REMITOS is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetRemitoDistribucion (p_idFaltante      IN  tblslvpedfaltante.idpedfaltante%type,                                   
  ...
  PROCEDURE SetUbicacionRemito(p_cdeancode     IN  tblslvubicacionremito.cdeancode%type,
                               p_dsubicacion   IN  tblslvubicacionremito.dsubicacion%type,
                               p_Ok            OUT number,
                               p_error         OUT varchar2);

  PROCEDURE GetUbicacionRemito(p_cdeancode     IN  tblslvubicacionremito.cdeancode%type default null,
                               p_Cursor        OUT CURSOR_TYPE);
                               
  PROCEDURE UpdUbicacionRemito(p_idubicacionremito     IN  tblslvubicacionremito.idubicacionremito%type,
                               p_cdeancode             IN  tblslvubicacionremito.cdeancode%type,
                               p_dsubicacion           IN  tblslvubicacionremito.dsubicacion%type,
                               p_dtvigenciadesde       IN  tblslvubicacionremito.dtvigenciadesde%type,
                               p_dtvigenciahasta       IN  tblslvubicacionremito.dtvigenciahasta%type,
                               p_Ok                    OUT number,
                               p_error                 OUT varchar2);  
                               
  PROCEDURE GetRemitoUbicacion(p_idRemito      IN  tblslvremito.idremito%type,                           	   
                               p_Cursor        OUT CURSOR_TYPE);

  PROCEDURE SetRemitoUbicacion(p_idubicacionremito     IN  tblslvubicacionremito.idubicacionremito%type,
   ...
   FUNCTION FormatoUbicacionRemito(p_idubicacion     tblslvremito.idubicacionremito%type)
                                                 return varchar2;
...
--c_qtDecimales                                  CONSTANT number := 2; -- cantidad de decimales para redondeo
  ...
  *****************************************************************************************************/
  FUNCTION SetInsertarRemitoFaltante(p_idpedfaltanterel tblslvremito.idpedfaltanterel%type)
    ...
    v_ubicacion    tblslvremito.idubicacionremito%type:=null;
  ...
  BEGIN
    ...
    --busca la primera ubicación del remito del pedido original
    begin
        select re.idubicacionremito 
          into v_ubicacion
          from tblslvremito re,
               tblslvtarea  ta
         where re.idtarea = ta.idtarea
           and ta.idconsolidadopedido = v_idpedido
           and rownum=1;
    exception
      when others then
        v_ubicacion:=null;       
  ...
  /****************************************************************************************************
  * %v 10/09/2020 - ChM  Versión inicial GetRemitoDistribucion
  * %v 10/09/2020 - ChM  lista los remitos de distribución que componen un p_idfaltante
  *****************************************************************************************************/
  PROCEDURE GetRemitoDistribucion (p_idFaltante      IN  tblslvpedfaltante.idpedfaltante%type,                                   
                                   p_Cursor          OUT CURSOR_TYPE) IS
                                   
   v_modulo varchar2(100) := 'PKG_SLV_REMITOS.GetRemitoDistribucion idfaltante:'||p_idFaltante;                   
    ...
    OPEN P_Cursor FOR
          select re.idremito,
           ...
           where re.idpedfaltanterel = pfrel.idpedfaltanterel
             and pfrel.idpedfaltante = p_idFaltante;             
   EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);
  END GetRemitoDistribucion;
  ...
  BEGIN
    ...
    OPEN p_Cursor FOR
           select re.idremito,
                  ...
                  re.nrocarreta rolls,
                  PKG_SLV_REMITOS.FormatoUbicacionRemito(re.idubicacionremito) ubicacion
             from tblslvremito                        re,
                  tblslvtarea                         ta,
                  personas                            pers,
                  pedidos                             pe,
 ...
 /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial SetUbicacionRemito
  * %v 05/08/2020 - ChM  inserta en la tabla tblslvUbicacionRemito
  *****************************************************************************************************/
  PROCEDURE SetUbicacionRemito(p_cdeancode     IN  tblslvubicacionremito.cdeancode%type,
                               p_dsubicacion   IN  tblslvubicacionremito.dsubicacion%type,
                               p_Ok            OUT number,
    ...
    v_modulo           varchar2(100) := 'PKG_SLV_REMITOS.SetUbicacionRemito';
                                                                   
   BEGIN
    insert into tblslvubicacionremito
                (idubicacionremito,
                 cdeancode,
                 dsubicacion,
                 dtvigenciadesde,
                 ...
                 dtinsert,
                 dtupdate)
         values (seq_ubicacionremito.nextval,    
                 p_cdeancode,
                 p_dsubicacion,
                 trunc(sysdate),
                 '31/12/9999',
  ...
  exception                  
    when others then
          n_pkg_vitalpos_log_general.write(2,
              ...
              p_Ok:=0;
              p_error:='Error. Comuniquese con Sistemas!';
  END SetUbicacionRemito;
  
  /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial GetUbicacionRemito
  * %v 05/08/2020 - ChM  lista los datos de la tabla tblslvUbicacionRemito
  *****************************************************************************************************/
  PROCEDURE GetUbicacionRemito(p_cdeancode     IN  tblslvubicacionremito.cdeancode%type default null,
                               p_Cursor        OUT CURSOR_TYPE)  IS 
   BEGIN
    OPEN p_Cursor FOR
               select ur.cdeancode,
                      ...
                      ur.dtvigenciadesde,
                      ur.dtvigenciahasta 
                 from tblslvubicacionremito ur
                where (p_cdeancode is null or ur.cdeancode=p_cdeancode);                 
  EXCEPTION                  
    WHEN OTHERS THEN
        NULL;
  END GetUbicacionRemito;
  
  /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial UpdUbicacionRemito
  * %v 05/08/2020 - ChM  actuliza en la tabla tblslvUbicacionRemito
  *****************************************************************************************************/
  PROCEDURE UpdUbicacionRemito(p_idubicacionremito     IN  tblslvubicacionremito.idubicacionremito%type,
                               p_cdeancode             IN  tblslvubicacionremito.cdeancode%type,
                               p_dsubicacion           IN  tblslvubicacionremito.dsubicacion%type,
    ...
    v_modulo           varchar2(100) := 'PKG_SLV_REMITOS.UpdUbicacionRemito';
                                                                   
   BEGIN
    update tblslvubicacionremito ur
       set ur.cdeancode = p_cdeancode,
           ur.dsubicacion = p_dsubicacion,
           ur.dtvigenciadesde = trunc(p_dtvigenciadesde),
           ur.dtvigenciahasta = trunc(p_dtvigenciahasta),
           ur.dtupdate = sysdate
     where ur.idubicacionremito = p_idubicacionremito;   
  ...
  exception                  
    when others then
          n_pkg_vitalpos_log_general.write(2,
              ...
              p_Ok:=0;
              p_error:='Error. Comuniquese con Sistemas!';
  END UpdUbicacionRemito;
  
   /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial GetRemitoUbicacion
  * %v 05/08/2020 - ChM  devuelve las ubicaciones para el remito
  *****************************************************************************************************/
  PROCEDURE GetRemitoUbicacion(p_idRemito      IN  tblslvremito.idremito%type,                          	   
                               p_Cursor        OUT CURSOR_TYPE)  IS 
   BEGIN
    ...
    OPEN p_Cursor FOR
                select ur.idubicacionremito,
                      pkg_slv_remitos.FormatoUbicacionRemito(ur.idubicacionremito)||'       Sugerida' Ubicacion
                 from tblslvubicacionremito ur
                where ur.idubicacionremito in 
                     (select nvl(re.idubicacionremito,0)
                        from tblslvremito re
                       where re.idtarea in (select distinct re2.idtarea 
                                              from tblslvremito re2
                                             where re2.idremito=p_idRemito)
                      union all
                      select nvl(re.idubicacionremito,0) 
                        from tblslvremito re
                       where re.idpedfaltanterel in (select distinct nvl(re2.idpedfaltanterel,0)
                                                       from tblslvremito re2
                                                      where re2.idremito=p_idRemito)                       
                     )                     
               union all                                             
               select ur.idubicacionremito,
                      pkg_slv_remitos.FormatoUbicacionRemito(ur.idubicacionremito) Ubicacion                  
                 from tblslvubicacionremito ur
                      --solo vigentes
                where trunc(sysdate) between ur.dtvigenciadesde and ur.dtvigenciahasta
                  -- lista todas las no sugeridas                  
                  and ur.idubicacionremito not in 
                     (select nvl(re.idubicacionremito,0) 
                        from tblslvremito re
                       where re.idtarea in (select distinct re2.idtarea 
                                              from tblslvremito re2
  ...
  END GetRemitoUbicacion; 
  
 /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial SetRemitoUbicacion
  * %v 05/08/2020 - ChM  Actualiza la ubicación del remito
  *****************************************************************************************************/
  PROCEDURE SetRemitoUbicacion(p_idubicacionremito     IN  tblslvubicacionremito.idubicacionremito%type,
                               p_idRemito              IN  tblslvremito.idremito%type,
                           	   p_Ok                    OUT number,
    ...
    v_modulo           varchar2(100) := 'PKG_SLV_REMITOS.SetRemitoUbicacion';
                                                                   
   BEGIN
    update tblslvremito re
       set re.idubicacionremito = p_idubicacionremito,
           re.dtupdate = sysdate
  ...
  exception                  
    when others then
          n_pkg_vitalpos_log_general.write(2,
              ...
              p_Ok:=0;
              p_error:='Error. Comuniquese con Sistemas!';
  END SetRemitoUbicacion; 
  
   /****************************************************************************************************
  * %v 05/08/2020 - ChM  Versión inicial FormatoUbicacionRemito
  * %v 05/08/2020 - ChM  muestra los datos de una ubicacion de remito para reportes
  *****************************************************************************************************/
  FUNCTION FormatoUbicacionRemito(p_idubicacion     tblslvremito.idubicacionremito%type)
                                                 return varchar2 IS 
   
   v_formato       varchar2(200);
   ...
   BEGIN
     ...
     if p_idubicacion is null then
        RETURN '-';
         ...
         select ur.cdeancode ||' - '||ur.dsubicacion
           into v_formato    
           from tblslvubicacionremito ur
          where ur.idubicacionremito = p_idubicacion;
      end if;                    
          RETURN v_formato;
    EXCEPTION                  
        WHEN OTHERS THEN
