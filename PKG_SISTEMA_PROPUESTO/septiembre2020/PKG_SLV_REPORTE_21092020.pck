  ...
  *****************************************************************************************************/
  ...
  BEGIN
    ...
    v_dtHasta := to_date(to_char(p_dtfechahasta, 'dd/mm/yyyy') || ' 23:59:59','dd/mm/yyyy hh24:mi:ss');
     ...
     OPEN p_Cursor FOR
          with conforma as  ( select pc.idpedido,
                                     cm.dtinsert,
                                     cm.cdsucursal,
                                     pc.cdarticulo,
                               ...
                               where pc.idpedido = prel.idpedido
                                 ...
                                 and cp.idconsolidadom = cm.idconsolidadom
                                 and pc.dtinsert between v_dtDesde and v_dtHasta 
                  ...
                  pedido as  (select p.idpedido,
                            ...
                            --     and p.dtaplicacion between &v_dtDesde and &v_dtHasta 
                            group by p.idpedido,
                                     dp.cdarticulo)    
                select su.dssucursal sucursal,
                       to_char(co.dtinsert,'dd/mm/yyyy') FechaConsolidado,
                       sec.dssector Sector, 
                       co.cdarticulo || '- ' || des.vldescripcion Articulo,
                       round(pe.precioun,3) preciounitario,
                       PKG_SLV_Articulo.SetFormatoArticulosCod(co.cdarticulo,
                       --valida pesables
                       sum(decode(pe.qtpiezas,0,pe.qtbase,pe.qtpiezas))) unidades_pedidas,
                       PKG_SLV_Articulo.SetFormatoArticulosCod(co.cdarticulo,
                       --valida pesables
                       sum(decode(co.qtpiezas,0,co.qtbase,co.qtpiezas))) unidades_pickeadas,
                       PKG_SLV_Articulo.SetFormatoArticulosCod(co.cdarticulo,
                       abs(sum((decode(co.qtpiezas,0,co.qtbase,co.qtpiezas)-decode(pe.qtpiezas,0,pe.qtbase,pe.qtpiezas))))) Faltantes,
                       PKG_SLV_Articulo.SetFormatoArticulosCod(co.cdarticulo,
                       PKG_SLV_Articulo.GetStockArticulos(co.cdarticulo)) STOCK,
                       posapp.n_pkg_vitalpos_materiales.GetUxB(co.cdarticulo) UXB,
                       PKG_SLV_Articulo.GetUbicacionArticulos(co.cdarticulo) UBICACION                            
                  from conforma               co,
                       pedido                 pe,
                       sucursales             su,
                       articulos              art,
                       descripcionesarticulos des,                                    
                       sectores               sec
                 where co.idpedido = pe.idpedido
                   and co.cdarticulo = pe.cdarticulo
                   and co.cdsucursal = su.cdsucursal 
                   and art.cdarticulo = co.cdarticulo
                   and art.cdsector = sec.cdsector              
                   and art.cdarticulo = des.cdarticulo
                   and case 
                         --verifica si es pesable 
                          when pe.qtpiezas<>0
                           and (co.qtpiezas-pe.qtpiezas <> 0) then 1
                          --verifica los no pesable
                          when pe.qtpiezas = 0 
                           and (co.qtbase-pe.qtbase <> 0)  then 1
                       else 0    
                       end = 1  
              group by su.dssucursal, 
                       to_char(co.dtinsert,'dd/mm/yyyy'),
                       sec.dssector,
                       co.cdarticulo,
                       co.cdarticulo || '- ' || des.vldescripcion,
                       round(pe.precioun,3)
