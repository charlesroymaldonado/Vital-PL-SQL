  ...
  **********************************************************************************************************/
  ...
  PROCEDURE SetControl(p_idremito           IN  tblslvremito.idremito%type,                        
   ...
   PROCEDURE SetReConteoDet(p_idremito           IN  tblslvremito.idremito%type, 
                            ...
                            p_qtunidad           IN  tblslvremitodet.qtunidadmedidabasepicking%type,
                            p_cdarticulototal    OUT varchar2,
                            p_Cursor             OUT CURSOR_TYPE,                                                        
                            p_Ok                 OUT number,
                            p_error              OUT varchar2);
...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  *****************************************************************************************************/
  ...
  END UpdateConteoDet;
   ...
   /****************************************************************************************************
  * %v 04/08/2020 - ChM  Versión inicial ObtenerIDConsolidado
  * %v 04/08/2020 - ChM  desde un id de remito devuelve el pedido al que pertenece e
                         indica si esta facturado y la cantidad de remitos que lo componen
  *****************************************************************************************************/
  FUNCTION ObtenerIDConsolidado (p_idremito        tblslvremito.idremito%type) 
                                 RETURN VARCHAR2   IS
                               
  v_modulo                      varchar2(100) := 'PKG_SLV_CONTROL.ObtenerIDConsolidado';  
  v_cdestado                    tblslvestado.cdestado%type:=0;
  v_totalRemitos                integer;
  v_tipoTarea                   tblslvtarea.cdtipo%type:=0;
  v_idpedido                    tblslvtarea.idconsolidadopedido%type:=0;
  
  BEGIN
        --Busca el id del consolidado segun id remito
        select ta.cdtipo cdtipotarea,           
               coalesce(ta.idconsolidadopedido,
     ...
     union all
        select c_TareaConsolidadoPedido cdtipotarea,           
               frel.idconsolidadopedido pedido
          from tblslvremito             re,
               tblslvpedfaltanterel     frel
         where re.idpedfaltanterel = frel.idpedfaltanterel
           and re.idremito = p_idremito; 
      --busca estados del comi y cantidad de remitos
      if v_tipoTarea = c_TareaConsolidadoComi then
        begin  
        select cc.cdestado,
               ContarRemitos(cc.idconsolidadocomi,v_tipoTarea)
          into v_cdestado,
               v_totalRemitos
          from tblslvconsolidadocomi cc
         where cc.idconsolidadocomi = v_idpedido;
        exception
          when others then
             v_cdestado:=0;  
         end; 
      ...
      if v_tipoTarea = c_TareaConsolidadoPedido then
        begin  
        select cp.cdestado,
               ContarRemitos(cp.idconsolidadopedido,v_tipoTarea)
          into v_cdestado,
               v_totalRemitos
          from tblslvconsolidadopedido cp
         where cp.idconsolidadopedido = v_idpedido;
        exception
          when others then
             v_cdestado:=0;  
         end; 
      end if; 
     if v_cdestado <> 0 then
           --verifica si esta en estado facturado el reparto o el comi
           if v_cdestado in (13,14,28,29) then
             return 'Facturado/'||v_totalremitos||' Remitos';
           else 
             return 'NO Facturado/'||v_totalremitos||' Remitos';  
           end if;
      else
        return '-';       
       end if;     
  ...
  *****************************************************************************************************/
  ...
  PROCEDURE GetControl(p_idremito           IN  tblslvremito.idremito%type,
   ...
   begin
    ...
    p_consolidado:= ObtenerIDConsolidado(p_idremito);       
    p_Ok    := 1;
    p_error := '';
    commit;   
    EXCEPTION
  ...
  *****************************************************************************************************/
  ...
  BEGIN
     ...
     --valida unidad de venta minima indivisibles
        v_cdunidadVentamin:=pkg_slv_articulo.GetValidaUnidadVentaMinimaArt(v_cdArticulo,v_cdunidad);  
    ...
    if  v_qtcontrol <> 1 then
     ...
     open p_Cursor for           
           select A.cdarticulo,
                  A.cdarticulo||' - '||des.vldescripcion||' '||A.Cantidad Articulo                  
  ...
  *****************************************************************************************************/
   ...
   open p_Cursor for 
           select cod.cdarticulo,
                  trim(cod.cdarticulo)||' - '||des.vldescripcion ||' '||
                  --codigo de barras
                  decode(nvl(cod.qtpiezaspicking,0),0,
                  PKG_SLV_ARTICULO.GetCodigoDeBarra(cod.cdarticulo,'UN'),
                  PKG_SLV_ARTICULO.GetCodigoDeBarra(cod.cdarticulo,'KG'))  Articulo
  ...
  *****************************************************************************************************/
   ...
   --devuelve la cantidad del actual conteo del articulo ingresado
   begin
     select trim(cod.cdarticulo)||' - '||des.vldescripcion ||'***'||
            PKG_SLV_Articulo.SetFormatoArticuloscod(cod.cdarticulo,
            --valida pesables
            decode(nvl(cod.qtpiezaspicking,0),0,
                       nvl(cod.qtunidadmedidabasepicking,0),
                       nvl(cod.qtpiezaspicking,0))) Articulo
       into p_cdarticulototal                        
       from tblslvconteodet        cod,
            descripcionesarticulos des
      where cod.cdarticulo = des.cdarticulo
        and cod.idconteo = p_idConteo  
  ...
  * %v 09/09/2020 - LM   se agrega un nvl cuando son articulos que no estan dentro del remito    
  *****************************************************************************************************/
  PROCEDURE GetErroresControl(p_idremito           IN  tblslvremito.idremito%type,
                              p_cursor             OUT CURSOR_TYPE,                  
                              p_Ok                 OUT number,
             ...
             from ( WITH 
                    ...
                    remito AS (              
                      ...
                      SELECT control.cdarticulo,
                             nvl(control.cantidadC,0)-nvl(remito.CantidadR,0) cantidad
                        FROM control
                        left join (remito)
                               on (control.cdarticulo=remito.cdarticulo) )A,
                    descripcionesarticulos des
              where A.cdarticulo = des.cdarticulo
                and nvl(A.cantidad,0)<>0;        
    p_Ok    := 1;
    p_error := '';    
    EXCEPTION
    WHEN OTHERS THEN
  ...
  BEGIN
    ...
    OPEN p_Cursor FOR
         select A.fechapedido,
                ...
                case
                  --estados Facturado
                  when A.cdestado in (13,14,28,29) then 1
                    else 0
                  end facturado  
  ...
  BEGIN
    ...
    OPEN p_Cursor FOR                
                select A.cdarticulo || '- ' || des.vldescripcion Articulo,
                       ...
                       posapp.n_pkg_vitalpos_materiales.GetUxB(A.cdarticulo) UXB  
