...
CREATE OR REPLACE PACKAGE BODY PKG_SLV_DISTRIBUCION is
                ...
                set (pc1.qtunidadmedidabase,
                    ...
                    (select pc.qtunidadmedidabase+v_dif,
                        ...
                        where pe.idpedido = dpe.idpedido
                          ...
                          and (pc.qtunidadmedidabase-pc.qtunidadmedidabase)>=abs(v_dif)
  ...
  *****************************************************************************************************/ 
  ...
  While i Is Not Null Loop       
     ...
     if PESABLES_P(i).BANDERA = 0 
        ...
        --OOOJJJJOOO falta por los redondeos ajustar aqui EN DESARROLLO 
         p_Ok    := 0;
         p_error := 'Error en DISTRIBUCIÓN EN DESARROLLO Pesables. Comuniquese con Sistemas!';   
     end if; 
     i:= PESABLES_P.NEXT(i);
   End Loop;  
  p_Ok    := 1;
 ...
 --solo articulos en promo ordenados por el que menos le falto en cantidad
  for promo in(
           ...
           order by pdis.qtunidadmedidabase,
                    pdis.cdarticulo) 
          loop
            --verifica si el articulo cambio 
            if v_artant is not null and promo.cdarticulo <> v_artant then               
 ...
 *****************************************************************************************************/
 ...
 BEGIN
   ...
   IF SQL%ROWCOUNT = 0 THEN
     ...
     RETURN;
   END IF;
   --verifico las diferencias que pudo generar los porcentajes de distribución de faltantes  EN DESARROLLO
   /*p_Ok    := VerificaDiferenciasFal(p_IdPedFaltante);
   if p_Ok  = 0 then
     p_ok    := 0;
     p_error := 'error en procentajes de Distribución EN DESARROLLO. comuniquese con sistemas!';
     rollback;
     return;
   end if;*/
 ...
 *****************************************************************************************************/
 ...
 for conformado in(
  ...
  --Actualizo la tabla tblslvconsolidado pedido a estado C_AFacturarConsolidadoPedido
   ...
   --verifico las diferencias que pudo generar los porcentajes de distribución EN DESARROLLO 
 ...
 *****************************************************************************************************/
 ...
 for conformado in(
   ...
   --verifico las diferencias que pudo generar los porcentajes de distribución OJO EN DESARROLLO   
