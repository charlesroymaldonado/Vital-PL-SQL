  ...
  *****************************************************************************************************/ 
  ...
  insert into tblslvpordistrib
       ...
       select p.idpedido,
              ...
              ROUND((sum(dp.qtunidadmedidabase)/
              TotalArtComi(p_idconsolidado,dp.cdarticulo))*100) porc,
              sysdate
         from tblslvconsolidadopedido cp,
              tblslvconsolidadopedidorel prel,
              tblslvconsolidadocomi cc,
  ...
  BEGIN
    for pes in
             ( select frel.idpedfaltanterel,
                      ...
                      round(fd.qtpiezaspicking * (pdis.porcdist / 100),0) QTDISTB                               
                 from tblslvpedfaltante          cf,
                      tblslvpedfaltantedet       fd,
                      tblslvpedfaltanterel       frel,
                      tblslvconsolidadopedido    cp,
                ...
                where cf.idpedfaltante = fd.idpedfaltante
                  ...
                  and round(fd.qtpiezaspicking * (pdis.porcdist / 100),0) > 0   
  ...
  BEGIN
    for pes in(
       select pe.idpedido,
              ...
              round(ccd.qtpiezaspicking * (pdi.porcdist/100),0) qtpiezaspicking,       
              dpe.ampreciounitario,
              dpe.amlinea,
              dpe.vluxb,
              nvl(dpe.dsobservacion,'-') dsobservacion,
        ...
        where pe.idpedido = dpe.idpedido
          ...
          --solo inserto para facturar los mayores a cero 
          and round(ccd.qtpiezaspicking * (pdi.porcdist/100),0)>0                 
          -- solo pesables
 ...
 *****************************************************************************************************/
 ...
 BEGIN
  ...
  --Actualizo la tabla tblslvconsolidadopedido con los articulos encontrados en la distribucion de faltante
 ...
 --verifico si el consolidado pedido es de comisionista
   ...
   exception 
    ...
    if v_ped is not null then  
       p_Ok    := 0;
       p_error := 'Error consolidado pedido N° '||p_IdPedidos||' de Comisionista no Reparto. No es posible distribuir.';
       RETURN;    
    end if;
 ...
 *****************************************************************************************************/
 ...
 for conformado in(
                  select pe.idpedido,
                          ...
                          dpe.cdunidadmedida,
                          dpe.qtunidadpedido,
                          round(ccd.qtunidadmedidabasepicking * (pdi.porcdist/100),0)qtbase,
                          ccd.qtpiezaspicking,       
                          dpe.ampreciounitario,
                          dpe.amlinea,
                          dpe.vluxb,
                    ...
                    where pe.idpedido = dpe.idpedido
                      ...
                      and pdi.cdtipo = c_TareaConsolidadoComi
                      --solo inserto para facturar los mayores a cero 
                      and round(ccd.qtunidadmedidabasepicking * (pdi.porcdist/100),0)>0                 
                      -- excluyo pesables
                      and nvl(ccd.qtpiezas,0)=0
                      --valida cesta navideña
                      and pe.idcnpedido is null 
