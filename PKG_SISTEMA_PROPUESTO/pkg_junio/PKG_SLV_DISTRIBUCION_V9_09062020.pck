 ...
 *****************************************************************************************************/
 ...
 BEGIN
    --verifico si el faltante esta distribuido no se puede volver a distribuir
   begin
     select f.idpedfaltante 
       into v_pedfal
       from tblslvpedfaltante f
      where f.cdestado = C_DistribFaltanteConsolidaPed
 ...
 --verifico si el pedido es de comisionista
   ...
   begin
     select cp.idconsolidadopedido
       into v_ped
       from tblslvconsolidadopedido cp
      where cp.cdestado = C_AFacturarConsolidadoPedido
        and cp.idconsolidadopedido = p_IdPedidos;
   exception 
     when no_data_found then
       v_ped:=null;
