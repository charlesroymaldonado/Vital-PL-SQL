...
CREATE OR REPLACE PACKAGE BODY PKG_SLV_DISTRIBUCION is
  ...
  *****************************************************************************************************/ 
  ...
  /*\****************************************************************************************************
  * %v 17/06/2020 - ChM  Versión inicial TempPesablesF
  * %v 17/06/2020 - ChM crea la tabla temporal de los pesables disponibles del consolidado Faltante
  *****************************************************************************************************\ 
    ...
    FOR PES IN
             (select pf.idpedfaltante,
               ...
               where pf.idpedfaltante = pfd.idpedfaltante
                 ...
                 and pf.idpedfaltante = p_idconsolidado)
    LOOP
      PESABLES_P(v_i).IDCONSOLIDADOPEDIDO:=pes.idpedfaltante;
      PESABLES_P(v_i).CDARTICULO:=pes.cdarticulo;     
      PESABLES_P(v_i).QTUNIDADMEDIDABASEPIK:=pes.qtunidadmedidabasepicking;
      PESABLES_P(v_i).QTPIEZASPIK:=pes.qtpiezaspicking;
      --IDPEDIDO en 0 cero indica pesable no asignado
  ...
  *****************************************************************************************************/ 
  PROCEDURE DistPesablesP(p_idconsolidado       tblslvconsolidadom.idconsolidadom%type,
                         p_Ok                  OUT number,
                         p_error               OUT varchar2
                        )is
  ...
  BEGIN
    for pes in(
       select pe.idpedido,
              ...
              round(cpd.qtpiezaspicking * (pdi.porcdist/100),0) qtpiezaspicking,
              dpe.ampreciounitario,
              dpe.amlinea,
              dpe.vluxb,
        ...
        where pe.idpedido = dpe.idpedido
         ...
         --solo inserto para facturar los mayores a cero
          and round(cpd.qtpiezaspicking * (pdi.porcdist/100),0)>0
          -- solo pesables
          and nvl(cpd.qtpiezas,0)<>0
          --valida cesta navideña
          and pe.idcnpedido is null 
    ...
    loop
       ...
       While i Is Not Null and v_resto > 0 Loop
         ...
         if PESABLES_P(i).IDPEDIDO = 0 
            ...
            --asigno el pesable al pedido en la tabla de memoria
            PESABLES_P(i).IDPEDIDO:=pes.idpedido;
  ...
  While i Is Not Null Loop       
     ...
     if PESABLES_P(i).IDPEDIDO = 0 
        ...
        null;--OOOJJJJOOO falta por los redondeos ajustar aqui 
     end if; 
     i:= PESABLES_P.NEXT(i);
   End Loop;  
  p_Ok    := 1;
 ...
 BEGIN
   ...
   insert into tblslvdistribucionpedfaltante
    ...
    (select frel.idpedfaltanterel,
            ...
            round(fd.qtpiezaspicking * (pdis.porcdist / 100),0) qtpiezaspicking            
       from tblslvpedfaltante          cf,
            tblslvpedfaltantedet       fd,
            tblslvpedfaltanterel       frel,
            tblslvconsolidadopedido    cp,
      ...
      where cf.idpedfaltante = fd.idpedfaltante
        ...
        and case
            when fd.qtpiezas = 0  and round(fd.qtunidadmedidabasepicking * (pdis.porcdist / 100),0) > 0 then 1
            when fd.qtpiezas <> 0 and round(fd.qtpiezaspicking * (pdis.porcdist / 100),0) > 0 then 1      
            end = 1  
 ...
 *****************************************************************************************************/
 ...
 insert into tblslvpedidoconformado
  ...
  if v_ped > 1 then 
     ...
     if p_Ok  = 0 then
       p_ok    := 0;
       p_error := 'error en Distribución de pesables. comuniquese con sistemas!';
       rollback;
       return;
     end if;
    end if;
