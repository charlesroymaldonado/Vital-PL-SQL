CREATE OR REPLACE PACKAGE PKG_SLV_TAREAS is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetTareaAsigConsolidado(p_idConsolidado   IN  Integer,
                                    p_TipoTarea       IN  Tblslvtipotarea.cdtipo%type,
                                    p_icconremitos    IN  integer default 0,
                                    p_Cursor          OUT CURSOR_TYPE);
  ...
  PROCEDURE GetPanelArmadores(p_Cursor     OUT CURSOR_TYPE); 
  
  FUNCTION  NombreCliente(p_IdTarea            tblslvtarea.idtarea%type) 
                          RETURN VARCHAR2;
                        
  PROCEDURE GetDetalleArmadores(p_idarmador      IN  personas.idpersona%type,                  
  ...
  PROCEDURE  PikingMasivoTarea(p_IdTarea          IN   tblslvtarea.idtarea%type,
                               p_IdPersonaManual  IN   personas.idpersona%type default null,                               
                               p_porcentaje       IN   integer,
                               p_Ok               OUT  number,
                               p_error            OUT  varchar2);                                                                                                                                                                                                
  ...
  PROCEDURE SetRegistrarPickingM(p_idRemito         IN   tblslvremito.idremito%type default null,
                                 ...
                                 p_IdPedidoBolsin   IN   pedidos.idpedido%type default null,
                                 p_idubicacion      IN   tblslvubicacionremito.idubicacionremito%type default 0,
  ...
  PROCEDURE SetLiberarArmador(p_IdArmador   IN   personas.idpersona%type,
                              p_idTarea     IN   tblslvtarea.idtarea%type default null,
                              p_Ok          OUT  number,
                              p_error       OUT  varchar2);
                              
    PROCEDURE GetArtTareaSelectivo(p_barras in VARCHAR2,
                                     r_cursor OUT CURSOR_TYPE,
                                     p_error  OUT VARCHAR2);  
                                                                        
  PROCEDURE GetBolsinXIdTarea(p_idtarea IN tblslvtarea.idtarea%TYPE,
                               p_cursor OUT CURSOR_TYPE);

  PROCEDURE SetBolsinEnPick(  p_idtarea           IN   tblslvtarea.idtarea%TYPE, 
                              p_idBolsin          IN   posapp.llaverfcespec.icllavero%type,
                              p_IdPersonaManual   IN   personas.idpersona%type  default null,
...
C_AsignadoTareaFaltConFalt                         CONSTANT tblslvestado.cdestado%type := 40;
C_EnCursoTareaFaltConFalt                          CONSTANT tblslvestado.cdestado%type := 41;
C_FinalizadoTareaFaltConFalt                       CONSTANT tblslvestado.cdestado%type := 42;
  ...
  * %v 15/09/2020 - LM  quito el filtro de persona que asigna contra tarea
  * %v 30/09/2020 - ChM Agrego validación de tarea desasignada and td.icdesasignar = 0 
  *****************************************************************************************************/
      ...
      OPEN p_Cursor FOR
           ...
           distinct m.idconsolidadom idconsolidadom,
                    (select tt.dstarea||': '
                      ...
                      where tt.cdtipo = p_TipoTarea)||m.idconsolidadom||
              ...
              where m.idconsolidadom = det.idconsolidadom               
                ...
                and det.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona= m.idpersona LM
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0);
    EXCEPTION
    WHEN OTHERS THEN
    ...
    BEGIN
      OPEN p_Cursor FOR
           ...
           distinct p.idconsolidadopedido idconsolidadom,
                    (select tt.dstarea||': '
                       from tblslvtipotarea tt 
                      where tt.cdtipo = p_TipoTarea)||p.idconsolidadopedido||
              ...
              where p.idconsolidadopedido= det.idconsolidadopedido
                ...
                and det.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona= p.idpersona LM
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0 );
    EXCEPTION
    WHEN OTHERS THEN
    ...
    BEGIN
      OPEN p_Cursor FOR
           ...
           distinct c.idconsolidadocomi idconsolidadom,
                    (select tt.dstarea||': '
                       from tblslvtipotarea tt 
                      where tt.cdtipo = p_TipoTarea)||c.idconsolidadocomi||
              ...
              where c.idconsolidadocomi = cdet.idconsolidadocomi
                ...
                and cdet.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona = c.idpersona LM
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0 );
    EXCEPTION
    WHEN OTHERS THEN
    ...
    BEGIN
      OPEN p_Cursor FOR
           ...
           distinct pf.idpedfaltante idconsolidadom,
                    (select tt.dstarea||': '
                       from tblslvtipotarea tt 
                      where tt.cdtipo = p_TipoTarea)||pf.idpedfaltante||
              ...
              where pf.idpedfaltante = det.idpedfaltante
                ...
                --valida si es tarea faltante que el articulo se pikeo en tarea anterior
                and decode(p_TipoTarea,c_TareaFaltanteConsolFaltante,det.qtunidadmedidabasepicking,1) is not null
                --valida si es tarea faltante listar solo los que tienen diferencia
                and case 
                     --verifica si es pesable 
                     when det.qtpiezas<>0 
                      and p_TipoTarea = c_TareaFaltanteConsolFaltante 
                      and det.qtpiezas-nvl(det.qtpiezaspicking,0) <> 0 then 1
                     --verifica los no pesable
                     when det.qtpiezas = 0 
                      and p_TipoTarea = c_TareaFaltanteConsolFaltante 
                      and det.qtunidadmedidabase-nvl(det.qtunidadmedidabasepicking,0) <> 0 then 1
                     --si es tarea consolidado pedido faltante
                ...
                and det.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea = td.idtarea
                                             ...
                                             --and ta.idpersona = pf.idpersona LM
                                             and ta.cdtipo = p_TipoTarea
                                             and td.icdesasignar = 0   );                                       
  ...
  * %v 15/07/2020 - LM   se cambia el grupo de tareas a "ARMADOR"
  *****************************************************************************************************/
      ...
      OPEN p_Cursor FOR
             Select pe.Idpersona,
              ...
              where p.idpersona = pe.idpersona
                and upper(p.nmgrupotarea)='ARMADOR'
           order by Armador;
    ...
    BEGIN
      OPEN p_Cursor FOR
             Select m.idconsolidadom idconsolidado,
                    ...
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(det.cdarticulo,'UN'),
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(det.cdarticulo,'KG')) barras,                  
                    ...
                    (det.qtpiezas-nvl(det.qtpiezaspicking,0)),
                     ...
                     --marca de selectivos 1 si es de especificos o selectivo 0 sino 
                     decode(trim(gs.cdsector),'26',1,0) EsSelectivo
               from tblslvconsolidadom m,
                    tblslvconsolidadomdet det,
              ...
              where m.idconsolidadom = det.idconsolidadom
                ...
                and det.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                             and ta.idconsolidadom=m.idconsolidadom
                                             --and ta.idpersona= m.idpersona LM no tiene porque ser la misma persona
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0 );
    EXCEPTION
    WHEN OTHERS THEN
  ...
  * %v 15/09/2020 - LM   quito el filtro de persona que asigna contra tarea
  * %v 25/09/2020 - ChM  agrego order by por sector original del pedido
  * %v 30/09/2020 - ChM Agrego validación de tarea desasignada and td.icdesasignar = 0  
  *****************************************************************************************************/
    ...
    BEGIN        
      OPEN p_Cursor FOR
             Select cm.idconsolidadocomi idconsolidado,
                    --valida si esta en el sector consolidadoM 
                    nvl(PKG_SLV_CONSOLIDADOS.SectorConsolidadoM(null,cm.idconsolidadocomi,cdet.cdarticulo),
                    gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')') Sector,                      
                    cdet.cdarticulo cdArticulo,
                    --codigo de barras 
                    decode(cdet.qtpiezas,0,
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(cdet.cdarticulo,'UN'),
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(cdet.cdarticulo,'KG')) barras, 
                    art.cdarticulo || '- ' || des.vldescripcion Articulo,
                    PKG_SLV_Articulo.SetFormatoArticulosCod(art.cdarticulo,
                    --formato en piezas si es pesable  
                    decode(cdet.qtpiezas,0,
              ...
              where cm.idconsolidadocomi = cdet.idconsolidadocomi
                ...
                and cm.cdestado not in (C_FinalizadoConsolidadoComi,C_FacturadoConsolidadoComi,C_AfacturarConsolidadoComi)
                and sec.cdsector = gs.cdsector
                and art.cdarticulo = des.cdarticulo
                and gs.cdsucursal =  g_cdSucursal
                ...
                and cdet.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona= cm.idpersona LM no tiene porque ser la misma persona
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0 )
        --agrego order by por sector original del pedido                                     
        order by gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')'
        ;                                             
    EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
              ...
              where cp.idconsolidadopedido = cdet.idconsolidadopedido
                ...
                and cdet.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                             ...
                                             --and ta.idpersona= cp.idpersona LM no tiene por que ser la misma persona
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0  );
    EXCEPTION
    WHEN OTHERS THEN
      ...
      OPEN p_Cursor FOR
             Select f.idpedfaltante idconsolidado,
                    ...
                    detf.cdarticulo cdArticulo,
                     --codigo de barras 
                    decode(detf.qtpiezas,0,
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(detf.cdarticulo,'UN'),
                    PKG_SLV_ARTICULO.GetCodigoDeBarra(detf.cdarticulo,'KG')) barras, 
                    art.cdarticulo || '- ' || des.vldescripcion Articulo,                             
                    PKG_SLV_Articulo.SetFormatoArticulosCod(art.cdarticulo,
                    --formato en piezas si es pesable  
                    decode(detf.qtpiezas,0,
                    decode(p_TipoTarea,c_TareaFaltanteConsolFaltante,
                    (detf.qtunidadmedidabase-nvl(detf.qtunidadmedidabasepicking,0)),
                     detf.qtunidadmedidabase),
                    --si es faltante muestra diferencias
                    decode(p_TipoTarea,c_TareaFaltanteConsolFaltante,
                    (detf.qtpiezas-nvl(detf.qtpiezaspicking,0)),
                     detf.qtpiezas))) Cantidad,
                     --marca de selectivos 1 si es de especificos o selectivo 0 sino 
                     decode(trim(gs.cdsector),'26',1,0) EsSelectivo             
              ...
              where f.idpedfaltante = detf.idpedfaltante
                ...
                --valida si es tarea faltante que el articulo se pikeo en tarea anterior
                and decode(p_TipoTarea,c_TareaFaltanteConsolFaltante,detf.qtunidadmedidabasepicking,1) is not null
                --valida si es tarea faltante listar solo los que tienen diferencia
                and case 
                     --verifica si es pesable 
                ...
                and detf.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona= f.idpersona LM no tiene por que ser la misma persona
                                             and ta.cdtipo= p_TipoTarea
                                             and td.icdesasignar = 0  );
    EXCEPTION
    WHEN OTHERS THEN
  ...
  * %v 15/09/2020 - LM  quito el filtro de persona al asignar
  *****************************************************************************************************/
      ...
      if p_TipoTarea = c_TareaConsolidaMultiFaltante then
       ...
       select nvl(max(ta.prioridad),0)+1
        ...
        where ta.idpersonaarmador = p_IdArmador
          ...
          and ta.cdestado not in (6,9,17,24,32,35,42);
           ...
           insert into tblslvtareadet
                ...
                select seq_tareadet.nextval,
                 ...
                 where det.idconsolidadom=m.idconsolidadom
                   --and m.idpersona=p_IdPersona LM. no tiene porque ser la misma persona
           ...
           insert into tblslvtareadet
                ...
                select seq_tareadet.nextval,
                 ...
                 where det.idconsolidadopedido=p.idconsolidadopedido
                   --and p.idpersona=p_IdPersona   LM no tiene porque ser la misma persona
  ...
  *****************************************************************************************************/
  ...
  BEGIN
      ...
      if p_TipoTarea = c_TareaConsolidaPedidoFaltante then
       ...
       if p_TipoTarea = c_TareaFaltanteConsolFaltante then
        v_cdestado:= C_AsignadoTareaFaltConFalt; --Asignado TareaFaltanteConsolidadoFaltante  en tblslvestado
      end if;
           ...
           insert into tblslvtareadet
                ...
                select seq_tareadet.nextval,
                 ...
                 where det.idpedfaltante=pf.idpedfaltante
                   --and pf.idpersona=p_IdPersona LM no tiene por que ser la misma persona
  ...
  *****************************************************************************************************/
  ...
  BEGIN
     ...
     --TipoTarea 10,20 ConsolidadoM y ConsolidadoMFaltante
     if p_TipoTarea in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante) then
      SetAsignaArtConsolidadoM(p_cdArticulos,p_idConsolidado,p_TipoTarea,p_IdPersona,p_IdArmador,p_cdModoIngreso, p_idtarea,p_Ok);
      if p_Ok <> 1 then
        p_Ok    := 0;
    ...
    --TipoTarea 25 ConsolidadoPedido
    if p_TipoTarea = c_TareaConsolidadoPedido then
      SetAsignaArtConsolidadoPedido(p_cdArticulos,p_idConsolidado,p_TipoTarea,p_IdPersona,p_IdArmador,p_cdModoIngreso, p_idtarea,p_Ok);
      if p_Ok <> 1 then
        p_Ok    := 0;
        p_error := 'Error Asignando Armadores. Comuniquese con Sistemas!';
        ROLLBACK;
    ...
    --TipoTarea 40,44 Faltantes Consolidado pedido y Faltantes Consolidado faltantes
    if p_TipoTarea in (c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante) then
      SetAsignaArtPedidoFaltantes(p_cdArticulos,p_idConsolidado,p_TipoTarea,p_IdPersona,p_IdArmador,p_cdModoIngreso, p_idtarea,p_Ok);
      if p_Ok <> 1 then
        p_Ok    := 0;
        p_error := 'Error Asignando Armadores. Comuniquese con Sistemas!';
        ROLLBACK;
    ...
    if p_TipoTarea in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
      SetAsignaArtConsolidadoComi(p_cdArticulos,p_idConsolidado,p_TipoTarea,p_IdPersona,p_IdArmador,p_cdModoIngreso, p_idtarea,p_Ok);
      if p_Ok <> 1 then
        p_Ok    := 0;
        p_error := 'Error Asignando Armadores. Comuniquese con Sistemas!';
        ROLLBACK;
  ...
  * %v 12/06/2020 - ChM  Agrego Tarea Faltante Consolidado Faltante
  * %v 06/10/2020 - ChM  creo parametro para mostrar con o sin idremitos
                         si p_icconremitos 1 muestra remito 
  *****************************************************************************************************/
  PROCEDURE GetTareaAsigConsolidado(p_idConsolidado   IN  Integer,
                                    p_TipoTarea       IN  Tblslvtipotarea.cdtipo%type,
                                    p_icconremitos    IN  integer default 0,
  ...
  v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetTareaAsigConsolidado';
    BEGIN
     ...
     distinct  
              ...
              e.dsestado Estado,
              --muestra remitos según parametro
              decode(p_icconremitos,1,nvl(re.idremito,0),0) idremito,
              decode(p_icconremitos,1,nvl(re.nrocarreta,0),0) roll                               
         from tblslvtarea ta 
    left join (tblslvremito re)
        ...
        where ta.idpersonaarmador = p.idpersona
          ...
          and case when p_tipoTarea = c_TareaConsolidadoMulti        
                   ...
                   when p_tipoTarea = c_TareaConsolidadoPedido       
                    ...
                    and ta.cdtipo = p_tipoTarea then 1 
                   --si es consolidado faltante devuelve los 40 y 44 
     ...
     union all
     --listado de remitos de distribución de faltantes
...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  /****************************************************************************************************
  * %v 19/08/2020 - ChM  Versión inicial GetPanelArmadores
  * %v 19/08/2020 - ChM  lista todos los Armadores y su tarea asociada 
  *****************************************************************************************************/
  PROCEDURE GetPanelArmadores(p_Cursor     OUT CURSOR_TYPE) IS

   v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GETPanelArmadores';
      ...
      with armadores as(       
                     Select pe.Idpersona,
                      ...
                      where p.idpersona = pe.idpersona
                        and upper(p.nmgrupotarea)='ARMADOR'),  
            tarea as(
                   Select   ta.idpersonaarmador,
                            tp.dstarea||' '||
                            coalesce(ta.idpedfaltante,ta.idconsolidadom,
                                     ta.idconsolidadopedido,ta.idconsolidadocomi) Actividad,
                            PKG_SLV_TAREAS.NombreCliente(ta.idtarea) Cliente,           
                            decode(ta.cdmodoingreso,0,'HandHeld',1,'Manual') Modo,
                            e.dsestado,
                            nvl(to_char(ta.dtinicio,'dd/mm/yyyy hh24:mi:ss'),'-') Fecha_Inicio,
                            nvl(to_char(ta.dtfin,'dd/mm/yyyy hh24:mi:ss'),'-') Fecha_Fin,
                            ta.dtinsert                 
                       from tblslvtarea     ta,
                            tblslvtipotarea tp,
                            tblslvestado    e
                      where ta.cdtipo = tp.cdtipo
                        and ta.cdestado = e.cdestado                           
                        and ta.dtinsert in (select max(ta2.dtinsert) from tblslvtarea ta2
                                              where ta2.idpersonaarmador=ta.idpersonaarmador )
                        -- solo estado en curso o asignado                          
                        and ta.cdestado in(4,5,7,8,15,16,22,23,30,31,33,34,38,40,41) 
                   )
                   select armadores.Idpersona IdArmador,
                          armadores.Armador,
                          nvl(tarea.Actividad,'-') Actividad,
                          nvl(tarea.cliente,'-') Cliente,
                          nvl(tarea.Modo,'-') Modo,                      
                          nvl(tarea.dsestado,'-') dsestado,
                          nvl(tarea.Fecha_Inicio,'-') Fecha_Inicio,
                          nvl(tarea.Fecha_Fin,'-') fecha_Fin
                     from armadores
                left join (tarea)
                       on (armadores.Idpersona = tarea.idpersonaarmador)  
                 order by tarea.dtinsert desc,
                          armadores.Armador;
    EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);
  END GetPanelArmadores;
   /****************************************************************************************************
  * %v 19/08/2020 - ChM  Versión inicial NombreCliente
  * %v 19/08/2020 - ChM  Desde un idtarea devuelve el nombre del cliente
  *****************************************************************************************************/
  FUNCTION  NombreCliente(p_IdTarea            tblslvtarea.idtarea%type) 
                          RETURN VARCHAR2 IS
     
  v_cdtipo                tblslvtipotarea.cdtipo%type:=null;
  v_idconsolidado         tblslvtarea.idconsolidadom%type:=null;   
  v_DsCliente             varchar2(200);
                    
    BEGIN
     --obtengo el tipo tarea
       begin
          select ta.cdtipo,
                 coalesce(ta.idpedfaltante,ta.idconsolidadom,
                          ta.idconsolidadopedido,ta.idconsolidadocomi)               
            into v_cdtipo,
                 v_idconsolidado
            from tblslvtarea ta
       ...
       exception
          when others then
            return '-';    
             ...
             if v_cdtipo in(c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante,
                            c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante) then
               return '-';
              end if;
             --actualiza la cantidad picking en consolidadocomi y FaltanteConsolidadocomi
             if v_cdtipo in(c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
                ...
                select TRIM(NVL (e.dsrazonsocial, e.dsnombrefantasia))||' ('||TRIM(e.cdcuit)||')' razonsocial
                 ...
                 where cc.idcomisionista = e.identidad
                   and cc.idconsolidadocomi = v_idconsolidado
                   and rownum=1;
                 exception
                   when others then
                     v_DsCliente:='-'; 
             ...
             if v_cdtipo = c_TareaConsolidadoPedido then
              --descripcion del consolidado pedido
                begin
                select TRIM(NVL (e.dsrazonsocial, e.dsnombrefantasia))||' ('||TRIM(e.cdcuit)||')' razonsocial
                  into v_DsCliente
                  from tblslvconsolidadopedido cp,
                 ...
                 where cp.identidad = e.identidad
                   and cp.idconsolidadopedido = v_idconsolidado
                   and rownum=1;
                 exception
                   when others then
                     v_DsCliente:='-'; 
  ...
  END NombreCliente;
  /****************************************************************************************************
  * %v 19/08/2020 - ChM  Versión inicial GetDetalleArmadores
  * %v 19/08/2020 - ChM  lista de un armador las tareas segun periodo
  * %v 15/09/2020 - LM   agrego el IDTarea
  *****************************************************************************************************/
  PROCEDURE GetDetalleArmadores(p_idarmador      IN  personas.idpersona%type,                  
                                p_dtfechadesde   IN  DATE,
                                p_dtfechahasta   IN  DATE, 
   ...
   v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetDetalleArmadores';
   v_dtDesde date;
   v_dtHasta date;
  ...
  BEGIN
    ...
    Select upper(pe.dsnombre) || ' ' || upper(pe.dsapellido) Armador
      into p_dsarmador
      from personas pe
     where pe.idpersona = p_idarmador;
      ...
      OPEN p_Cursor FOR
             Select ta.idtarea,
                    ...
                    decode(ta.cdmodoingreso,0,'HandHeld',1,'Manual') Modo,
                    e.dsestado,
                    decode(instr(e.dsestado,'Finalizado'),0,1,0) Desasignar,
                    nvl(to_char(ta.dtinicio,'dd/mm/yyyy hh24:mi:ss'),'-') Fecha_Inicio,
                    nvl(to_char(ta.dtfin,'dd/mm/yyyy hh24:mi:ss'),'-') Fecha_Fin, 
                    ta.dtinsert Fecha_Asignacion                
               from tblslvtarea     ta,
                    tblslvtipotarea tp,
                    tblslvestado    e
              where ta.cdtipo = tp.cdtipo
                ...
                and ta.dtinsert between v_dtDesde and v_dtHasta    
           order by ta.dtinsert desc,
                    ta.prioridad;
  ...
  *****************************************************************************************************/
  ...
  WHEN NO_DATA_FOUND THEN
     p_Ok    := 0;
     p_error:='codigo de barras no pertenece a un artículo de la tarea: '||to_char(p_IdTarea);
  WHEN OTHERS THEN
     p_Ok    := 0;
      ...
      v_estado:=0;
         select count(*)
          ...
          where ta.idtarea = dta.idtarea
            ...
            --  tareas asignadas o en curso segun tblslvestado
            and ta.cdestado in (4,5,7,8,15,16,22,23,30,31,33,34,40,41);
   else
       --verifica si la tarea esta asignada al armador
      v_estado:=0;
         select count(*)
  ...
  * %v 24/08/2020 - ChM Ajuste para ubicación del remito de consolidadoM
  * %v 30/09/2020 - ChM Agrego validación de tarea desasignada and td.icdesasignar = 0  
  *****************************************************************************************************/
  ...
  BEGIN
       ...
       if p_IdtareaManual is not null then
         begin
             ...
             if v_cdtipotarea <> P_tipoTareaManual and P_tipoTareaManual is not null then
                p_Ok    := 0;
                p_error:='La Tarea '||to_char(p_IdtareaManual)||' no corresponde al tipo '||to_char(P_tipoTareaManual)||' enviado!';
                return;
             end if;
              --valida si el cdmodoingreso es handheld y la tarea es manual error
              if v_cdmodoingreso = c_TareaHandHeld then         
                   p_Ok    := 0;
                   p_error:='La tarea: '||to_char(p_IdtareaManual)||' esta definida para picking con HandHeld!';
                   return;            
              end if;
              -- valida si la tarea pertenece al armador y esta asignada o en curso
              if VerificaPersonaTarea(null,p_IdtareaManual) = 0  then
                  p_Ok:=0;
                  p_error:='Tarea Finalizada!';
                  rollback;
                  return;
              end if;
         exception
           when no_data_found then
                p_Ok    := 0;
                p_error:='La Tarea no existe';
                return;
         end;
         else 
           --si p_IdtareaManual es null v_cdmodoingreso se setea a HH 0 ingreso handheld 1 ingreso manual 
          v_cdmodoingreso:=0;   
      end if;
    ...
    for tarea in
      (select ta.idtarea,
        ...
        where ta.cdtipo = tip.cdtipo
          ...
          --valida modo ingreso    
          and ta.cdModoIngreso = v_cdmodoingreso              
    ...
    --devuelve la descripción del armador
     ...
     -- OJO probar en HH como incorporar el sector
     -------------------------------------------------------------------------------------
     ...
     open p_cursor for
           select dta.cdarticulo Cdarticulo,
                  ...
                  -- Ajuste para ubicación del remito de consolidadoM ChM 
                  PKG_SLV_Articulo.GetUbicacionArticulos(dta.cdarticulo,p_IdTarea) Ubicacion,
    ...
    BEGIN
         select count(*)
          ...
          where ta.idtarea = dta.idtarea
            and ta.idtarea = p_IdTarea
            and ta.idpersonaarmador = p_IdPersona
            --  tareas disponibles segun tblslvestado Asignado o en curso
            and ta.cdestado in (4,5,7,8,15,16,22,23,30,31,33,34,40,41)
  ...
  BEGIN
     select ta.cdtipo
       ...
       if v_tipotarea = c_TareaConsolidadoMulti then
        ...
        --tipo faltante consolidado faltante
        if v_tipotarea = c_TareaFaltanteConsolFaltante then
           if p_band = 1 then -- 1 en curso 0 finalizado
           update tblslvtarea ta
              set ta.cdestado = C_EnCursoTareaFaltConFalt,
                  ta.dtupdate = sysdate,
                  ta.dtinicio = sysdate
            where ta.idtarea = p_IdTarea;
         else
           update tblslvtarea ta
              set ta.cdestado = C_FinalizadoTareaFaltConFalt,
                  ta.dtupdate = sysdate,
                  ta.dtfin = sysdate
            where ta.idtarea = p_IdTarea;
         end if;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
    --recupera el valor picking del detalle tarea
     ...
     if p_cdunidad not in ('UN','KG','PZA') then
       V_UxB:=posapp.n_pkg_vitalpos_materiales.GetUxB(p_cdArticulo,p_cdunidad);
     ...
     if v_cantidadBasePiezas <= (v_cant_piezas-v_cant_piezas_pick) then
      ...
      --verifica si la cantidad a regisitrar es mayor a cero, sino, no la agrega al remito.
       if p_icgeneraremito = 1 and p_idRemito <> 0 and v_cantidadBase > 0 then      
          v_res:=PKG_SLV_REMITOS.SetDetalleRemito(p_idRemito,p_idTarea,p_cdArticulo,v_cantidadBase,p_cdunidad,p_idpedidobolsin,p_cantidad_pes);         
          if v_res = -1 then
            return -3; -- devuelve -3 si la cantidad <> 1 en pesable
            end if;
          if v_res <> 1 then
  ...
  *****************************************************************************************************/
  FUNCTION VerificaDetalleTarea(p_IdTarea          tblslvtarea.idtarea%type,
    ...
    v_modulo               varchar2(100) := 'PKG_SLV_TAREAS.VerificaDetalleTarea';
    v_cantidad_base       tblslvtareadet.qtunidadmedidabase%type;
    v_cantidad_pick       tblslvtareadet.qtunidadmedidabasepicking%type;
  ...
  * %v 11/05/2020 - ChM  Finaliza la tarea por completo y agrega 0 en los faltantes si existen
  *                       (creada para picking manual)
  * %v 25/08/2020 - ChM  ajusto para usarlo en el picking HH por eliminar el cierre automatico de tarea 
  *****************************************************************************************************/
      ...
      if p_idremito <> 0 then
        ...
        if PKG_SLV_REMITOS.SetFinalizarRemito(p_idremito) = 0 then
          ...
          n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       ' Error: ' || SQLERRM); 
          p_error := 'No es posible actualizar Remito: ' || to_char(p_idremito) ||
                     'Comuniquese con Sistemas!';
          rollback;
          return;
        else
          --verifica si asigna ubicación
           if p_idubicacion <> 0 then
              PKG_SLV_REMITOS.SetRemitoUbicacion(p_idubicacion,p_idRemito,p_Ok,p_error);
              if p_ok = 0 then
                  n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       ' Error: ' || SQLERRM);               
  ...
  END SetFinTarea;
  /****************************************************************************************************
  * %v 23/07/2020 - ChM  Solo para prueba valida si llego al porcentaje picking del parametro
  *****************************************************************************************************/
  FUNCTION VerificaTope(p_IdTarea            tblslvtarea.idtarea%type,
                        p_cdarticulo         tblslvtareadet.cdarticulo%type,                              
                        p_porcentaje         integer)
                        return integer is
                        
  v_qtunidadmedidabase                 tblslvtareadet.qtunidadmedidabase%type;
  v_qtunidadmedidabasepicking          tblslvtareadet.qtunidadmedidabasepicking%type;
  v_qtpiezas                           tblslvtareadet.qtpiezas%type;
  v_qtpiezaspicking                    tblslvtareadet.qtunidadmedidabase%type;                      
  
  begin
    
    select sum(td.qtunidadmedidabase),
           sum(td.qtunidadmedidabasepicking),
           sum(td.qtpiezas),
           sum(td.qtpiezaspicking)
      into v_qtunidadmedidabase,
           v_qtunidadmedidabasepicking,
           v_qtpiezas,
           v_qtpiezaspicking     
      from tblslvtareadet td
     where td.idtarea=p_IdTarea
       and td.cdarticulo = p_cdarticulo;
    --verifica si alcanzo la cantidad deseada  
    if v_qtpiezas <> 0 and v_qtpiezaspicking>=round(v_qtpiezas*(p_porcentaje/100)) then
         return 0;
    else      
         if v_qtpiezas = 0 and v_qtunidadmedidabasepicking>=round(v_qtunidadmedidabase*(p_porcentaje/100)) then
          return 0;
        else
          return 1;  
        end if;
  ...
  /****************************************************************************************************
  * %v 23/07/2020 - ChM  Solo para prueba recibe un idtarea y piquea segun un porcentaje de faltantes
                         los articulos que la conforman. 
  *****************************************************************************************************/
  PROCEDURE  PikingMasivoTarea(p_IdTarea          IN   tblslvtarea.idtarea%type,
                               p_IdPersonaManual  IN   personas.idpersona%type default null,                               
                               p_porcentaje       IN   integer,
      ...
      v_Modulo                VARCHAR2(100) := 'SLVAPP.PKG_SLV_TAREAS.PikingMasivoTarea';
      v_icgeneraremito        tblslvtipotarea.icgeneraremito%type;
      v_idremito              tblslvremito.idremito%type:=null;
  ...
  BEGIN
   -- validar si la tarea genera remito
     ...
     select tp.icgeneraremito
      ...
      where ta.cdtipo = tp.cdtipo
        and ta.idtarea = p_IdTarea;       
     --si genera remito busca el idremito   
        if v_icgeneraremito = 1 then
          begin
              select re.idremito,re.nrocarreta
                into v_idremito,V_nrocarreta 
                from tblslvremito re
               where re.idtarea = p_IdTarea;  
          exception 
            when no_data_found then     
              p_Ok    := 0;
              p_error:='La Tarea: '||to_char(p_Idtarea)||' genera remito y no ha ingresado numero de Carreta';
               return;
          end;  
        else
           v_idremito:=null;
      ...
      exception
        when others then 
           p_Ok    := 0;
           p_error:='La Tarea: '||to_char(p_Idtarea)||' No Existe';
           return;   
     end; 
   ...
   for art in 
       (select distinct              
               tdet.cdarticulo,
               --valida pesables
               decode(tdet.qtpiezas,0,
               --valida indivisible
               decode(PKG_SLV_ARTICULO.GetValidaUnidadVentaMinimaArt(tdet.cdarticulo,'BTO'),'-',
               PKG_SLV_ARTICULO.GetCodigoDeBarra(tdet.cdarticulo,'UN'),
               PKG_SLV_ARTICULO.GetCodigoDeBarra(tdet.cdarticulo,'BTO')),
               PKG_SLV_ARTICULO.GetCodigoDeBarra(tdet.cdarticulo,'KG')) barras 
          from tblslvtareadet tdet               
         where tdet.idtarea = p_IdTarea)          
   loop   
      while verificaTope(p_IdTarea,art.cdarticulo,p_porcentaje)=1 loop                     
          SetRegistrarPickingM(v_idremito,
                               v_nrocarreta,
                               art.barras,
                               1, --p_cantidad                              
                               p_IdTarea,
   ...
   EXCEPTION
    WHEN OTHERS THEN
       n_pkg_vitalpos_log_general.write(2,
                                       ...
                                       '  Error: ' || SQLERRM);
      p_Ok    := 0;
      p_error:='Tarea: '||to_char(p_Idtarea)||' Error Picking MASIVO';

   END  PikingMasivoTarea;
  
   /****************************************************************************************************
  * %v 11/05/2020 - ChM  Versión inicial SetRegistrarPickingM
  * %v 11/05/2020 - ChM  para un idpersona registra el picking manual
  * %v 02/07/2020 - LM. se agrega la validacion y parametro del idpedidoBolsin
  * %v 05/08/2020 - ChM ajustes para ubicación de remitos
  *****************************************************************************************************/
  ...
  BEGIN
     ...
     -- recupera si la tarea genera ubicación
   ...
   if v_cdmodoingreso <> p_cdmodoingreso  then
      p_Ok    := 0;
      p_error:='La Tarea: '||to_char(p_Idtarea)||' no corresponde al tipo picking enviado!';
      return;
   end if;
   ...
   if v_cdmodoingreso <> c_TareaHandHeld  then
      if p_IdPersonaManual is null then
         p_Ok    := 0;
         p_error:='La Tarea: '||to_char(p_Idtarea)||' está en modo ingreso MANUAL con IdPersona en NULL';
         return;
      end if;
   end if;
   ...
   if p_cdBarras = 'FIN' then
     SetFinTarea (p_IdTarea,p_idRemito,p_idubicacion,v_idpersonaarmador,p_Ok,p_error);
   if p_Ok = 0 then
     return;
   end if; 
    p_FinTarea:=1;
  ...
  * %v 02/07/2020 - LM   se agrega la validacion y parametro del idPedidoBolsin 
  * %v 20/07/2020 - ChM  valida unidad de venta minima 
  * %v 19/08/2020 - ChM  ajustes para ubicación de remitos (p_idUbicacion)
  * %v 25/08/2020 - ChM  incorporo p_cdBarras = 'FIN' por eliminar el fin tarea automatico HH, 
                         por diferencia funcional con picking manual
  *****************************************************************************************************/
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona        IN   personas.idpersona%type,
   ...
   BEGIN
     ...
     -- validar si la tarea genera remito y ubicación
     select tp.icgeneraremito,            
            ta.cdmodoingreso
       into v_icgeneraremito,            
            v_modoIngreso
      ...
      --valida si se desea finalizar la tarea
      --incorporo por eliminar el fin tarea automatico HH, por diferencia funcional con picking manual
       if p_cdBarras = 'FIN' then
         SetFinTarea (p_IdTarea,p_idRemito,p_idubicacion,p_IdPersona,p_Ok,p_error);       
         p_FinTarea:=1; 
         return; 
       end if;
     ...
     if upper(trim(p_cdBarras)) ='F' and v_idremito <> 0 then
         if PKG_SLV_REMITOS.SetFinalizarRemito(v_idremito)=0 then
           ...
           n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       ' Error: ' || SQLERRM);  
           p_error:='No es posible actualizar Remito: '||to_char(v_idremito)||' Comuniquese con Sistemas!';
           rollback;
           return;
         else -- se finaliza remito y termina el procedimiento
           --verifica si asigna ubicación
           if p_idubicacion <> 0 then
              PKG_SLV_REMITOS.SetRemitoUbicacion(p_idubicacion,v_idRemito,p_Ok,p_error);
              if p_ok = 0 then
                  n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       ' Error: ' || SQLERRM);               
          ...
          if v_idremito = 0 then
           p_Ok:=0;
           p_error:='No es posible finalizar Remito en 0.';
           rollback;
           return;
          end if;
      end if;
   ...
   if length (p_cdarticulo)>1 and p_cantidad >= 0  then     
     ...
     --valida unidad de venta minima indivisibles ChM 20/07/2020    
        v_cdunidadVentamin:=pkg_slv_articulo.GetValidaUnidadVentaMinimaArt(p_cdArticulo,v_cdunidad);     
     ...
     if trim(v_cdunidad) = '-' then
          ...
          v_res:=SetDetalleTarea(p_IdTarea,p_cdArticulo,p_cantidad,v_cdunidad,v_cantidad_pes,v_icgeneraremito,p_IdPedidoBolsin,v_idremito);
          ...
          if v_res = -2 then
           ...
           --Error si la Cantidad picking de pesable <> 1
          if v_res = -3 then
             p_Ok:=0;
             p_error:='La cantidad ingresada del pesable debe ser 1 pieza!';
             rollback;
             return;
          end if;
    ...
    --LM: si no viene de bolsin (p_idpedidobolsin null), hacer commint. En los bolsines se hara commit al finalizar el loop
    if nvl(p_idpedidobolsin,'X') <> 'X' then
      commit;
    end if;
    EXCEPTION
    WHEN OTHERS THEN
  ...
  * %v 24/08/2020 - ChM  Ajuste para ubicación del remito de consolidadoM 
  * %v 07/09/2020 - LM   se ajusta el orden de las ubicaciones
  *****************************************************************************************************/
  PROCEDURE GetArticulosXTarea(p_IdTarea      IN  tblslvtarea.idtarea%type,
                               p_DsSucursal   OUT sucursales.dssucursal%type,                   
                               p_CursorCAB    OUT CURSOR_TYPE,                                       
  ...
  BEGIN
    ...
    --cursor de detalle de articulos
     OPEN p_Cursor FOR
                   --valida si esta en el sector consolidadoM 
            select nvl(PKG_SLV_CONSOLIDADOS.SectorConsolidadoM(p_IdTarea,null,A.COD)
                   ,A.Sector) Sector,
                   A.articulo,
                   --codigo de barras
                   decode(A.basepza,0,
              ...
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                         ...
                         --  det.qtunidadmedidabasepicking picking,
                           ...
                           --Ajuste para ubicación del remito de consolidadoM ChM
                           PKG_SLV_Articulo.GetUbicacionArticulos(ART.cdarticulo,p_IdTarea) UBICACION,
                           gs.orden
                      from tblslvtarea ta,
                           tblslvtareadet det,
                           tblslv_grupo_sector gs,
       ...
       if v_prioridad is null then
        ...
        insert into tblslvtarea
             ...
             values (seq_tarea.nextval,
                     ...
                     null,   --dtupdate
                     1,
                     g_cdSucursal);   --pausa aplioca solo para HANDHELP
    EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
  ...
  * %v 30/09/2020 - ChM  Agrego desasignar tarea borrado logico set det.icdesasignar = 1
  *****************************************************************************************************/
  PROCEDURE SetLiberarArmador(p_IdArmador   IN   personas.idpersona%type,
                              p_idTarea     IN   tblslvtarea.idtarea%type default null,
                              p_Ok          OUT  number,
   ...
   v_icgeneraremito           tblslvtipotarea.icgeneraremito%type;
    ...
    BEGIN
      --Verifica si la tarea esta finalizada
      if p_idTarea is not null then
         begin 
           select ta.idtarea
            ...
            where ta.idpersonaarmador = p_IdArmador
              and ta.idtarea = p_idTarea
               -- tareas disponibles Finalizadas
              and ta.cdestado in (select e.cdestado
                                    from tblslvestado e
                                   where e.dsestado like '%Finalizado%');
             if v_tarea is not null then
                p_Ok:=0;
                p_error:='Tarea finalizada. no es posible Desasignar!';
                rollback;
                return;
              end if;                       
            exception
      ...
      begin 
          select count(*)
            ...
            where ta.idtarea = det.idtarea
              and ta.idpersonaarmador = p_IdArmador
              and (p_idTarea is null or ta.idtarea = p_idTarea)
             ...
             where ta.idpersonaarmador = p_IdArmador
               ...
               and ta.cdestado in (select e.cdestado
                                    ...
                                    where e.dsestado like '%Asignado%'
                                      ...
                                      and e.cdestado <> 38))
          loop                                      
             --elimina logicamente todas los detalle tareas asignadas al armador
              update tblslvtareadet det
                 set det.icdesasignar = 1
               where det.idtarea = tareasig.idtarea;
              if SQL%ROWCOUNT = 0  then
                p_Ok:=0;
                n_pkg_vitalpos_log_general.write(2,
                                             'Modulo: ' || v_modulo ||
                                             '  Error: Imposible eliminar detalle de tareas asignadas Tarea: '||tareasig.idtarea );
                p_error:='Comuniquese con Sistemas!';
                rollback;
                return;
              end if;
            -- pone la tarea Finalizada. 0 en el parametro
            if SetEstadoTarea(tareasig.idtarea,0)<>1 then
                p_Ok:=0;
                n_pkg_vitalpos_log_general.write(2,
                                             'Modulo: ' || v_modulo ||
                                             '  Error: Imposible finalizar tareas asignadas Tarea: '||tareasig.idtarea );
                p_error:='Comuniquese con Sistemas!';
                rollback;
                return;
            end if;
  ...
  --recorre las tareas en curso para finalizar picking del detalle tarea
   ...
   loop
     ...
     if(v_tarea <> 1) then
         ...
         n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: Imposible finalizar tarea '||to_char(curso.idtarea)||'artículo: '||curso.cdarticulo );
          p_error:='Comuniquese con Sistemas!';         
         rollback;
         return;
     end if;
   ...
   loop
     ...
     if(v_tarea <> 1) then
         ...
         n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: Imposible finalizar tarea: '||to_char(curso.idtarea) );
          p_error:='Comuniquese con Sistemas!';          
         rollback;
         return;
     end if;
     ...
     --si genera remito los finaliza
     if v_icgeneraremito = 1 then
       for remitos in
                  (select re.idremito,
                          re.idubicacionremito                     
                     from tblslvremito re
                    where re.idtarea = curso.idtarea
                      --solo remitos no finalizados
                      and re.cdestado <> C_FinalizadoRemito )
        loop
            if PKG_SLV_REMITOS.SetFinalizarRemito(remitos.idremito)=0 then
               p_Ok:=0;
               n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: No es posible actualizar remito '||to_char(remitos.idremito));
               p_error:='Comuniquese con Sistemas!';               
               rollback;
               return;
            else
              if NVL(remitos.idubicacionremito,0) <> 0 then
                  PKG_SLV_REMITOS.SetRemitoUbicacion(remitos.idubicacionremito,remitos.idremito,p_Ok,p_error);
                  if p_ok = 0 then
                    
                     n_pkg_vitalpos_log_general.write(2,'Modulo: ' || v_modulo ||
                                       '  Error: No es posible actualizar UBICACION remito '||to_char(remitos.idremito));
                     p_error:='Comuniquese con Sistemas!';            
                     rollback;
                     return;
                  end if;
 ...
 /****************************************************************************************************
  * %v 19/06/2020 - LM  Obtiene los articulos de la tarea por codigo de barra. Se llama desde posapp
  * %v 31/08/2020 - LM  se corrigen los estados de la tarea cons comisionista
  *****************************************************************************************************/
  PROCEDURE GetArtTareaSelectivo( p_barras in VARCHAR2,
                                  r_cursor OUT CURSOR_TYPE,
                                  p_error  OUT VARCHAR2) IS

    v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetArtTareaSelectivo';
   -- v_identif   integer;
    v_tipoTarea   integer;
    v_NroTarea    integer;
    v_estado      integer;
  ...
  v_error:='';
  p_error:='';
   SELECT SUBSTR(p_barras,1,2) INTO v_tipoTarea FROM DUAL;
   SELECT TO_NUMBER( SUBSTR(p_barras,3,9))INTO v_NroTarea FROM DUAL;
    

   if   v_tipoTarea=25 then --consolidado pedido
      begin
         select t.cdestado into v_estado 
         from tblslvtarea t, tblslvconsolidadopedido cp
         where t.idtarea=v_NroTarea
         and t.idconsolidadopedido=cp.idconsolidadopedido;
      Exception
         When no_data_found Then
            p_error := 'El Consolidado Pedido no existe.';
            Return;
      End;
     if v_estado!=15 and v_estado!=16 then --15. Asignado   16.En curso
       p_error:='El Pedido no esta en curso.';
       return;
     end if;
     select count(*) into v_cantArt from (
              select td.idtarea Nro_identificador, td.cdarticulo, td.qtunidadmedidabase-nvl(td.qtunidadmedidabasepicking,0) CantPed
              from tblslvtareadet td, tblslv_grupo_sector gs
              where td.idgrupo_sector=gs.idgrupo_sector
              and gs.cdgrupo=99
              and td.idtarea=v_NroTarea
              ) CPP
              where CPP.CantPed>0;

     if v_cantArt=0 then
       p_error:='El Pedido no tiene articulos pendientes en especificos.';
       return;
     end if;
     OPEN r_cursor FOR
             select * from (
     ...
     if v_tipoTarea=50 then --consolidado comi
        begin
             select t.cdestado into v_estado 
             from tblslvtarea t, tblslvconsolidadocomi cc
             where t.idtarea=v_NroTarea
             and t.idconsolidadocomi=cc.idconsolidadocomi;
        Exception
        When no_data_found Then
            p_error := 'El Consolidado Comisionista no existe.';
            Return;
        End;
        if v_estado!=33 and v_estado!=34 then
           p_error:='La tarea Consolidado Comisionista no esta en curso.';
           return;
         end if;
      ...
      if v_cantArt=0 then
       p_error:='El Consolidado Sector no tiene articulos pendientes en especificos.';
       return;
     end if;
       OPEN r_cursor FOR
              select td.idtarea Nro_identificador, td.cdarticulo, td.qtunidadmedidabase-nvl(td.qtunidadmedidabasepicking,0) CantPed
              ...
              and gs.cdgrupo=99
              and td.idtarea=v_NroTarea
              and (td.qtunidadmedidabase- nvl(td.qtunidadmedidabasepicking,0))>0;
            
     else
       p_error:='Codigo de barras no encontrado';
     end if;
   end if;
  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       ...
                                       'Detalle Error: ' || v_error ||
                                       '  Error: ' || SQLERRM);
      p_error:='No es posible encontrar la tarea. Comuniquese con Sistemas.';
      ROLLBACK;
  END GetArtTareaSelectivo;
  
 /****************************************************************************************************
  * %v 30/06/2020 - LM  Obtiene los llaveros con sus articulos por idTarea
  *****************************************************************************************************/
  PROCEDURE GetBolsinXIdTarea(p_idtarea IN tblslvtarea.idtarea%TYPE,
                                       p_cursor OUT CURSOR_TYPE) IS
      v_Modulo    VARCHAR2(100) := 'SLVAPP.PKG_SLV_TAREAS.GetBolsinXIdTarea';

  BEGIN
   OPEN p_cursor FOR
           select idtarea, idpedido, icllavero, cdarticulo, qtunidadmedidabase, qtunidadpedido,cdunidadmedida,
           case  qtunidadpedido
                    when 1 then
                       eancode
                    else
                       qtunidadpedido||'*'|| trim( eancode)
                   end eanCode,  sqdetallepedido, dsarticulo, qtunidadpedfacturada,fecha

            from (SELECT ll.idtarea idtarea, dp.idpedido,ll.icllavero, dp.cdarticulo,
                  dp.qtunidadmedidabase, dp.qtunidadpedido, dp.cdunidadmedida,
                 (
            select b.cdeancode from barras b
            where b.cdunidad=dp.cdunidadmedida
            and b.cdarticulo=dp.cdarticulo
            and b.icprincipal=1
            and rownum<2
            ) eancode, dp.sqdetallepedido, dp.dsarticulo, nvl(dp.qtunidadpedfacturada,0)qtunidadpedfacturada, p.dtaplicacion fecha
                 from posapp.llaverfcespec ll, pedidos p, detallepedidos dp
                 where ll.cdestado=1
                 and ll.cdcaja is null
                 and ll.idtarea=p_idtarea
                 and ll.iddoctrx=p.iddoctrx
                 and p.idpedido=dp.idpedido
                 and dp.dsobservacion<>'PR') art;
...
END  GetBolsinXIdTarea;
  

/****************************************************************************************************
  * %v 01/07/2020 - LM  Registra en el pick los articulos del bolsin
  *****************************************************************************************************/
  PROCEDURE SetBolsinEnPick(  p_idtarea           IN   tblslvtarea.idtarea%TYPE, 
                              p_idBolsin          IN   posapp.llaverfcespec.icllavero%type,
                              p_IdPersonaManual   IN   personas.idpersona%type  default null,
                              ...
                              p_Ok                OUT  number,
                              p_error             OUT  varchar2) IS
      v_Modulo                VARCHAR2(100) := 'SLVAPP.PKG_SLV_TAREAS.SetBolsinEnPick';
      v_idRemito              TBLSLVREMITO.Idremito%TYPE;
      v_icGeneraRemito        TBLSLVTIPOTAREA.Icgeneraremito%type;
      v_idPersonaArmador      PERSONAS.Idpersona%TYPE;
      --v_finTarea              integer;
      v_carretaIni            TBLSLVREMITO.Nrocarreta%TYPE;
      --cur_art                 cursor_type;
  BEGIN
    p_ok:=1;
    p_error:='';
    
    select ta.idpersonaarmador, tt.icgeneraremito into v_idPersonaArmador, v_icGeneraRemito
    from tblslvtarea ta, tblslvtipotarea tt
    where ta.idtarea = p_idtarea
    and tt.cdtipo=ta.cdtipo;
    
   for r_artBolsin in (select idtarea, idpedido, icllavero, cdarticulo, qtunidadmedidabase, qtunidadpedido,cdunidadmedida,
                      eanCode,  sqdetallepedido, dsarticulo, qtunidadpedfacturada,fecha
                      from (SELECT ll.idtarea idtarea, dp.idpedido,ll.icllavero, dp.cdarticulo,
                        dp.qtunidadmedidabase, dp.qtunidadpedido, dp.cdunidadmedida,
                       (
                        select b.cdeancode from barras b
                        ...
                        and rownum<2
                             ...
                             and ll.cdcaja is null
                             and ll.idtarea=p_idtarea
                             and ll.icllavero=p_idBolsin
                             and ll.iddoctrx=p.iddoctrx
                             and p.idpedido=dp.idpedido
                             and dp.dsobservacion<>'PR')
                          )
   loop
     if v_icGeneraRemito = 1 then
        PKG_SLV_REMITOS.getcarreta(v_idPersonaArmador,p_idtarea,v_idRemito,v_carretaIni);
     end if;
       
     if nvl(p_idpersonamanual,'X')='X' then  --es picking hh
       PKG_SLV_TAREAS.SetRegistrarPicking(p_idpersona,v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, r_artbolsin.cdarticulo, p_idtarea,r_artbolsin.idpedido,p_idubicacion, p_Fintarea, p_ok, p_error); 
     else
       PKG_SLV_TAREAS.SetRegistrarPickingM(v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, p_IdTarea,p_IdPersonaManual, 1,r_artbolsin.idpedido,p_idubicacion, p_Fintarea, p_ok, p_error);
     end if;
     if (p_ok=0) then
       Rollback;
       Exit;
     else --actualizo la cantidad facturada del bolsin, que en este caso es todo
       update posapp.detallepedidos dp set dp.qtunidadpedfacturada=dp.qtunidadpedido
       where dp.idpedido =r_artbolsin.idpedido
       and dp.cdarticulo=r_artbolsin.cdarticulo;
     end if;
   ...
   if p_ok=1 then
         ...
         update posapp.llaverfcespec ll set ll.cdestado=
       (
        select case
          when   sum(nvl(dp.qtunidadpedfacturada, 0))=0 then 1
          when  sum(nvl(dp.qtunidadpedido,0))-sum(nvl(dp.qtunidadpedfacturada, 0))=0 then 2
          else 3 end estado
       from llaverfcespec llc, pedidos p, detallepedidos dp
       where llc.icllavero=ll.icllavero
       and llc.iddoctrx=p.iddoctrx
       and p.idpedido=dp.idpedido
       and dp.dsobservacion<>'PR'
       ), ll.cdcaja='SLVM' 
         where   ll.idtarea=p_idtarea
         and ll.icllavero=p_idBolsin
         and ll.cdestado=1;
