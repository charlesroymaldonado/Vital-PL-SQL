...
C_FinalizadoRemito                                 CONSTANT tblslvestado.cdestado%type := 37;
  ...
  *****************************************************************************************************/
  ...
  FUNCTION SetDetalleRemito(p_idRemito         tblslvremito.idremito%type,
   ...
   v_qtpizaspicking                tblslvremitodet.qtpiezaspicking%type:=null;
      ...
      if v_res>0 then
        ...
        if v_cantdetrem>0 then
          select det.idremitodet,
                nvl(det.qtunidadmedidabasepicking,0),
                nvl(det.qtpiezaspicking,0)
           into v_idremitodet,
          ...
          where det.idremito = p_idRemito
            and det.cdarticulo=p_cdArticulo;
        --valido si es pesable, multiplico la cantidad por peso del articulo
        --sumo las piezas   
        if p_cdunidad in ('KG','PZA') then
         v_cant:=nvl(v_qtunidadmedidabasepicking,0)+p_cantidad*p_cantidad_pes;  
          v_pza:=v_qtpizaspicking+p_cantidad;
        else
          v_cant:=nvl(v_qtunidadmedidabasepicking,0)+p_cantidad;  
          v_pza:=0;
        end if;  
         update tblslvremitodet det
             set det.qtunidadmedidabasepicking = v_cant,
                 det.qtpiezaspicking = v_pza,
