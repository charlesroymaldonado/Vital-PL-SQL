...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  * %v 20/02/2020 - ChM  verifica si la cantida piking es igual a la cantidad base en la tarea
  *****************************************************************************************************/
    ...
    if v_icgeneraremito = 0 then --if NO genera remito
     ...
     if p_cantidad = 0 then
       ...
       --verifica si aun quedan detalle de tareas por piking
         if VerificaTareaPicking(p_IdPersona,p_IdTarea) = 1 then
            p_Ok:=0;
            p_error:='Existen Articulos por Picking Para esta Tarea!';
            return;
          else
           ...
           null; -- OOOOOOJOOOOOOOOOO aca el llamado a actualizar objetos de fin de piking       
         end if;     
     end if;
     ...
     --verifica si no existe lineas pendientes por picking
