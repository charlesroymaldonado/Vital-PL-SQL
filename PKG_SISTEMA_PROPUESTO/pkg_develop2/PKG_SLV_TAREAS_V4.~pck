...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  *****************************************************************************************************/
  FUNCTION SetInsertarRemito(p_idconsolidadocomi     tblslvremito.idconsolidadocomi%type,
                             p_idconsolidadopedido   tblslvremito.idconsolidadopedido%type,
                             p_idpedfaltanterel      tblslvremito.idpedfaltanterel%type,
  ...
  BEGIN
   insert into  tblslvremito 
        values (seq_remito.nextval,    --idremito
                p_idconsolidadocomi,   --idconsolidadocomi 
                p_idconsolidadopedido, --idconsolidadopedido
  ...
  *****************************************************************************************************/
  FUNCTION SetEstadoTarea(p_IdTarea          tblslvtarea.idtarea%type,
   ...
   v_estado               tblslvtarea.cdestado%type; 
  ...
  *****************************************************************************************************/
  ...
  BEGIN
     ...
     --revisa si es null para atualizar dtinicio en tblslvtarea
     if(v_cantidad_pick is null) then
        v_cantidad_pick:=0;
         update tblslvtarea ta
            set ta.dtinicio = sysdate,
                ta.dtupdate = sysdate
          where ta.idtarea = p_IdTarea;
       if SQL%ROWCOUNT = 0  then 
          n_pkg_vitalpos_log_general.write(2,
                  'Modulo: ' || v_modulo ||
                  ' imposible actualizar dtinicio en tarea: '||p_IdTarea);
          return -1; -- devuelve -1 si no actualiza
       end if;
     ...
     --Valida que lo ingresado sea menor a lo almacenado en detalle tarea
  ...
  END SetDetalleTarea;
    ...
    /****************************************************************************************************
  * %v 20/02/2020 - ChM  Versión inicial SetFindetalleTarea
  * %v 20/02/2020 - ChM  actualiza el icfinalizado del detalle tarea
  *****************************************************************************************************/
  ...
  *****************************************************************************************************/
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona      IN   personas.idpersona%type,
   ...
   BEGIN
     ...
     -- validar si la tarea genera remito y devuelve los datos de crear remito si corresponde
     ...
     if p_idremito = 0 and p_NroCarreta <> 0 then
        ...
        if v_idremito <> 0 then
         ...
         if VerificaTareaPicking(p_IdPersona,p_IdTarea) = 1 then
          --crear remito 
           if SetInsertarRemito(v_idconsolidadocomi,v_idconsolidadopedido,null,p_NroCarreta)=0 then
              p_Ok:=0;
          ...
          if VerificaTareaPicking(p_IdPersona,p_IdTarea) = -1 then
             p_Ok:=0;
             p_error:='Falla VerificaTareaPicking.Comuniquese con Sistemas!';
             return;
          end if;   
         end if;  
        end if;
    ...
    if v_icgeneraremito = 0 then --if NO genera remito
     ...
     if v_cdunidad = '-' then
        ...
        -- actualiza la cantidad picking
          v_res:=SetDetalleTarea(p_IdTarea,p_cdArticulo,p_cantidad,v_cdunidad); 
