create or replace package PKG_SLV_TAREAS is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetArtBarraxTarea(p_IdTarea          IN   tblslvtarea.idtarea%type,
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona        IN   personas.idpersona%type default null,
                                p_idRemito         IN   tblslvremito.idremito%type,               
                                p_NroCarreta       IN   tblslvremito.nrocarreta%type,                  
                                p_cdBarras         IN   barras.cdeancode%type,
                                p_cantidad         IN   tblslvtareadet.qtunidadmedidabase%type,
...
C_FinalizadoTareaPausa                             CONSTANT tblslvestado.cdestado%type := 39;    
      ...
      OPEN p_Cursor FOR 
             Select distinct p.idconsolidadopedido, 
              ...
              where p.idconsolidadopedido= det.idconsolidadopedido
                and p.cdestado in (C_CreadoConsolidadoPedido,C_EnCursoConsolidadoPedido) --estado consolidado pedidos no finalizado   
      ...
      OPEN p_Cursor FOR 
             Select distinct c.idconsolidadocomi, 
              ...
              where c.idconsolidadocomi = det.idconsolidadocomi
                and c.cdestado in (C_CreadoConsolidadoComi,C_EnCursoConsolidadoComi) --estado consolidado comisionistas no finalizado               
      ...
      OPEN p_Cursor FOR 
             Select distinct pf.idpedfaltante, 
              ...
              where pf.idpedfaltante = det.idpedfaltante
                and pf.cdestado in (C_CreadoFaltanConsolidaPedido,C_EnCursoFaltanConsolidaPedido) --estado faltante de pedidos no finalizado          
      ...
      OPEN p_Cursor FOR 
             Select cm.idconsolidadocomi, 
              ...
              where cm.idconsolidadocomi = cdet.idconsolidadocomi
                ...
                --valida si es tarea faltante que el articulo se pikeo en tarea anterior
                and decode(p_TipoTarea,c_TareaConsolidaPedidoFaltante,cdet.qtunidadmedidabasepicking,1) is not null
                --valida si es tarea faltante listar solo los que tienen diferencia
                and decode(p_TipoTarea,c_TareaConsolidaPedidoFaltante,(cdet.qtunidadmedidabase-cdet.qtunidadmedidabasepicking),1)<>0
                --valida no listar articulos ya asignados al armador
  ...
  *****************************************************************************************************/
  ...
  /****************************************************************************************************
  * %v 11/03/2020 - ChM  Versión inicial GetTareaAsigConsolidadoM
  ...
  *****************************************************************************************************/
  ...
  /****************************************************************************************************
  * %v 8/04/2020 - ChM  Versión inicial GetTareaAsigConsolidadoPedido
  * %v 8/04/2020 - ChM  lista las tareas que conforman un IdConsolidadoPedido
  *****************************************************************************************************/
  PROCEDURE GetTareaAsigConsolidadoPedido(p_idConsolidado  IN  Tblslvconsolidadom.Idconsolidadom%type,
                                          p_tipo            IN  Tblslvtipotarea.cdtipo%type default null,
                                          p_Cursor          OUT CURSOR_TYPE) IS
   
   v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetTareaAsigConsolidadoPedido';
    ...
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);                           
  END GetTareaAsigConsolidadoPedido;
   /****************************************************************************************************
  * %v 8/04/2020 - ChM  Versión inicial  GetTareaAsigPedidoFaltantes
  * %v 8/04/2020 - ChM  lista las tareas que conforman un IdpedFaltante
                                4 tareas de Faltante Consolidado Pedido   
  *****************************************************************************************************/
  PROCEDURE GetTareaAsigPedidoFaltantes(p_idPedFaltante   IN  Tblslvconsolidadom.Idconsolidadom%type,
                                        p_tipo            IN  Tblslvtipotarea.cdtipo%type default null,
                                        p_Cursor          OUT CURSOR_TYPE) IS
    ...
    BEGIN             
      OPEN p_Cursor FOR 
       select ta.idpedfaltante,
        ...
        where ta.idpersonaarmador = p.idpersona
          and ta.cdestado = e.cdestado
          and ta.idpedfaltante = p_idPedFaltante
          and (p_tipo is null or ta.cdtipo = p_tipo)
     order by ta.prioridad;
    EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);                           
  END GetTareaAsigPedidoFaltantes;  
  /****************************************************************************************************
  * %v 8/04/2020 - ChM  Versión inicial GetTareaAsigConsolidadoComi
  * %v 8/04/2020 - ChM  lista las tareas que conforman un IdConsolidadoComi
  *                       p_tipo null todas las tareas
  *                             5 solo las tareas Consolidado Comisionista
  *                             6 tareas de Faltante Consolidado Comisionista
  *****************************************************************************************************/
  PROCEDURE GetTareaAsigConsolidadoComi(p_idConsolidado   IN  Tblslvconsolidadom.Idconsolidadom%type,
                                        p_tipo            IN  Tblslvtipotarea.cdtipo%type default null,
                                        p_Cursor          OUT CURSOR_TYPE) IS
   
   v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetTareaAsigConsolidadoComi';
  
    BEGIN             
      OPEN p_Cursor FOR 
       select ta.idconsolidadocomi,
              ta.idtarea,
              decode(ta.cdmodoingreso,0,'HandHeld','Manual') modoingreso,
              p.dsnombre||' '||p.dsapellido Armador,
              e.dsestado Estado       
    ...
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);                           
  END GetTareaAsigConsolidadoComi;  
 /****************************************************************************************************
  * %v 08/04/2020 - ChM  Versión inicial GetTareaAsigConsolidado
  * %v 08/04/2020 - ChM  lista las tareas que conforman un Consolidado                            
  *****************************************************************************************************/
  PROCEDURE GetTareaAsigConsolidado(p_idConsolidado   IN  Integer,
                                    p_TipoTarea       IN  Tblslvtipotarea.cdtipo%type,
                                    p_Cursor          OUT CURSOR_TYPE) IS
    ...
    BEGIN             
      --TipoTarea 1,2 ConsolidadoM y ConsolidadoMFaltante
     if p_TipoTarea in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante) then
      GetTareaAsigConsolidadoM(p_idConsolidado,p_TipoTarea,p_Cursor); 
     end if;  
     --TipoTarea 3 Consolidado pedido
     if p_TipoTarea = c_TareaConsolidadoPedido then
      GetTareaAsigConsolidadoPedido(p_idConsolidado,p_TipoTarea,p_Cursor); 
     end if; 
     --TipoTarea 4 Faltantes Consolidado pedido
     if p_TipoTarea = c_TareaConsolidaPedidoFaltante then
      GetTareaAsigPedidoFaltantes(p_idConsolidado,p_TipoTarea,p_Cursor);
     end if;     
    --TipoTarea 5,6 ConsolidadoComi y ConsolidadoComiFaltante
    if p_TipoTarea in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
      GetTareaAsigConsolidadoComi(p_idConsolidado,p_TipoTarea,p_Cursor);    
    end if; 
...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  *****************************************************************************************************/
  ...
  END GetCarreta;    
   ...
   /****************************************************************************************************
  * %v 17/02/2020 - ChM  Versión inicial GetArtBarraxTarea
  * %v 17/02/2020 - ChM  devuelve el cdarticulo asociado a un codigo de barras y un idTarea
  *****************************************************************************************************/
 ...
 if tarea.idconsolidadocomi is not null then
  ...
  END SetFinalizarConsolidados;
   ...
   /****************************************************************************************************
  * %v 08/05/2020 - ChM  Versión inicial SetFinTarea
  * %v 08/05/2020 - ChM  Finaliza la tarea por completo (creada para picking manual)
  *****************************************************************************************************/
 ...
 (select ta.idpedfaltante,
  ...
  END SetFinalizarTarea;
   /****************************************************************************************************
  * %v 09/05/2020 - ChM  Versión inicial SetRegistrarPicking
  * %v 09/05/2020 - ChM  para un idpersona de registra el picking manual
  *****************************************************************************************************/
  ...
  *****************************************************************************************************/
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona        IN   personas.idpersona%type default null,
   ...
   -- verifica el idpersona segun el tipo tarea HH o Manual
   if v_cdmodoingreso = c_TareaHandHeld  then
      if p_IdPersona is null then
         p_Ok    := 0;
         p_error:='Error la Tarea: '||p_Idtarea||' está en modo ingreso HH con Armador en NULL'; 
         return;   
      end if;  
   else 
     ...
     -- valida si la tarea pertenece al armador y esta asignada o en curso
      if VerificaPersonaTarea(v_idpersonaarmador,p_IdTarea) = 0  then  
          p_Ok:=0;
     ...
     select tp.icgeneraremito
      ...
      where ta.cdtipo = tp.cdtipo
        and ta.idtarea = p_IdTarea
        and ta.idpersonaarmador = v_idpersonaarmador
        and rownum = 1; 
     ...
     if p_idremito = 0 and p_NroCarreta <> 0 and upper(trim(p_cdBarras)) <> 'F' then
        v_idremito:=0;
        GetCarreta(v_idpersonaarmador,p_IdTarea,v_idRemito,v_NroCarreta);
        if v_idremito <> 0 then
         ...
         if VerificaTareaPicking(v_idpersonaarmador,p_IdTarea) = 1 then
          --crear remito
   ...
   if length (p_cdarticulo)>1 and p_cantidad >= 0  then
     ...
     --verifica si NO existe lineas pendientes por picking. Finaliza piking de la tarea
         if VerificaTareaPicking(v_idpersonaarmador,p_IdTarea) = 0 then
            if v_icgeneraremito = 1 then --if SI genera remito
              ...
              --finaliza Tarea        
            if SetFinalizarTarea(v_idpersonaarmador,p_IdTarea) <> 1 then
                p_Ok:=0;
 ...
 END SetPrioridadTarea;
  ...
  /****************************************************************************************************
  * %v 11/03/2020 - ChM  Versión inicial GetArticulosXTarea
