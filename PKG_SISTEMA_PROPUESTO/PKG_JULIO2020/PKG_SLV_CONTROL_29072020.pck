CREATE OR REPLACE PACKAGE PKG_SLV_CONTROL is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetArticulosControl(p_idcomi         IN  tblslvconsolidadopedido.idconsolidadocomi%type,
   ...
   PROCEDURE VerificaConteo(p_idConteo           IN  tblslvconteo.idconteo%type,                                                      
                            p_Ok                 OUT number,
                            p_error              OUT varchar2);                               
...
****************************************************************************************************/
  ...
  *****************************************************************************************************/
  ...
  BEGIN
      ...
      if p_cdBarras ='F' then
          PKG_SLV_CONTROL.SetControlarRemito(p_idremito,p_Ok,p_error,p_cursor); 
         return;        
      end if;
  ...
  --valido si es el primer control del remito 
   ...
   BEGIN
     --valida si existe el p_idremito en tblslvcontrolremito 
      ...
      if v_estado in (C_Controlado,C_ControladoConError) then
       ...
       --si desea finalizar el renglon (FA) 
      if p_cdBarras = 'FA' then
  ...
  *****************************************************************************************************/
  ...
  BEGIN
   ...
   open p_Cursor for 
    ...
    --actualizo dtfin de tblslvcontrolremito
  ...
  *****************************************************************************************************/
     ...
     --actualiza a CONTROLADO CON ERROR la tblslvcontrolremito
       ...
       update tblslvcontrolremito cr
        ...
        where cr.idcontrolremito=v_idControl;
             ...
             END IF;
   p_Ok:=0;
   p_error := 'Remito '||to_char(p_idremito)||' Controlado con diferencias. Por favor ajustar';
   --finaliza conteo
   if FinalizaConteo(v_idConteo) <> 1 then
      p_Ok    := 0;
      p_error:='Error. Comuniquese con Sistemas';
  ...
  /****************************************************************************************************
  * %v 27/07/2020 - ChM  Versión inicial  VerificaConteo
  * %v 27/07/2020 - ChM  recibe v_idcontrol y verifica el conteo de los articulos con error
  *****************************************************************************************************/
   PROCEDURE VerificaConteo(p_idConteo           IN  tblslvconteo.idconteo%type,                                              
                            p_Ok                 OUT number,
                            p_error              OUT varchar2)
    ...
    v_modulo           varchar2(100) := 'PKG_SLV_CONTROL.VerificaConteo';
  ...
  BEGIN    
    ...
    loop
     ...
     --si verifica OK segundo llamado a SetcontrolarRemito 
      if v_cantidad = 0 then 
         PKG_SLV_CONTROL.SetControlarRemito(v_idremito,p_Ok,p_error,v_cursor); 
         if p_ok = 0 then
           rollback;
           return;
         end if;  
         ...
         return;  
           ...
           /*   select cpd.cdarticulo,              
              ...
              sum (cpd.qtunidadmedidabasepicking),
              sum (cpd.qtpiezaspicking)          
     ...
     where cp.idconsolidadocomi=&v_idcontrol   */      
        ...
        /*    select cpd.cdarticulo,              
              ...
              sum (cpd.qtpiezaspicking),
              (select count(*) from 
                                (select count (nvl(cpd2.qtunidadmedidabasepicking,0)) 
                                            from tblslvconsolidadopedidodet cpd2
                                           where cpd2.cdarticulo = cpd.cdarticulo
                                             and nvl(cpd2.qtunidadmedidabasepicking,0)<>0
                                           group by cpd2.qtunidadmedidabasepicking) A ) repeticiones
     ...
     select * from tblslvconsolidadopedido    cp
     where cp.idconsolidadocomi=&v_idcontrol*/
      --compara los conteos anteriores, cuento si al menos dos conteos anteriores son iguales
     begin
       for valida in
              (select cd.cdarticulo, 
                      count(cd.cdarticulo) aparece,
                      --valida pesable
                      decode(sum(cd.qtpiezaspicking),0,
                      (select count (nvl(cd2.qtunidadmedidabasepicking,0)) 
                          from tblslvconteodet cd2
                         where cd2.cdarticulo = cd.cdarticulo
                           and nvl(cd2.qtunidadmedidabasepicking,0)<>0
                         group by cd2.qtunidadmedidabasepicking),
                       (select count (nvl(cd3.qtpiezaspicking,0)) 
                          from tblslvconteodet cd3
                         where cd3.cdarticulo = cd.cdarticulo
                           and nvl(cd3.qtpiezaspicking,0) <> 0
                         group by cd3.qtpiezaspicking)) repeticion                   
        ...
        loop
         ...
         --FALTA LA CONDICION DE VALIDACION 
         -----------------------------------------------------------------------------------------------------------
         -- AQUI CREAR EL NUEVO CONTEO CON LOS DETALLES DE LOS ARTICULOS DIFERENTES Y DEVOLVER EL NUEVO P_IDCONTEO
         -- PARA QUE EL FRONTEND PUEDA LLAMAR A GETRECONTEO SETRECONTEODET    
             p_Ok    := 0;
             p_error := 'Existen artículos con conteos diferentes';
             RETURN;
             -----------------------------------------------------------------------------------------------------          
      Exception
        when no_data_found then
     ...
     --si hay diferencias pero existen dos conteos iguales se llama controlar remito 
