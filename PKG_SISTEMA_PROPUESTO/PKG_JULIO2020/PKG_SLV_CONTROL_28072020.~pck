...
****************************************************************************************************/
   ...
   BEGIN
     --valida si existe el p_idremito en tblslvcontrolremito 
      ...
      if v_estado in (C_Controlado,C_ControladoConError) then
       ...
       --si desea finalizar (F) llama a VerificaConteo  
      if p_cdBarras ='F' then
        PKG_SLV_CONTROL.VerificaConteo(p_idConteo,p_Ok,p_error);
         if p_Ok = 0 then                      
            return;          
         else
      ...
      if p_idConteo = 0 then              
       ...
       select count(cod.cdarticulo) 
        ...
        where cod.idconteo = p_idConteo                     
          ...
          and cod.cdarticulo = v_cdarticulo;
       exception
         when no_data_found then
           p_Ok    := 0;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
    ...
    update tblslvcontrolremitodet crd               
     ...
     --si desea finalizar (F) actualiza en cero todos los ajustes
    if p_cdBarras ='F' then
    v_error:='Falla UPDATE tblslvcontrolremitodet';  
    update tblslvcontrolremitodet crd               
       set crd.qtajusteunidadmbase = 0,
           crd.qtajustepiezas = 0,
           crd.dtupdate=sysdate
     where crd.idcontrolremito = v_idControl
       and crd.qtajusteunidadmbase is null;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
    ...
    --OJO FALTA PROGRAMAR QUE HACER SI LOS CONTEOS ESTAN DISTINTOS
    -----------------------------------------------------------------------
    ...
    loop
      ...
      IF SQL%ROWCOUNT = 0 THEN
        p_Ok    := 0;
        p_error := 'Error. Comuniquese con sitemas.';
        n_pkg_vitalpos_log_general.write(2, 'Modulo: ' || v_modulo );             
        ROLLBACK;
        RETURN;
      END IF;     
