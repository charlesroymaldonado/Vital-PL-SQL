CREATE OR REPLACE PACKAGE PKG_HABILITACION_APP is
  ...
  procedure ExisteCorreoenContactos(p_correo    IN contactosentidades.dscontactoentidad%TYPE,
                                    p_identidad IN entidades.identidad%type,
...
end PKG_HABILITACION_APP;
/
CREATE OR REPLACE PACKAGE BODY PKG_HABILITACION_APP is

  g_Sucursal    sucursales.cdsucursal%Type := getvlparametro('CDSucursal',
                                                             'General');

  /****************************************************************************************
  * Retorna un cursor con todas las autorizaciones asignadas por cuit
  * %v 15/08/2017 - IAquilano - v1.0
  * %v 17/03/2021 - LM - agrego la columna NombreCuenta
  *****************************************************************************************/
  ...
  Begin
    ...
    open cur_out for
      select ta.vlaplicacion Aplicacion,
         ...
         when ta.icactivo = 1 then
           'Habilitado'
              ...
              tp.icvalidamail,
              tp.icexcluyente,
              nvl(( select c.nombrecuenta from tblcuenta c
                where c.idcuenta=ta.idcuenta
              ),'Cuenta suc: '|| s.dssucursal) NombreCuenta
        from tblentidadaplicacion ta, personas p, sucursales s, tblaplicacionautorizacion tp
  ...
  * %v 15/09/2021 - ChM - Mejoro el menmsaje de error para que comprenda mejor el cliente
  *****************************************************************************************/
  ...
  Function FnExisteCorreo(p_correo  IN contactosentidades.dscontactoentidad%TYPE,
    ...
    --v_Modulo VARCHAR2(100) := 'PKG_HABILITACION_APP_CENTRAL.FnExisteCorreo';
  ...
  v_cont := null;
    ...
    select distinct 
           LISTAGG('('||s.dssucursal||') '||c.nombrecuenta||' '||e.cdcuit||' '||e.dsrazonsocial||'  ['||ce.mail||']   '||' '||ce.vlaplicacion,', ')
           WITHIN GROUP (ORDER BY '('||s.dssucursal||') '||c.nombrecuenta||' '||e.cdcuit||' '||e.dsrazonsocial||'  ['||ce.mail||']   '||' '||ce.vlaplicacion,', ')
      into v_cont
     ...
     and ce.icactivo=1
     and ce.identidad = e.identidad
     and ce.cdsucursal=s.cdsucursal
     and ce.idcuenta=c.idcuenta;
     return v_cont;
  EXCEPTION
  ...
  *****************************************************************************************/
  ...
  Function FnExisteCorreoEnContactos(p_correo  IN contactosentidades.dscontactoentidad%TYPE)
   ...
   -- v_Modulo VARCHAR2(100) := 'PKG_HABILITACION_APP_CENTRAL.FnExisteCorreoEnContactos';
     v_cont   VARCHAR2(1000);
  ...
  v_cont := null;
    ...
    select LISTAGG('('||s.dssucursal||') '||e.cdcuit||' '||e.dsrazonsocial||'  ['||ce.dscontactoentidad||']   '||', ')
           WITHIN GROUP (ORDER BY '('||s.dssucursal||') '||e.cdcuit||' '||e.dsrazonsocial||'  ['||ce.dscontactoentidad||']   '||', ')
      into v_cont
      from contactosentidades ce,
           entidades e,
           sucursales s           
     where trim(ce.dscontactoentidad) = trim(p_correo)
       and ce.cdformadecontacto = 2
       and ce.identidad = e.identidad
       and e.cdmainsucursal=s.cdsucursal;
   return v_cont;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      return v_cont;
  ...
  *****************************************************************************************/
  ...
  Begin
    ...
    select count(*)
      ...
      from contactosentidades ce
     where trim(ce.dscontactoentidad) = trim(p_correo)
       --and ce.identidad = p_identidad
       and ce.cdformadecontacto = 2;

    If nvl(v_cont, 0) > 0 then
      p_result := 1; --ya existe
    else
      p_result := 0; --no existe
    End if;
  ...
  * %v 20/08/2021 - ChM - ajusto validaciones de mail 
  * %v 06/09/2021 - APW - corrijo mensaje
  * %v 15/09/2021 - ChM - Mejoro el mensaje de error para que comprenda mejor el cliente
  *****************************************************************************************/
Procedure HabilitacionUsuario(p_identidadaplicacion IN tblentidadaplicacion.identidadaplicacion%TYPE,
                              p_identidad           IN entidades.identidad%type,
                              p_idcuenta            IN tblcuenta.idcuenta%type,
  ...
  v_aplicacionactiva    integer;
  v_validamailn          integer;
  v_validamailactiva     integer;
...
n_pkg_vitalpos_log_general.write(2, 'Modulo: ' || v_Modulo || '  P_identidadaplicacion: ' ||  p_identidadaplicacion|| ' P_identidad: '||p_identidad||' P_idcuenta: '||p_idcuenta||' p_aplicacion: '||p_aplicacion||' p_idpersona: '||p_idpersona||' p_mail: '||p_mail||' ICACTIVO: '||p_icactivo);
   ...
   --valida si el mail es correcto con la expresión regular
   if (REGEXP_INSTR (p_mail,'^[a-zA-Z0-9._%-]+@[a-zA-Z0-9._%-]+\.[a-zA-Z]{2,4}'))=0 then
     p_error := 'Correo '||p_mail||' con formato incorrecto!!';
        return;
    end if;
  ...
  If p_identidadaplicacion is null then
    ...
    begin
      select 1, tt.icvalidamail
        into v_aplicacionactiva, v_validamailactiva
    ...
    exception
      when no_data_found then
        v_aplicacionactiva := 0;
    end;

   If v_mail is not null then
      p_error := 'El correo ya existe en otra cuenta. '||v_mail;
      return;
    end if; 
...
End HabilitacionUsuario;
 ...
 Procedure UpdCorreoAplicacion(p_mailviejo         IN tblentidadaplicacion.mail%TYPE,
   ...
   v_Modulo     VARCHAR2(100) := 'PKG_HABILITACION_APP.HabilitacionUsuario';
   v_mail       varchar2(1000):=null;
   v_mail2      varchar2(1000):=null;
 ...
 Begin
   ...
   If v_mail2 is not null then
      p_error := 'El correo ya existe en otra cuenta. '||v_mail2;
      return;
    end if;  
       ...
       pkg_cliente.ActualizarContacto(p_SqContactoEntidad,
                                      p_Identidad,
                                      p_CdFormadeContacto,
                                      p_IdPersonaModif,
                                      p_mailnuevo,
  ...
  *****************************************************************************************/
  ...
  Begin
    ...
    open cur_out for
      ...
      SELECT rownum Codigo, t.VLAPLICACION Descripcion, t.icvalidamail, t.icexcluyente
