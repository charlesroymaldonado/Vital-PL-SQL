...
END CargarStock;
   ...
   * %v 13/07/2021 - ChM - Agrego validación de fecha hasta del parámetro cursor cuando tipo precio PA y OF
                           Agrego cálculo de los precios PA
   **********************************************************************************************************/
       ...
       where p.cdarticulo = vs.refid
         ...
         and p.id_canal in (select distinct vs.id_canal
          ...
          and p.cdsucursal in ( select distinct vs.cdsucursal
                        ...
                        where vs.cdsucursal<>'9999'
                          and vs.icactivo = 1)
         and (p.dtvigenciadesde=pc_fecha or (pc_tipo in ('PA','OF') and p.dtvigenciahasta=pc_fecha))
  ...
  for P in
    ...
    -- Carga o actualiza los precios lista de los articulos de vtex que se agregaron en la fecha del parametro
     ...
     -- Carga o actualiza los precios PA de los articulos de vtex que se agregaron en la fecha del parametro
      OPEN c_precio(p_Fecha,'PA');
     FETCH c_precio  BULK COLLECT INTO r_precio;      --cargo el cursor en la tabla en memoria
     CLOSE c_precio;
     FOR i IN 1 .. r_precio.COUNT LOOP
       BEGIN
