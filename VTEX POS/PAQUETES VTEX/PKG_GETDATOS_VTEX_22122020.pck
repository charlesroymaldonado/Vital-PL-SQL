...
create or replace package body PKG_GETDATOS_VTEX is
  ...
  ************************************************************************************************/
  ...
  PROCEDURE GetBrands(Cur_Out Out Cursor_Type) IS 
    ...
    v_modulo varchar2(100) := 'PKG_GetDatos.GetBrands';
  ...
  ************************************************************************************************/
  ...
  PROCEDURE GetCat(Cur_Out Out Cursor_Type) IS 
    ...
    v_modulo varchar2(100) := 'PKG_GetDatos.GetCat';
  ...
  BEGIN
    ...
    OPEN cur_out FOR
      SELECT vp.productid id,
             ...
             vp.uxb
        FROM vtexproduct vp
       WHERE vp.icprocesado = 1; --lista solo productos por procesar
  EXCEPTION
    WHEN OTHERS THEN
      pkg_log_general.write(1,
                            'Modulo: ' || v_Modulo || '  Error: ' ||
    ...
    OPEN cur_out FOR
      SELECT vs.skuid id,             
       ...
       WHERE vp.refid = vs.refid         
         and vs.icprocesado = 1; --lista solo SKU por procesar
  EXCEPTION
    WHEN OTHERS THEN
      pkg_log_general.write(1,
                            'Modulo: ' || v_Modulo || '  Error: ' ||
