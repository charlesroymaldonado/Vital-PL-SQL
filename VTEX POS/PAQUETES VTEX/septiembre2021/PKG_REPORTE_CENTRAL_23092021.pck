...
PROCEDURE GetUsuariosPersonas (p_sucursales IN VARCHAR2,
                                 p_idpersona  in personas.idpersona%type,
                                 p_cur_out    OUT cursor_type);
   ...
   BEGIN
      ...
      SELECT re.cdregion,
         ...
         and   do.dtdocumento BETWEEN TRUNC(p_fechaDesde) AND TRUNC(p_fechaHasta + 1)
          ...
          AND rs.cdsucursal= su.cdsucursal
          AND su.cdregion = re.cdregion
          AND (p_idEntidad is null OR p_identidad = do.identidad)
      ...
      OPEN cur_out FOR
        SELECT su.cdsucursal,
          ...
          WHERE dp.iddoctrx        = pe.iddoctrx
            ...
            AND dp.identidad = nvl(p_identidad, dp.identidad )
   ...
   BEGIN
      OPEN cur_out FOR
         SELECT su.cdsucursal,
                ...
                mm.idmovmateriales,
                 case 
                  when pe.icorigen=4 then 'EC'||IDVTEX (pe.idpedido)
        ...
        with salidaok as
         (select s.iddoctrx, p.dsapellido, p.dsnombre
           ...
           where p.idpersona = s.idpersona
             and s.dtsalida > p_fechadesde
             and s.cdmensajesalida = 1)
        select distinct su.dssucursal Suc,
                        upper(emp.dsapellido || ', ' || emp.dsnombre) empleado,
                        d.dtdocumento,
                        GetDescDocumento(d.iddoctrx) factura,
                        round(d.amdocumento + ac.amdescuentoconiva, 2) montonsindescuento,
                        round(ac.amdescuentoconiva, 2) descuento,
         ...
         where emp.idpersona = ac.idpersona
           ...
           and rs.idreporte = v_idReporte
           and (ac.idpersona = p_idpersona or p_idpersona is null)
         order by 3 desc;
...
End GetEstadoCargaComi;
  ...
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    open p_cur_out for
      select su.dssucursal Suc,
             p.cdlegajo legajo,
             upper(p.dsapellido || ', ' || p.dsnombre) empleado,
             p.dscuil cuil,
             decode(p.icactivo, 1, 'SI', 'NO') personaactiva,
             c.dsloginname usuario,
             decode(c.icestadousuario, 1, 'SI', 'NO') UsuarioActivo,
             trunc(c.dtvencimientopassword) VencePassword
        from personas p,
             cuentasusuarios c,
             tbltmp_sucursales_reporte rs,
             sucursales su
       where p.idpersona = c.idpersona
         and c.cdsucursal = rs.cdsucursal
         and c.cdsucursal = su.cdsucursal
         and rs.idreporte = v_idReporte
         and p.idpersona = p_idpersona or p_idpersona is null
       order by cdlegajo
;     
