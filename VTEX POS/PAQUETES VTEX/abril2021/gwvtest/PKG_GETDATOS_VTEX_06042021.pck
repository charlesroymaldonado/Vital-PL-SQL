...
CREATE OR REPLACE PACKAGE BODY PKG_GETDATOS_VTEX is
  ...
  ************************************************************************************************/
    ...
    OPEN cur_out FOR
     SELECT vpr.cdsucursal,
       ...
       WHERE vs.cdsucursal = vpr.cdsucursal
         ...
         and vp.icprocesado = 1 --solo articulso procesados
         order by vpr.skuid;
  EXCEPTION
    WHEN OTHERS THEN
      pkg_log_general.write(1,
  ...
  /***********************************************************************************************
  * %v 31/03/2021 - ChM - Obtiene cursor con todos los precios en oferta por canal y sucursal
  ...
  ************************************************************************************************/
  ...
  PROCEDURE GetOffer (Cur_Out      Out Cursor_Type) IS
    ...
    v_estado        tblcontrolprocesocentral.estado%type:=null;
    
  BEGIN
   
    begin
    select nvl(pc.estado,-1)
      into v_estado
      from tblcontrolprocesocentral pc
     where pc.dtultimacorrida>=trunc(sysdate)
       and pc.nmgrupo='VTEXOFFER'
       and pc.nmproceso='GenerarListaOfertasVTEX';
    exception
      when others then
        v_estado:=null;   
    end;   
  --reviso si no se realizado el SETOFFER para hoy no paso el cursor
  if v_estado <> 1 then
    pkg_procesoautomatico_central.GrabarControlProceso('VTEXOFFER','GenerarListaOfertasVTEX', -1, NULL); -- inicio
    OPEN cur_out FOR     
      SELECT distinct     
             vpr.skuid,
             vp.name,
       ...
       WHERE vs.cdsucursal = vpr.cdsucursal
         ...
         --OJO COMENTO SOLO PARA DESARROLLO ooooooooooooooooooooooooooooJOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
         -- and vpr.dttoof>=trunc(sysdate)        
        -- and vp.factor is null
         order by vpr.skuid;
   end if;      
  EXCEPTION
    WHEN OTHERS THEN
  ...
  END GetOffer;
  /***********************************************************************************************
  * %v 06/04/2021 - ChM - marca en tblcontrolprocesocentral que se proceso correctamente la 
  *                       carga del CSV de ofertas en masterdata VTEX
  ************************************************************************************************/

  PROCEDURE SetOffer IS

    v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.SetOffer';

  BEGIN
      pkg_procesoautomatico_central.GrabarControlProceso('VTEXOFFER','GenerarListaOfertasVTEX',1, NULL); 
    null;
  EXCEPTION
    WHEN OTHERS THEN
      pkg_log_general.write(1,
