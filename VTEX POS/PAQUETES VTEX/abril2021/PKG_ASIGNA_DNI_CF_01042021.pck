...
create or replace package body PKG_ASIGNA_DNI_CF is
    ...
    v_monto:=0;
     ...
     begin          
          select sum(saldo)
           into v_monto
           from (      
                select nvl(sum(nvl(adr.amdocumento,0)),0) saldo             
                       from tblacumdnireparto adr
                      where adr.iddatoscli = p_iddatoscli
                        and adr.dtdocumento between v_dtDesde and v_dtHasta
                      union
                       --SUMA LO QUE VA INSERTANDO EN PEDIDODNICONFORMADO
                      select nvl(sum(nvl((pdc.ampreciounitario * pdc.qtunidadmedidabase),0)),0) saldo
                        from pedidodniconformado pdc
                       where pdc.iddatoscli = p_iddatoscli
                        --solo detallepedidos no procesados
                        and pdc.icprocesado = 0
                        and pdc.dtinsert between v_dtDesde and v_dtHasta
                 );
  ...
  ************************************************************************************************************/
   FUNCTION BTOXArticuloDNICF ( p_iddatoscli         tblacumdnireparto.iddatoscli%type,                            
    ...
    BEGIN
     ...
     -- suma las facturas en el rango establecido   
      select sum (base), sum(piezas)
        into v_base,
             v_piezas      
        from (
              select nvl(sum(nvl(dp.qtunidadmedidabase,0)),0) base,
                     nvl(sum(nvl(dp.qtpiezas,0)),0) piezas         
               ...
               union
               --SUMA LO QUE VA INSERTANDO EN PEDIDODNICONFORMADO
               select nvl(sum(nvl(pdc.qtunidadmedidabase,0)),0) base,
                      nvl(sum(nvl(pdc.qtpiezas,0)),0) piezas
                 from pedidodniconformado pdc
                where pdc.iddatoscli = p_iddatoscli
                   --solo detallepedidos no procesados
                  and pdc.icprocesado = 0
                  and pdc.cdarticulo = p_cdarticulo
                  and pdc.dtinsert between v_dtDesde and v_dtHasta
            );
  ...
  /************************************************************************************************************
  * inserta en la tabla pedidodniconformado los detalle pedido asignados a un DNI del Cliente
  * %v 22/03/2021 ChM: v1.0
  ************************************************************************************************************/
   FUNCTION Insertpedidodniconformado ( p_idpedido              pedidodniconformado.idpedido%type,
                                        p_iddatoscli            pedidodniconformado.iddatoscli%type,
                                        p_cdarticulo            pedidodniconformado.cdarticulo%type,
                                        p_qtunidadmedidabase    pedidodniconformado.qtunidadmedidabase%type,
                                        p_qtpiezas              pedidodniconformado.qtpiezas%type,
                                        p_ampreciounitario      pedidodniconformado.ampreciounitario%type,
                                        p_vluxb                 pedidodniconformado.vluxb%type,
                                        p_icrespromo            pedidodniconformado.icrespromo%type,
                                        p_cdpromo               pedidodniconformado.cdpromo%type,
                                        p_dsobservacion_pedido  pedidodniconformado.dsobservacion_pedido%type
                                        ) 
                         RETURN tblacumdnireparto.amdocumento%type is
                         
      v_Modulo    VARCHAR2(100) := 'PKG_ASIGNA_DNI_CF.Insertpedidodniconformado';
     
  BEGIN
   insert into pedidodniconformado pc
          ( pc.idpedidodniconformado,
            pc.idpedido,
            pc.iddatoscli,
            pc.cdarticulo,
  ...
  ************************************************************************************************************/
  FUNCTION  Dividir_Pedido_DNICF ( p_cdcuit          entidades.cdcuit%TYPE,
    ...
    v_identidadreal                documentos.identidadreal%type;   
    v_cantBTO                      pedidodniconformado.qtunidadmedidabase%type;
    
    --Cursor con el detalle de todos los artículos a distribuir en los DNI de CF
     CURSOR DET IS               
          SELECT dp.cdarticulo,
            ...
            FROM pedidos                 pe,                 
                 tx_pedidos_insert       tx,
                 detallepedidos          dp                 
           WHERE TRUNC(pe.dtaplicacion) = p_dtPedido    --11/12/2020
             AND pe.cdtipodireccion = p_cdTipoDireccion -- para prueba 1
             AND pe.sqdireccion = p_sqDireccion         -- para prueba 4
             AND tx.cdcuit = p_cdcuit                   --20-94031884-0  
             AND pe.idpedido = tx.idpedido
   ...
   --creo una variable tipo cursor DET
   TYPE lv_DET is table of DET%rowtype;
   r_DET     lv_DET;      
   
    --select * from pedidodniconformado;
    --este procedimiento es independiente del Validar pedido que lo llama.
    PRAGMA AUTONOMOUS_TRANSACTION;
    
  BEGIN
    
    --levanto el cursor de los artículos disponibles para distribuir en DNI de CF
     OPEN DET;
     FETCH DET BULK COLLECT INTO r_DET;      --cargo el cursor en la tabla en memoria
     CLOSE DET;     
    ...
    -- recorro cada artículo para asignarlo a los DNI
    FOR i IN 1 .. r_DET.COUNT LOOP     
        FOR dni IN
        ...
        LOOP
         ...
         v_BTO:=BTOXArticuloDNICF (dni.iddatoscli,r_DET(i).cdarticulo);
         
         --verifica si la cantidad de BTO supera el maximo para CF cambio el DNI
         if v_BTO >= g_MaxBTODni then
            continue;  
         end if;
         
         if v_BTO=-1 then
             --llama asignar la cantidad total del articulo
             null;
         else
             --calcula cantidad de BTO necesaria valido pesables
             if r_DET(i).qtpiezas <> 0 then
               v_cantBTO:=r_DET(i).qtpiezas;
             else  
               v_cantBTO:=(r_DET(i).qtbase/r_DET(i).uxb);
             end if;  
             -- verifica si no tiene restricción  BTO =-1 se inserta la cantidad total
             --verifica si la cantidad de BTO que se asignará no supera lo disponible para el DNI
             if v_cantBTO<=v_BTO then
               --llama asignar la cantidad total del articulo
               null;
             else
               --si lo supera envia la cantidad disponible del DNI x uxb
               null;  
             end if;
         end if; --BTO=-1       
          /*--verifica si el DNI tiene Saldo para el artículo
          if (distrib(v_i).preciounitario*distrib(v_i).qtbase)+v_saldo<=g_MaxMontoDni then  
                  
                 --verifica si es un articulo excluido del control de BTO
                 if v_BTO =-1 then 
                   --Insertpedidodniconformado()
                   null;
                 end if;
                
               --verifica si el DNI y artículo tiene BTO disponibles
               if (distrib(v_i).qtbase/distrib(v_i).uxb)+v_BTO <= g_MaxBTODni then  
                 --Insertpedidodniconformado()
                null;
               else
                 --no tiene bultos sale del ciclo
                 null;
