...
END ControlarVTEXpromoPOS;
 ...
 /************************************************************************************************************
  * Informar sobre las promociones con error de: no carga en VTEX por sucursal que recibe (FALLA la API)
  *  solo para control matutino 
  * %v 14/04/2021 ChM: v1.0
  ************************************************************************************************************/
   ...
   when no_data_found then
    ...
    p_result:=0;
      select count(*) 
        into p_result               
        from vtexpromotion vp 
              --solo promos con error de carga en VTEX
        where vp.icprocesado > 1
              --solo promos vigentes
          and v_fecha between vp.begindateutc and vp.enddateutc
...
/************************************************************************************************************
  * Informar sobre las promociones en estado cero 0 no cargadas en VTEX por sucursal que recibe 
  * error el servicio no las sube. para control constante monitor
  * %v 14/04/2021 ChM: v1.0
  ************************************************************************************************************/
PROCEDURE ControlarVTEXpromoCERO ( p_cdsucursal IN sucursales.cdsucursal%type,
                                   p_result     OUT integer 
                                 )
IS
   v_modulo               varchar2(100) := 'PKG_CONTROL.ControlarVTEXpromoCero';
   v_fecha                date := trunc(sysdate);   
    v_cdsucursal          sucursales.cdsucursal%type;
