...
BEGIN
   ...
   OPEN c_stock;
     ...
     FOR i IN 1 .. r_stock.COUNT LOOP
       ...
       -- if v_qtstock <> r_stock(i).qtstock then
           update vtexstock vs
...
FUNCTION GetAgente ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.IDAGENT%type IS
         ...
         WHEN NO_DATA_FOUND THEN
           ...
           BEGIN
            ...
            if v_id3 is not null then
              RETURN '-6';  --agente registrado en comi y telemarketing
            else
              RETURN v_id;
              end if;
            EXCEPTION
              WHEN TOO_MANY_ROWS THEN
                  RETURN '-5';  --agente duplicado en telemarketing
              WHEN OTHERS THEN
                  RETURN v_id;
           END;
        WHEN OTHERS THEN
 ...
 EXCEPTION
   WHEN TOO_MANY_ROWS THEN
       RETURN '-1';  --agente duplicado en comi
   WHEN NO_DATA_FOUND THEN
        BEGIN
            select cv.idviajante
              into v_id
             ...
             BEGIN
              select ct.idpersona
               ...
               --verifica si la identidad se encuentra en las 2 tablas
              if v_id3 is not null then
                RETURN '-7';  --agente registrado en vendedor y telemarketing
              else
                RETURN v_id;
                end if;
              EXCEPTION
        ...
        EXCEPTION
           WHEN TOO_MANY_ROWS THEN
             RETURN '-8';  --agente duplicado en vendedor
           WHEN NO_DATA_FOUND THEN
             --buscar el agente telemarketing
             BEGIN
              select ct.idpersona
  ...
  CURSOR c_ADDRESS IS
    select distinct
     ...
     where e.identidad = de.identidad
       ...
       and vc.icactive = 1
       --para prueba
      -- and ta.dtupdate>=trunc(sysdate-2)
       ;
