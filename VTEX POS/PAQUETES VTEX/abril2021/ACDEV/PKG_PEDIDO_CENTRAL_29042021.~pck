...
CREATE OR REPLACE PACKAGE BODY PKG_PEDIDO_CENTRAL IS
    ...
    OPEN cur_out FOR
      SELECT pi.cdcuit ||' '|| decode(pe.identidad,do.identidadreal,' ','(CF)')  cdcuit,
       ...
       WHERE pe.iddoctrx = do.iddoctrx
         ...
         AND pe.icestadosistema in (11, 12, 13, 14, 17,19) --Trabado por credito 19 CF reparto ChM
  ...
  * %v 21/04/2021 ChM - valida todos los canales TE, VE y CO de CF
  ************************************************************************************************************/
  ...
  BEGIN
    ...
    FOR v_RegPed IN (SELECT pe.identidad,
                      ...
                      WHERE pi.idpedido = pe.idpedido
                        ...
                        AND pe.icestadosistema not in(1,-1) -- si ya están marcados para liberar o es de dividir -1 no los revisa 
     ...
     -- 28/07/16 RLC, Busca ID de Suc y Suc<>Armado
      ...
      --CHM ELIMINO LOS CANALES VE Y CO 30/03/2021
     -- ChM filtra canal VE y CO solo de origen 4 VTEX 01/04/2021
      -- 21/04/2021 ChM - valida todos los canales TE, VE y CO de CF
           IF /*(TRIM(v_RegPed.Id_Canal)='TE' or (TRIM(v_RegPed.Id_Canal)in ('VE','CO') and nvl(v_RegPed.Icorigen,-1)=4)) and */trim(v_RegPed.Identidad) = 'IdCfReparto'  THEN
  ...
  ************************************************************************************************************/
  ...
  BEGIN
    ...
    FOR v_RegPed IN c_RegPed LOOP
      ...
      if trim(v_RegPed.Identidad) = 'IdCfReparto' and trim(v_RegPed.Dsreferencia) not like '%{%'  then 
           pkg_control.GrabarMensaje(sys_guid(),
                                    null,
                                    sysdate,                                    
     ...
     --si el estado del pedido es 19 verifico si esta en ACUMDNIREPARTO ChM
     --esto se hace innecesario a partir del 29/04/2021
    /* IF v_RegPed.Icestadosistema = 19 THEN
       v_doctrx:=null;
        BEGIN
          SELECT acu.iddoctrx
     ...
     /*   if trim(v_RegPed.Identidad) = 'IdCfReparto' and TRIM(v_RegPed.Id_Canal)<>'TE' then
           --ChM estado 18 para todos los pedidos de VE y CO de icorigen distintos a VTEX
          if TRIM(v_RegPed.Id_Canal) in ('VE','CO') and nvl(v_RegPed.Icorigen,-1)<>4 then
              update pedidos
