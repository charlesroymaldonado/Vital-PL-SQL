...
PROCEDURE ControlarVTEXOffer ( p_cdsucursal IN sucursales.cdsucursal%type,
                               p_tipoerror  IN integer default 0,
                               p_result     OUT integer); 
   ...
   begin
     select count(*)
      ...
      from vtexcollectionsku vcs,
           vtexcollection    vc --OJO QUDO MAL CONTAR CADA UNA ON UN CURSOR POR SEPARADO
     where vc.collectionid=vcs.collectionid;     
     exception
       when no_data_found then
...
BEGIN --FALTA MARCA DE YA REVISADO POR POS COMO EN PROMOCIONES
    p_result:=0;
    select count(*)
      into p_result 
      from vtexaddress  va
...
BEGIN
  p_result:=0;--FALTA MARCA DE YA REVISADO POR POS COMO EN PROMOCIONES
    select count(*)
      into p_result 
      from vtexclients vc
     where case 
