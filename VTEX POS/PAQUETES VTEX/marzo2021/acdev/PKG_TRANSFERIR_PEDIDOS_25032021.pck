...
BEGIN
  ...
  /*o	Texto del 1 + 40  el idCuenta  (cliente)
    ...
    o	Texto del 92 + 30 ammonto pedido
    
    o	Texto del 122 + 100  observaciones del pedido
  */
  
   --obtengo v_cdtipodireccion, v_sqdireccion,
   v_cdtipodireccion:=trim(substr(p_cabecera,82,8));   
   v_sqdireccion:=to_number(trim(substr(p_cabecera,90,2)));
  ...
  --divido por 100 por que vtex envia los dos decimales en los ultimos 2 digitos
   v_ammonto:= to_number(trim(substr(p_cabecera,92,30)))/100; 
   --si no viene el cdtipodireccion buscar por el idaddressvtex
   if v_cdtipodireccion is null then          
         begin
            select va.cdtipodireccion,
                   va.sqdireccion,
                   0 retira
              into v_cdtipodireccion,
                   ...
                   v_icretiraensucu
              from vtexaddress va
             where va.id_cuenta = trim(substr(p_cabecera,1,40))
               and va.iddireccion_vtex =trim(substr(p_cabecera,42,40));    
          exception
              when no_data_found then
                   --indica si el pedido se retira en sucursal no trae idaddress
                   if length(trim(substr(p_cabecera,42,40)))>1 then
                      v_icretiraensucu := 1;     
                   else
                      Setvtexorders (p_pedidoid_vtex,null,2,'No es posible recuperar datos de dirección del cliente: '||p_cabecera);         
                     RETURN; 
                  end if;      
               when others then
                     Setvtexorders (p_pedidoid_vtex,null,2,'No es posible recuperar datos de dirección del cliente: '||p_cabecera);         
