CREATE OR REPLACE PACKAGE PKG_CLD_DATOS is
 ...
 -- Procedure RefrescarClientesApp;
  ...
  FUNCTION GetAgente ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.IDAGENT%type;

  FUNCTION Getcanal ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.ID_CANAL%type;

   PROCEDURE RefrescarClients;

   PROCEDURE BajaAddress;
...
BEGIN
    ...
    --procesar cada una de las filas
        ...
        g_l_product(i).variedad := r_product.variedadname;
  ...
  for colle in
   ...
   loop
        ...
        if colle.id_tipo = 'OF' then
           ...
           insert into VTEXCOLLECTIONSKU
                      ...
                      select distinct
                       ...
                       where pre.cdarticulo = vsk.refid
                         ...
                         and case
                             when colle.cdsucursal='9999    ' AND RevisaOfertaPromo(pre.cdarticulo,P_FECHA,'OF') = 1 then 1
                         ...
                         and pre.cdarticulo not in (select distinct art.cdarticulo
                                                      from tblarticulo_tapa art
                      ...
                      select distinct
                       ...
                       where vp.id_promo_pos = vps.id_promo_pos
                         ...
                         and vp.icprocesado = 1
                          --todas las sucursales si es para la 9999
                         and case
                             when colle.cdsucursal='9999    ' then 1
                             when colle.cdsucursal=vp.cdsucursal AND RevisaOfertaPromo(vps.refid,P_FECHA,'PR') = 1 then 1
                               else 0
                             end = 1
                         and case
          ...
          --inserto los SKUs de las colecciones de oferta,tapa y promociones por sucursal y canal
        if colle.id_tipo = 'LI' then
          --insert de las ofertas
           insert into VTEXCOLLECTIONSKU
                      (collectionid,skuid, refid)
                      select distinct
...
* %v 08/02/2021 - ChM
***************************************************************************************************/
FUNCTION GetAgente ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.IDAGENT%type IS
...
* %v 08/02/2021 - ChM
***************************************************************************************************/
FUNCTION Getcanal ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.ID_CANAL%type IS
 ...
 cursor c_Clients is
      (select
       distinct
                ...
                null observacion,
                'CL'  AGENT,
                PKG_CLD_DATOS.GetAgente(e.identidad) idagent,
                PKG_CLIENTE_CENTRAL.GetHabilitBebidaAlcoholica(e.identidad) ICALCOHOL,
                PKG_CLD_DATOS.GetCanal(e.identidad) id_canal,
          ...
          where ta.identidad = e.identidad
            ...
            --para prueba 
            and ta.dtupdate>=trunc(sysdate-1)
      );
BEGIN
      OPEN c_Clients;
...
* %v 08/02/2021 - ChM
* Ajsuto logica para inactivar clientes de alta en VTEX que deben quedar inactivos por no estar en POS
* %v 08/03/2021 - ChM
***************************************************************************************************/
      ...
      FOR i IN 1 .. g_l_Clients.COUNT LOOP
          ...
          select *
           ...
           where vc.id_cuenta = g_l_Clients(i).id_cuenta
             --activo debe existir solo un idcuenta 
             and vc.icactive = 1;
           ...
           if(clients.email <> g_l_Clients(i).email and clients.clientsid_vtex<>'1') then             
              ...
              update vtexclients vc
                 ...
                 where vc.id_cuenta = clients.id_cuenta
                   and vc.clientsid_vtex = clients.clientsid_vtex;  
              ...
              select *
               ...
               where vc.email = g_l_Clients(i).email
                 --cualquier cuenta que use el correo que esta inactiva solo una
                 and vc.icactive = 0
                 and rownum = 1;
                   ...
                   --actualizó INACTIVAS LAS vtexaddress anteriores para consistencia de datos
                     update vtexaddress va
                        set va.icactive = 0,
                            ...
                            va.dtprocesado=null,
                            va.observacion=null
                      where va.id_cuenta = clients.id_cuenta
                        and va.clientsid_vtex = clients.clientsid_vtex;   
               end if;
                EXCEPTION
              ...
              WHEN NO_DATA_FOUND THEN
               ...
               if(clients.razonsocial <> g_l_Clients(i).razonsocial or clients.cdsucursal <> g_l_Clients(i).cdsucursal
                  ...
                  or clients.email <> g_l_Clients(i).email or clients.icalcohol <> g_l_Clients(i).icalcohol
                  or clients.id_canal <> g_l_Clients(i).id_canal or clients.dsnombrefantasia <> g_l_Clients(i).dsnombrefantasia) then
                    update vtexclients vc
        ...
        BEGIN
          select *
           ...
           if(address.cdpostal <> r_address(i).cdcodigopostal or address.dscalle <> r_address(i).dscalle or
              ...
              address.dsprovincia <> r_address(i).dsprovincia) then
                update vtexaddress va
