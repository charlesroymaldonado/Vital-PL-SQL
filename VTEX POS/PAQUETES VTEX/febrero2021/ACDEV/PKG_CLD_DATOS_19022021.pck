CREATE OR REPLACE PACKAGE PKG_CLD_DATOS is
  ...
  FUNCTION Getcanal ( p_identidad    in entidades.identidad%type) RETURN VTEXCLIENTS.ID_CANAL%type;
   ...
   FUNCTION GETvariedad  RETURN INTEGER;                                 
             ...
             --inactiva el existente
              ...
              --inserta el nuevo registro por el cambio de email en VTEX  
               ...
               if(clients.razonsocial <> g_l_Clients(i).razonsocial or clients.cdsucursal <> g_l_Clients(i).cdsucursal
                  ...
                  or clients.id_canal <> g_l_Clients(i).id_canal) then              
                    update vtexclients vc
...
/**************************************************************************************************
* construye la variedad en la vtexcatalog desde vtexarticuloscategorizados
* %v 19/02/2021 - ChM
***************************************************************************************************/
  FUNCTION GETvariedad  RETURN INTEGER IS
    
    BEGIN
      for cat in (select distinct 
                     ca.departmentid,
                     ca.categoryid,
                     ca.subcategoryid,    
                     ca.variedadid,
                     ca.variedadname 
                from vtexarticuloscategorizados ca,
                     vtexcatalog                vc 
               where ca.departmentid=vc.departmentid 
                 and ca.categoryid=vc.categoryid 
                 and ca.subcategoryid=vc.subcategoryid)
     loop
      update vtexcatalog vc
         set vc.variedadid = cat.variedadid,
             vc.vareidadname = cat.variedadname
       where vc.departmentid =cat.departmentid
         and vc.categoryid = cat.categoryid
         and vc.subcategoryid = cat.subcategoryid;                
     end loop;
   commit;
   return 1   ;       
  EXCEPTION
