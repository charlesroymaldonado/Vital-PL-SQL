...
BEGIN
  /*o	Texto del 1-3 el affiliateId
    o	Texto el  4  el salesChannel
    o	Texto del 5 al 45  el id  (cliente)
    o	Texto del 46 al 106  el email (cliente)Texto del 
    o	Texto 107 marca de Consumidor Final.
    o	Texto del 108 al 138 value Monto total pedido.
    o	 Texto del 139 al 179 id vendedor o comi.
    o	Texto del 180 icretiraensucursal.
    o	Texto del 181 al 189  cdtipodireccion.
    o	Texto del 190 al 193 sqdireccion.
    o	Texto del 194 al 294 observación.
  */
  ...
  --divido por 100 por que vtex envia los dos decimales en los ultimos 2 digitos
   ...
   begin
    ...
    v_icretiraensucu := trim(substr(p_cabecera,180,1));     
    --indica cdtipodireccion
    v_cdtipodireccion := trim(substr(p_cabecera,181,8)); 
    --indica sqdireccion 
    v_sqdireccion:= to_number(trim(substr(p_cabecera,190,3)));  
    
    --observaciones del pedido
    v_observacion:= trim(substr(p_cabecera,194,100));
   ...
   --isertar los items del pedido si el arreglo trae datos
    IF (p_detalle(1) IS NOT NULL and LENGTH(TRIM(p_detalle(1)))>1) THEN
       FOR i IN 1 .. p_detalle.Count LOOP
          ...
          --o	Texto del 9-29 price  (2 últimos dígitos decimal)
          --o	Texto del 30-36 quantity
          --o	Texto del 37-77 identifier (promo id VTEX) si no existe no se envía.
          --o	Texto del 78 marca de promo (1) si la línea es promo (0) si la línea es producto 

          v_cdarticulo:=trim(substr(p_detalle(i),1,8));          
          v_price:=to_number(trim(substr(p_detalle(i),9,20)))/100;          
          v_quantity:=to_number(substr(p_detalle(i),30,6)); --la cantidad en VTEX viene solo en unidades UN
          v_idpromo_vtex:=trim(substr(p_detalle(i),37,40));
          v_icresppromo:=to_number(substr(p_detalle(i),78,1));
