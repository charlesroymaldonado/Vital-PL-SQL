...
/**************************************************************************************************
* cambia el estado de la tabla vtexpedidos según parametro de entrada del procedimiento
...
PROCEDURE InsertarPedidoPOS (p_pedidoid_vtex       IN  vtexpedidos.pedidoid_vtex%type,
  ...
  v_dspersonas                varchar2(200);
  v_idpersonaresponsable     personas.idpersona%type;
...
BEGIN
  ...
  --recupero identidad del cliente    
  begin
      select e.identidad
        into v_identidad
        from entidades e 
       where e.cdcuit='20-96007739-4'; --trim(substr(p_cabecera,1,3))
   exception 
         when others then
             if(SetVtexPedidos (p_pedidoid_vtex,null,2,'No existe cliente: ')=0) then         
               RETURN;  
             end if;  
   end;  
   --recupera el id de la persona responsable ID Comi o ID vendedor
   begin
              select per.idpersona, per.dsnombre || ' ' || per.dsapellido 
                into v_idpersonas, v_dspersonas
                from personas per 
               where per.dsnombre like '%CHARLES ROY%';  --trim(substr(p_cabecera,1,3))
    exception 
         when others then
             if(SetVtexPedidos (p_pedidoid_vtex,null,2,'No existe idresponsable: ')=0) then         
               RETURN;  
             end if;  
   end;  
   --recupero el id cuenta del cliente
   begin
              select cu.idcuenta
                into v_idcuenta
                from tblcuenta cu
               where cu.idcuenta = trim(substr(p_cabecera,5,45));
    exception 
         when others then
             if(SetVtexPedidos (p_pedidoid_vtex,null,2,'No se encuentra la cuenta del cliente: '||trim(substr(p_cabecera,5,45)))=0) then         
               RETURN;  
             end if;  
   end; 
   
     --Averiguo la Situacion de IVA
    if trim(substr(p_cabecera,107,1)) = 1 then
      v_cdsituacioniva := '2';
    else
      v_cdsituacioniva := '1';
    end if;
  ...
  select count(distinct td.cdarticulo)
    into v_qtmateriales
    from tblcc_detalle td
   where td.idtransaccion_cc = p_idtransaccion_cc;

  \*--Averiguo el monto total del pedido
  select sum(td.amlineaconiva)
    into v_ammonto
    from tblcc_detalle td
   where td.idtransaccion_cc = p_idtransaccion_cc;*\

  --Id del Pedido
  select sys_guid() into v_idpedido from dual;          */ 
   /*
 ...
 --inserto datos en documentos con tipo de comprobante PEDI
  ...
  VALUES
      (sys_guid()         , NULL                  , NULL                             , v_cdsucursal   , v_identidad      , 'PEDI'        ,
       '1'                , r.idpersonaresponsable, OBTENERCONTADORNUMCOMPROB('PEDI'), CONTADORSISTEMA, SYSDATE          , v_amnetodocumento ,
       NULL               , v_amnetodocumento     , 0                                , 0              , NULL             , v_dsreferencia,
       NULL               , NULL                  , NULL                             , r.idcuenta     , r.identidadreal  , NULL          )
  RETURNING iddoctrx INTO v_iddoctrx;

  --Inserto finalmente el registro cabecera en la tabla pedidos (uso el mismo id del de la transaccion de VTEX)
  insert into pedidos
  ...
  values
  (v_idpedido        , r.identidad     , r.idpersonaresponsable, v_dspersona, v_iddoctrx      , v_qtmateriales, null             ,
   null              , v_cdsituacioniva, -1                     , 3          , sysdate         , sysdate       , r.cdtipodireccion,
   r.idvendedor      , r.sqdireccion   , v_amnetodocumento     , null       , r.idcomisionista, r.id_canal    , p_pedidoid_vtex   ,
   r.icretiraensucu  , r.iczonafranca  );

  for r1 in ( Select *
                from tblcc_detalle td
               where td.idtransaccion_cc = p_idtransaccion_cc
                 and td.vlcantidad <> 0 ) loop
     --Inserta el detalle del pedido para una transacciÃ³n dada

     if r1.cdunidadmedida in ('PZA','KG') then
       v_qtpiezas := r1.vlcantidad;
     --  v_qtunidadmedidabase := r1.vlcantidad;
       v_qtunidadmedidabase := r1.vlcantidad * r1.vluxb;
       r1.cdunidadmedida := 'KG'; -- fuerzo KG si es PZA
       r1.vlcantidad := v_qtunidadmedidabase;
       r1.vluxb := 0;
     else
       v_qtpiezas := 0;
       --Calculo unidad medida base
       begin
         v_qtunidadmedidabase := r1.vlcantidad * r1.vluxb;
       exception when others then
         v_qtunidadmedidabase := NULL;
       end;
     end if;

     --qtunidadpedido para el pesable multiplicar por el precio promedio
     --qtunidadmedidabase hay una tabla que es unidadesmedida
     -- es vlcantidad x UxB o vlcantidad x promedio
      ...
      values
      (v_idpedido         , r1.orden        , r1.cdunidadmedida, r1.cdarticulo   , r1.vlcantidad , v_qtunidadmedidabase, v_qtpiezas   ,
       r1.ampreciounitario, r1.amlineasiniva, r1.vluxb         , r1.dsobservacion, r1.icresppromo, lpad(to_char(to_number(r1.cdpromo)),8,'0') , r1.dsarticulo);
  ...
  values
  (v_iddoctrx, v_idpedido        , r.cdsucursal, v_cdcuit);

  -- Inserto en las observaciones el nro orden -- LF
  insert into observacionespedido
  (idpedido, dsobservacion)
  values
  (v_idpedido, r.icorden);

 p_idpedido := v_idpedido;

  -- LM. Acumula si es CF
  if r.icconsumidorfinal=1 then
    AcumularSaldoDni(r.iddatoscli, v_iddoctrx,NULL);
  end if;*/
  
  
  -------------------------------------------PARA EL CASO DE APLICAR EL DIVIDIR PEDIDO-------------------------------------------
   /*-- Segun si es consumidor final o no elijo diferente monto para dividir pedido
     if v_reg_cabecera.icconsumidorfinal = 1 then
       v_limitedividepedido := cc_n_pkg_vitalpos_core.GETVLPARAMETRO('MAX_PEDIDO_CF','General');
     else
       v_limitedividepedido := cc_n_pkg_vitalpos_core.GETVLPARAMETRO('MAX_PEDIDO','General');
     end if;

     --Si esta cumple alguna condiciÃ³n para dividir pedido
     if v_reg_cabecera.icconsumidorfinal = 1 or p_particionapedido = 1 then

        -- Si el monto total del pedido es inferior al monto para dividir no proceso la divisiÃ³n
        if  ((    v_reg_cabecera.icconsumidorfinal = 1
            and v_subtotalSIVA > cc_n_pkg_vitalpos_core.GETVLPARAMETRO('MAX_PEDIDO_CF','General'))
         or (     p_particionapedido = 1
             and v_subtotalSIVA > cc_n_pkg_vitalpos_core.GETVLPARAMETRO('MAX_PEDIDO','General'))) then

             --MPASSIOTTI - 15/05/2017 - Migracion CC
             \* Se modifica llamado a PKG_DIVIDIR_PEDIDO.Dividir por insert en tabla temporal.
              * La particion de pedidos se realizara desde AC *\
     ...
     ------------------------------------ojo marcar en la tabla VTEXpedidos ic Procesado en 1 indicando que el pedido ya viajo a POS
