...
PROCEDURE InsertarPedidoPOS (p_pedidoid_vtex       IN  vtexpedidos.pedidoid_vtex%type,
  ...
  v_idpersonas               personas.idpersona%type:=null; --null para canal VTEX la persona Responsable
  v_dspersonas               varchar2(200):=null;
...
BEGIN
   ...
   begin
            if v_id_canal ='VE' then
              select per.idpersona
                into v_idvendedor
                from personas per 
               where per.idpersona = trim(substr(p_cabecera,139,40));
             end if;  
            if v_id_canal ='CO' then
              select e.identidad
                into v_idcomisionista
                from entidades e
               where e.identidad = trim(substr(p_cabecera,139,40));
             end if;   
    exception 
         when others then
             if(SetVtexPedidos (p_pedidoid_vtex,null,2,'No existe ID vendedor o comisionista: '||trim(substr(p_cabecera,139,40))||' canal: '||v_id_canal)=0) then         
               RETURN;  
             end if;  
   end;  
  ...
  --divido por 100 por que vtex envia los dos decimales en los ultimos 2 digitos
    v_ammonto:= to_number(trim(substr(p_cabecera,108,30)))/100;   
 ...
 --inserto datos en documentos con tipo de comprobante PEDI
  ...
  VALUES
      (sys_guid()         , NULL                  , NULL                             , v_cdsucursal   , v_identidad      , 'PEDI'        ,
       '1'                , v_idpersonaresponsable, OBTENERCONTADORNUMCOMPROB('PEDI'), CONTADORSISTEMA, SYSDATE          , v_ammonto     ,
  ...
  --Inserto finalmente el registro cabecera en la tabla pedidos (uso el mismo transid del de la transaccion de VTEX)
  insert into pedidos
  ...
  values
  (sys_guid()        , v_identidad     , v_idpersonaresponsable, v_dspersona, v_iddoctrx      , v_qtmateriales, null             ,
