...
PROCEDURE promoMultipleUXB (p_Fecha IN tblprecio.dtvigenciadesde%type) IS
 ...
 CURSOR c_promo_cond IS
        --LISTO LAS PRONMOCIONES HIJAS QUE SE CREARAN
        select distinct A.id_promo_pos,A.cdsucursal,A.id_canal,(A.valorcond*A.UXB)/A.uv condnueva
          from (
                select s.*,               
                       p.cdsucursal,
                       p.id_canal,
   ...
   TYPE lv_promo is table of c_promo_cond%rowtype;
        r_promo_cond        lv_promo;
        promo          vtexpromotion%rowtype;

BEGIN

     OPEN c_promo_cond;
     FETCH c_promo_cond  BULK COLLECT INTO r_promo_cond;      --cargo el cursor en la tabla en memoria
     CLOSE c_promo_cond;
      FOR i IN 1 .. r_promo_cond.COUNT LOOP
          NULL;
      END LOOP;
   commit;
EXCEPTION
