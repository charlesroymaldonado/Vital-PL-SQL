  ...
  BEGIN
    ...
    OPEN cur_out FOR
        SELECT A.id_promo_pos,
          ...
          FROM (
         SELECT DISTINCT
                     vp.id_promo_pos||'-'||vp.id_promo_hija id_promo_pos,
                     vp.id_promo_vtex,
                     vp.name,
                     vp.begindateutc,
                     vp.enddateutc,
                     ...
                     ( select distinct LISTAGG(vps.skuid, ', ') WITHIN GROUP (ORDER BY vps.skuid)
                             ...
                             where vps.id_promo_pos=vp.id_promo_pos
                               and vps.id_promo_hija=vp.id_promo_hija) listSku1BuyTogether,
               ...
               WHERE vs.cdsucursal = vp.cdsucursal
                ...
                -- and vp.cdsucursal = p_cdsucursal
                 ...
                 and vp.id_promo_pos||vp.id_promo_hija  in ( select distinct id_promo_pos||vp.id_promo_hija from Vtexpromotionsku vps)
                 -- verifica no incluir promos con más de 100 SKUS
                 and vp.uxb<>-2
