...
BEGIN
  ...
  -- if p_dtmodificacion > v_dtupdate then
         --si no tiene vigencia es PL y el precio cambio, actuliza
         if p_dtvigenciadesde is null and v_pricepl <> p_precio then
             update vtexprice vp
          ...
          if p_dtvigenciadesde is not null then
   ...
   loop
     ...
     select count(*)
      ...
      where p_fecha between p.dtvigenciadesde and p.dtvigenciahasta
        ...
        --solo reviso ofertas y PA
        and p.id_precio_tipo in ('OF','PA');
     if v_cant=0 then
