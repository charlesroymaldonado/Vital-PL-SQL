...
PROCEDURE GetCat(Cur_Out Out Cursor_Type);
 ...
 PROCEDURE GetClients (p_id_canal       IN  vtexclients.id_canal%type,
  ...
  PROCEDURE GetPersatClientes(Cur_Out Out Cursor_Type);
  
  PROCEDURE SetPersatClientes (p_refId            IN arr_refId,                              
                               p_Ok               OUT number,
                               p_error            OUT varchar2);    
                               
  PROCEDURE GetPersatGeolocalizar(Cur_Out Out Cursor_Type);                                              
  
	PROCEDURE SetPersatGeolocalizar (p_refId            IN arr_refId,                              
                                   p_Ok               OUT number,
                                   p_error            OUT varchar2);

   PROCEDURE verificarOFFER;
   
   PROCEDURE GetActualizaImagen (p_id_canal    In  vtexsellers.id_canal%type,
                                p_icimagenes  Out  vtexsellers.icimagenes%type);

   PROCEDURE SetActualizaImagen (p_id_canal     In  vtexsellers.id_canal%type,
                                p_icimagenes   In  vtexsellers.icimagenes%type,
                                p_Ok           OUT number,
                                p_error        OUT varchar2);                                
...
CREATE OR REPLACE PACKAGE BODY PKG_GETDATOS_VTEX is
  ...
  BEGIN
    ...
    OPEN cur_out FOR
      SELECT vp.productid id,
             ...
             vp.releasedate,
             --envio siempre el refid
             vp.refid KeyWords, --falta concatenar los cdbarras pricipales del articulo
  ...
  ************************************************************************************************/
  ...
  BEGIN
    ...
    OPEN cur_out FOR
      SELECT vs.skuid id,
             vp.productid,
              --is active del producto ChM 11052022
             vp.isactive,
             vp.name,
             trim(vs.refid) refid,
             1 PackagedHeight,
  ...
  * %v 31/08/2022 - ChM  - Agrego precio sin IVA
  ************************************************************************************************/
...
Comentarios:	*/
  ...
  * %v 03/08/2022 - ChM - Ajsuto para marcar oportunidad solo los PA menores o igual a PL
  * %v 31/08/2022 - ChM  - Agrego precio sin IVA
  ************************************************************************************************/
    ...
    OPEN cur_out FOR
      SELECT distinct
             ...
             case
               --verifica si tiene oferta vigente
               when vpr.dttoof>=trunc(sysdate) then
                   round(vpr.priceofsiniva-(vpr.priceofsiniva*(vo.valoracc/100)),2)
               --verifica si tiene PA vigente
               when vpr.dttoPA>=trunc(sysdate) then
                   round(vpr.pricePAsiniva-(vpr.pricePAsiniva*(vo.valoracc/100)),2)
                else
                  round(vpr.priceplsiniva-(vpr.priceplsiniva*(vo.valoracc/100)),2)
              end priceofsiniva,
              -----------
...
--             vo.valorcond*vo.uxb cantminima,
             ...
             --extraigo el valor condición de la leyenda para simpre buscar el original de la promo Padre
             case
        ...
        UNION
          SELECT distinct
           ...
           WHERE vs.cdsucursal = vpr.cdsucursal
             ...
             and vp.refid = vk.refid
             and vp.id_canal = vk.id_canal
             -- 03/08/2022 - ChM - Ajusto para marcar oportunidad solo los PA menores o igual a PL
             and vpr.pricepa<=vpr.pricepl 
  ...
  * %v 06/06/2022 - ChM - Agrego orden al cursor para VTEX
  ********************************************************************************************************/
  ...
  * %v 06/09/2022 - ChM - agrego 2 politicacomercial para sin IVA
  ************************************************************************************************/
  ...
  BEGIN
    ...
    OPEN cur_out FOR
        SELECT A.id_promo_pos,
               ...
               -- agrego 2 politicacomercial para sin IVA
               to_char(id_canal_vtex)||',2' idsSalesChannel,
  ...
  ************************************************************************************************/
  ...
  BEGIN
     ...
     IF (p_refId(1) IS NOT NULL and LENGTH(TRIM(p_refId(1)))>1) THEN
       FOR i IN 1 .. p_refId.Count LOOP
          -- Cambio el tamaño del acuerdo de 32 + 4 más
  ...
  ************************************************************************************************/
  ...
  /***********************************************************************************************
  * %v 24/06/2022 - ChM - Obtiene cursor con todos los clientes y direcciones para integrar a Persat
  * %v 23/08/2022 - ChM - Agrego grupo 15 para clientes de baja
  * %v 06/09/2022 - ChM - Invierto descripcion con razon social
  ************************************************************************************************/

  PROCEDURE GetPersatClientes(Cur_Out Out Cursor_Type) IS

    v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.GetPersatClientes';
  
  BEGIN

    OPEN cur_out FOR
        select t.id_cliente_dir,
               t.cuit|| ' ' || trim(t.cdtipodireccion) || trim(t.sqdireccion) razonsocial,
               t.razonsocial || ' ' || trim(t.cdtipodireccion) || trim(t.sqdireccion)  descripcion,
               t.dscalle,
               t.dsnumcalle,
               t.dslocalidad,
               t.dsprovincia,
               ...
               null Tiempo_serv,
               case                  
                 when t.icactive=1 and (t.latitud=0 or t.longitud=0) then 8 --'Relocalizar'
                 when t.icactive=1 and t.icnuevo=1 then 0
                   else 0
               end Type_id,
               decode (t.icactive,1,Trim(t.legajo_vendedor),15) Group_id,               
               null detalle_direccion,
               null telefono,
               t.email               
          from PERSATCLIENTES t, 
               personas pe
         where pe.cdlegajo = t.legajo_vendedor           
           --solo pendientes por procesar
           and t.icprocesado=0;
  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(1,
                            ...
                            SQLERRM);
      return;
  END GetPersatClientes;

  /***********************************************************************************************
  * %v 24/06/2022 - ChM - Actualiza a procesado el listado de clientes que recibe del arr_refid  
  ************************************************************************************************/
  ...
  PROCEDURE SetPersatClientes (p_refId            IN arr_refId,                              
    ...
    v_modulo               varchar2(100) := 'PKG_GetDatos_VTEX.SetPersatClientes';
    v_id_cliente_dir       persatclientes.id_cliente_dir%type;
    v_icprocesado          persatclientes.icprocesado%type;   
    v_observacion          persatclientes.observacion%type;

  BEGIN

     IF (p_refId(1) IS NOT NULL and LENGTH(TRIM(p_refId(1)))>1) THEN
       FOR i IN 1 .. p_refId.Count LOOP
          --separa del arreglo del 1-40 char del refid equivalente al id_cliente_dir
          --el caracter 41 EL estado puede ser : 1 procesado sin error, 2 procesado con error
          --los siguientes del 42 al 4000 observacion del error
          v_id_cliente_dir:=trim(substr(p_refId(i),1,40));
          v_icprocesado:=to_number(substr(p_refId(i),41,1));          
          v_observacion:= substr(p_refId(i),42,3950);
         --actualiza a procesado el cliente en persatclientes
         begin
             update persatclientes pc
                set pc.icprocesado=v_icprocesado,                    
                    pc.observacion = v_observacion,
                    pc.dtprocesado = sysdate
              where pc.id_cliente_dir = v_id_cliente_dir
                ;
             exception
               when others then
                   n_pkg_vitalpos_log_general.write(1,
                                       'Modulo: ' || v_modulo ||
                                       ' Detalle Error: id_cliente_dir ' || v_id_cliente_dir ||
                                       ' icprocesado ' || v_icprocesado);
     ...
     ELSE
      p_Ok:=0;
      p_error:='Error Arreglo Vacio no es posible Actualizar Clientes Persat';
   	  ROLLBACK;
      RETURN;
     END IF;
  COMMIT;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(1,
                            ...
                            SQLERRM);
      p_Ok:=0;
      p_error:='Error no es posible Actualizar Clientes Persat';
   	  ROLLBACK;
      RETURN;
  END SetPersatClientes;
  
  /***********************************************************************************************
  * %v 27/06/2022 - ChM - Obtiene cursor con todos los clientes y direcciones sin dato latitud, longitud
  ************************************************************************************************/

  PROCEDURE GetPersatGeolocalizar(Cur_Out Out Cursor_Type) IS

    v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.GetPersatGeolocalizar';
  ...
  BEGIN
    ...
    OPEN cur_out FOR
        select t.id_cliente_dir,
               ...
               t.email
          from PERSATCLIENTES t
          --solo sin dato para geolocalizar
         where (t.latitud=0 or t.longitud=0)
           --solo ya procesados OK en Persat
           and t.icprocesado=1;
  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(1,
                            ...
                            SQLERRM);
      return;
  END GetPersatGeolocalizar;

  /***********************************************************************************************
  * %v 24/06/2022 - ChM - Actualiza latitud y longitud el listado de clientes que recibe del arr_refid  
  ************************************************************************************************/
  ...
  PROCEDURE SetPersatGeolocalizar (p_refId            IN arr_refId,                              
    ...
    v_modulo               varchar2(100) := 'PKG_GetDatos_VTEX.SetPersatGeolocalizar';
    v_id_cliente_dir       persatclientes.id_cliente_dir%type;
    v_latitud              persatclientes.latitud%type;
    v_longitud             persatclientes.longitud%type;
    v_icprocesado          persatclientes.icprocesado%type;   
    v_observacion          persatclientes.observacion%type;
  ...
  BEGIN
     ...
     IF (p_refId(1) IS NOT NULL and LENGTH(TRIM(p_refId(1)))>1) THEN
       FOR i IN 1 .. p_refId.Count LOOP
          --separa del arreglo del 1-40 char del refid equivalente al id_cliente_dir
          --el caracter 41 EL estado puede ser : 1 procesado sin error, 4 procesado con error latitud,longitud
          --del 41 al 51 latitud
          --del 51 al 61 longitud
          --los siguientes del 42 al 4000 observacion del error
          v_id_cliente_dir:=trim(substr(p_refId(i),1,40));
          v_icprocesado:=to_number(substr(p_refId(i),41,1));  
          v_latitud:=to_number(substr(p_refId(i),42,10)); 
          v_longitud:=to_number(substr(p_refId(i),52,10));         
          v_observacion:= substr(p_refId(i),62,3900);
         --actualiza a procesado el cliente en persatclientes
         begin
             update persatclientes pc
                set pc.latitud=v_latitud,
                    pc.longitud=v_longitud,
                    pc.icprocesado=v_icprocesado,                    
                    pc.observacion = v_observacion,
                    pc.dtprocesado = sysdate
              where pc.id_cliente_dir = v_id_cliente_dir
                ;
     ...
     ELSE
      p_Ok:=0;
      p_error:='Error Arreglo Vacio no es posible Actualizar Geolocalización de Clientes Persat';
   	  ROLLBACK;
      RETURN;
     END IF;
  COMMIT;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(1,
                            ...
                            SQLERRM);
      p_Ok:=0;
      p_error:='Error no es posible Actualizar Geolocalización de Clientes Persat';
   	  ROLLBACK;
      RETURN;
  END SetPersatGeolocalizar;
  
  /******************************************************************************************************* 
  * %v 08/07/2022 - ChM - obtiene indicador para actualizar imagen
  ********************************************************************************************************/

  PROCEDURE GetActualizaImagen (p_id_canal    In  vtexsellers.id_canal%type,
                                p_icimagenes  Out  vtexsellers.icimagenes%type) IS

    v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.GetActualizaImagen';

  BEGIN     
         select vs.icimagenes
           into p_icimagenes
           from vtexsellers vs
          where vs.id_canal = p_id_canal
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      p_icimagenes:=null;
      return;
  END GetActualizaImagen;

  /***********************************************************************************************
  * %v 08/07/2022 - ChM - Actualiza el parametro de imagenes para que el jo controle frecuencia
  ************************************************************************************************/
  ...
  PROCEDURE SetActualizaImagen (p_id_canal     In  vtexsellers.id_canal%type,
    ...
    v_modulo               varchar2(100) := 'PKG_GetDatos_VTEX.SetActualizaImagen';

  BEGIN

     update Vtexsellers vs
        set vs.icimagenes=p_icimagenes,
            vs.dtprocesadoimg=sysdate
      where vs.cdsucursal = '9999    '
        and vs.id_canal=p_id_canal
        ;
  COMMIT;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      p_Ok:=0;
      p_error:='Error no es posible actualizar parametro imagenes';
   	  ROLLBACK;
      RETURN;
  END SetActualizaImagen;
