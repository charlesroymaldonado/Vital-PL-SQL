CREATE OR REPLACE PACKAGE PKG_CLD_DATOS is
  ...
  type t_persatclientes is table of PERSATCLIENTES%ROWTYPE;
  type t_persatclientes_pipe is table of PERSATCLIENTES%ROWTYPE;
 ...
 FUNCTION GETmeasurementunit (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE)
                              RETURN vtexsku.measurementunit%type;

 FUNCTION GETMULTIPLICADOR (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE,
                            P_ID_CANAL   TBLLISTA_PRECIO_CENTRAL.ID_CANAL%TYPE DEFAULT 'VE') RETURN number;

  Function Pipeproducts Return t_product_pipe pipelined;
  Function PipeSKUS Return t_SKU_pipe pipelined;
  FUNCTION PipePersatClientes RETURN t_PersatClientes_pipe  PIPELINED;
  ...
  FUNCTION desactivarpromoVTEX (P_FECHA      TBLARTICULO_TAPA.VIGENCIADESDE%TYPE) RETURN INTEGER;

  PROCEDURE promoMultipleUXB (p_Fecha IN tblprecio.dtvigenciadesde%type);
  ...
  FUNCTION Getcanal ( p_identidad    in entidades.identidad%type,
   ...
   PROCEDURE RefrescarPersatClientes;
...
CREATE OR REPLACE PACKAGE BODY PKG_CLD_DATOS is
 ...
 g_l_persatclientes t_persatclientes := t_persatclientes();
...
--Buscar el IVA del artículo
 ...
 END GETPrecioconIVA;
/**************************************************************************************************
* devuelve la unidad de medida del articulo
* %v 23/02/2022 - ChM
***************************************************************************************************/
  FUNCTION GETmeasurementunit (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE)
                               RETURN vtexsku.measurementunit%type  IS
...
* %v 23/02/2022 - ChM - Agrego logica para pesables
***************************************************************************************************/
  FUNCTION GETMULTIPLICADOR (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE,
                             P_ID_CANAL   TBLLISTA_PRECIO_CENTRAL.ID_CANAL%TYPE DEFAULT 'VE') RETURN number IS
    V_MULTIPLICADOR          number:=1;
    v_measurementunit        vtexsku.measurementunit%type;

    BEGIN

     v_measurementunit:=PKG_CLD_DATOS.GETmeasurementunit(P_CDARTICULO);
     if v_measurementunit in ('KG','PZA') then
       begin
       select ua.vlpesopromedio
...
* %v 01/07/2022 - ChM excluye retornables
* %v 23/08/2022 - ChM independencia de los catalogos e-commerce y negocio
***************************************************************************************************/
  ...
  v_l_product t_product := t_product();
    ...
    CURSOR c_product ( pc_id_canal   vtexproduct.id_canal%type) IS
      ...
      where ar.cdarticulo = da.cdarticulo
        ...
        and trim(ar.cdpluenvase) is null --ChM 01/07/2022 excluye retornables
        and not exists
      (select 1
               from articulosnocomerciales t
              where t.cdarticulo = ar.cdarticulo)      
        and substr(ar.cdarticulo, 1, 1) <> 'A'
        and ar.cdarticulo=ac.cdarticulo
        and ac.cdarticulo = ae.cdarticulo (+)      
   ...
   CargarTablaproduct();
        ...
        insert into vtexproduct
          select tvp.productid,
                 ...
                 tvp.id_canal,
                 tvp.multipleEAN,
                 tvp.icrevisadopos From Table(Pipeproducts) tvp;
  commit;
...
* %v 18/05/2021 - ChM Elimino el factor y agrego multiplicador
* %v 23/07/2021 - ChM Agrego id_canal
* %v 05/08/2022 - ChM Agrego estado 06 en bajas de productos VTEX
* %v 24/08/2022 - ChM elimino articulos_excluidos
***************************************************************************************************/
          ...
          update vtexproduct vp
            ...
            where vp.isactive = 1  -- solo doy de baja los activos
              ...
              and vp.refid in (select ar.cdarticulo
                                 from articulos                    ar
                                where ar.cdestadoplu in ('03','01','06')  --03 articulo desactivado permanentemente 01 no habilitado para la venta
                                  and substr(ar.cdarticulo, 1, 1) <> 'A'
                                  and nvl(ar.cddrugstore,'XX') not in ('EX', 'DE', 'CP')
...
* %v 23/02/2022 - ChM ajusto pesables
* %v 17/03/2022 - ChM agrego validación para pesables con measurementunit <=0  el SKU este inactivo
* %v 17/03/2022 - ChM Agrego decode(unitmultiplier, para evitar cero
*************************************************************************************************************/
PROCEDURE CargarTablaSKU IS
  ...
  CURSOR c_sku IS
      select distinct
             ...
             vp.name skuname,
             case
             when PKG_CLD_DATOS.GETMULTIPLICADOR(vp.refid,vp.id_canal)<=0 then 0
               else
                  --ajusto si el artículo no tiene barras el SKU este inactivo
                  nvl2(n_pkg_vitalpos_materiales.GetCodigoBarras(vp.refid),1,0)
             end isactive,
             vp.releasedate CREATIONDATE,
             decode(PKG_CLD_DATOS.GETMULTIPLICADOR(vp.refid,vp.id_canal),0,1,PKG_CLD_DATOS.GETMULTIPLICADOR(vp.refid,vp.id_canal)) unitmultiplier,
             PKG_CLD_DATOS.GETmeasurementunit(vp.refid) measurementunit,
             null dtupdate,
             0 icprocesado,
             null observacion,
             sysdate dtinsert,
...
END CargarSKU;
/*************************************************************************************************************
* obliga a subir los sku con error para rescatar imagenes cargadas
* %v 02/02/2022 - ChM
* %v 02/08/2022 - ChM Agrego actualizar subida de imagenes de repositorio
*************************************************************************************************************/
PROCEDURE SKUsinImagenes IS
  v_Modulo varchar2(100) := 'PKG_CLD_DATOS.SKUsinImagenes';


BEGIN
    update vtexproduct p
       set p.icprocesado=0
     where p.refid||p.id_canal in ( select s.refid||s.id_canal
                                      from vtexsku s
                                      where s.icprocesado<>1);

     update vtexsku s
        set s.icprocesado=0
      where s.icprocesado<>1;
      
     Update vtexsellers s
        set s.icimagenes = 0
      where s.cdsucursal = '9999'
        and s.icactivo = 1;
   commit;
...
* %v 17/03/2022 - ChM Agrego canal en update and vs.id_canal =r_stock(i).id_canal
* %v 24/08/2022 - ChM elimino articulos_excluidos
***************************************************************************************************/
...
* %v 24/02/2022 - ChM - agrego actulizar solo precios o fechas que cambiaron
* %V 14/03/2022 - ChM - Agrego NVL de la consulta
* %v 18/03/2022 - ChM - Agrego tipo de precio para cosntruir la columna PA
* %v 16/08/2022 - ChM - Agrego Precio sin IVA para VTEX
***************************************************************************************************/
PROCEDURE ins_upd_vtexprice ( p_cdarticulo       IN  articulos.cdarticulo%type,
                              ...
                              p_precioSinIva     IN  vtexprice.pricepl%type,
                              ...
                              p_id_precio_tipo   IN  tblprecio.id_precio_tipo%type )
  IS
    v_modulo                  VARCHAR2(100) := 'PKG_CLD_DATOS.ins_upd_vtexprice';
    v_pricepl                 tblprecio.amprecio%type;
    v_priceof                 vtexprice.priceof%type;
    v_priceplsinIVA           tblprecio.amprecio%type;
    v_priceofsinIVA           vtexprice.priceof%type;
    v_dtfromof                vtexprice.dtfromof%type;
    v_dttoof                  vtexprice.dttoof%type;
    v_pricepa                 vtexprice.pricepa%type;
    v_pricepasinIVA           vtexprice.pricepa%type;
    v_dtfrompa                vtexprice.dtfrompa%type;
    v_dttopa                  vtexprice.dttopa%type;

    v_dtupdate     vtexprice.dtupdate%type:=p_dtmodificacion;

BEGIN
  select NVL(vp.pricepl,0) pricepl,
         NVL(vp.priceplsiniva,0) priceplsiniva,
         NVL(vp.priceof,0) priceof,
         NVL(vp.priceofsiniva,0) priceofsiniva,
         NVL(vp.dtfromof,'01/01/01') dtfromof,
         NVL(vp.dttoof,'01/01/01') dttoof,
         NVL(vp.pricepa,0) pricepa,
         NVL(vp.pricepasiniva,0) pricepasiniva,
         NVL(vp.dtfrompa,'01/01/01') dtfrompa,
         NVL(vp.dttopa,'01/01/01') dttopa,
         vp.dtupdate
    into v_pricepl,
         v_priceplsinIVA,
         v_priceof,
  ...
  -- if p_dtmodificacion > v_dtupdate then
         if p_id_precio_tipo='PL' and (v_pricepl <> p_precio or v_priceplsinIVA <> p_precioSinIva) then
             update vtexprice vp
                set vp.pricepl = p_precio,
                    vp.priceplsiniva = p_precioSinIva,
                    vp.dtupdate = sysdate,
                    vp.icprocesado = 0,
                    vp.dtprocesado = null,
                    vp.observacion =null
          ...
          --si es precio oferta y cambia precio o fechas, actualiza
          if p_id_precio_tipo='OF'  then
            if v_dtfromof<>p_dtvigenciadesde or v_dttoof<>p_dtvigenciahasta or p_precio<>v_priceof or p_precioSinIva<>v_priceofsinIVA  then
                 update vtexprice vp
                    set vp.priceof = p_precio,
                        vp.priceofsiniva = p_precioSinIva,
          ...
          --si es precio acercamiento y cambia precio o fechas, actualiza
          if p_id_precio_tipo='PA'  then
            if v_dtfrompa<>p_dtvigenciadesde or v_dttopa<>p_dtvigenciahasta or p_precio<>v_pricepa or p_precioSinIva<>v_pricepasinIVA then
                 update vtexprice vp
                    set vp.pricepa = p_precio,
                        vp.pricepasiniva = p_precioSinIva,
                        vp.dtfrompa = p_dtvigenciadesde,
                        vp.dttopa = p_dtvigenciahasta,
                        vp.dtupdate = sysdate,
                        vp.icprocesado = 0,
                        vp.dtprocesado = null,
                        vp.observacion = null
  ...
  WHEN NO_DATA_FOUND THEN
   if p_id_precio_tipo='PL' then
      ...
      values (p_cdsucursal,
              ...
              0,
              null);
   else if p_id_precio_tipo='OF' then
              insert into vtexprice vp
                     (vp.cdsucursal,
                      vp.skuid,
                      vp.refid,
              ...
              values (p_cdsucursal,
                      ...
                      0,
                      null);
         else if p_id_precio_tipo='PA' then
              insert into vtexprice vp
                     (vp.cdsucursal,
                      vp.skuid,
                      vp.refid,
              ...
              values (p_cdsucursal,
                      ...
                      p_cdarticulo,
                      p_id_canal,
                      p_precio, --sino existe en VTEXPRICE se carga PL=PA
                      p_precioSinIva,
                      null,
                      null,
                      null,
...
* %v 02/06/2022 - IA: Ya no calculo precio con IVA, lo traigo desde SAP.
* %v 17/08/2022 - ChM Ajusto agregar precios sin Iva
***************************************************************************************************/
FUNCTION aplicaprecioespecial (p_Fecha IN tblprecio.dtvigenciadesde%type) RETURN integer
  IS
    ...
    v_priceOFsinIVA      tblprecio.amprecio%type:=null;
            ...
            v_priceOF:=esp.priceof;
             update vtexprice s
                set s.priceof = v_priceOF,
                    s.priceofsiniva = esp.priceofsiniva,
       ...
       if esp.icestado = 1 then
          ...
          if not(p_fecha>= esp.dtfromof and p_fecha<=esp.dttoof) then
             update vtexpricespecial es
              ...
              begin
              select A.amprecioiva,
                     A.AMPRECIO,
                     A.dtvigenciadesde,
                     A.dtvigenciahasta
                into v_priceOF,v_priceOFsinIVA,v_dtfromof,v_dttoof
                from (select pre.amprecioiva,--me traigo el precio con iva de SAP
                             pre.amprecio,
                             pre.dtvigenciadesde,
                             pre.dtvigenciahasta,
                             pre.dtmodificacion
             ...
             where  rownum = 1;
             --v_priceOF:=GETPrecioconIVA(esp.refid,esp.cdsucursal,v_priceOF); --Ya no lo calculo, lo traigo de SAP IA: 02/06/2022
            exception
              when others then
               v_priceOF:=null;
               v_priceOFsinIVA:=null;               
               v_dtfromof:=null;
               v_dttoof:=null;
            end;
            --actualizo a la ultima oferta de tblprecio
             update vtexprice s
                set s.priceof = v_priceOF,
                    s.priceofsiniva = v_priceOFsinIVA,
...
/**************************************************************************************************
* verifica si el articulo de vtexprice está con oferta vigente en tblprecio o vtexpricespecial
* si es así ok, sino lo da de baja en vtexprice acortando la fecha dttoof al trunc(sysdate-1)
* %v 14/03/2022 - ChM jira POSG - 861
***************************************************************************************************/
FUNCTION verificaOfertasBorradas (p_Fecha IN tblprecio.dtvigenciadesde%type) RETURN integer
  IS
  v_cant   integer:=-1;
BEGIN
  for OFE In
     (select distinct
             ...
             pr.priceof
        from vtexprice pr
             --ofertas vigentes en VTEX
       where p_fecha between pr.dtfromof and pr.dttoof
         --solo para prueba
         -- and pr.refid in ('0133318 ','0177456 ','0177882 ')
          )
   loop
     --si está en VTEXpricespecial la función retorna 0 indica oferta especial vigente, no lo da de baja
     if PKG_CLD_DATOS.verificaprecioespecial('OF',OFE.cdsucursal,OFE.id_canal,OFE.REFID) = 0 then
        continue;
     end if;
     --verifica si la oferta aún esta vigente en tblprecio
     select count(*)
       into v_cant
       from tblprecio p
      where p_fecha between p.dtvigenciadesde and p.dtvigenciahasta
        and p.cdsucursal=ofe.cdsucursal
        and p.id_canal=ofe.id_canal
        and p.cdarticulo=ofe.refid
        --solo reviso ofertas
        and p.id_precio_tipo in ('OF');
     if v_cant=0 then
        update vtexprice pre
           -- si no esta vigente en tblprecio la venzo en VTEX para ayer sysdate-1
           set pre.dttoof=sysdate-1,
               pre.dtupdate=sysdate,
               pre.icprocesado=0,
               pre.dtprocesado=null
         where pre.cdsucursal=ofe.cdsucursal
           and pre.skuid=ofe.skuid
           and pre.refid=ofe.refid
           and pre.id_canal=ofe.id_canal;
     end if;
     end loop;
  RETURN 1;
EXCEPTION
  when others then
    return 0;
END verificaOfertasBorradas;

/**************************************************************************************************
* verifica si el articulo de vtexprice está con PA vigente en tblprecio
* si es así ok, sino lo da de baja en vtexprice acortando la fecha dttoPA al trunc(sysdate-1)
* %v 14/03/2022 - ChM jira POSG - 869
*    02/05/2022 - ChM agrego actualizar los articulos cuyo PA vencidos deben subir PL
***************************************************************************************************/
FUNCTION verificaPABorrados (p_Fecha IN tblprecio.dtvigenciadesde%type) RETURN integer
  IS
  v_cant   integer:=-1;
BEGIN
  for PAC In
     (select distinct
        ...
        from vtexprice pr
             --PA vigentes en VTEX
       where p_fecha between pr.dtfromPA and pr.dttoPA
         --solo para prueba
         -- and pr.refid in ('0133318 ','0177456 ','0177882 ')
          )
   loop
     --verifica si EL PA aún esta vigente en tblprecio
     select count(*)
       into v_cant
       from tblprecio p
      where p_fecha between p.dtvigenciadesde and p.dtvigenciahasta
        and p.cdsucursal=PAC.cdsucursal
        and p.id_canal=PAC.id_canal
        and p.cdarticulo=PAC.refid
        --solo reviso PA
        and p.id_precio_tipo in ('PA');
     if v_cant=0 then
        update vtexprice pre
           -- si no esta vigente en tblprecio la venzo en VTEX para ayer sysdate-1
           set pre.dttoPA=sysdate-1,
               pre.dtupdate=sysdate,
               pre.icprocesado=0,
               pre.dtprocesado=null
         where pre.cdsucursal=PAC.cdsucursal
           and pre.skuid=PAC.skuid
           and pre.refid=PAC.refid
           and pre.id_canal=PAC.id_canal;
     end if;
     end loop;
      --ChM - 02/05/2022 - agrego actualizar los articulos cuyo PA vencidos deben subir PL
    update vtexprice p 
       set p.icprocesado=0,
           p.dtprocesado=null,
           p.dtupdate=trunc(sysdate)
     where p.id_canal in ('VE','CO' )
           --solo PA no vigentes
       and not (trunc(sysdate) between p.dtfrompa  and p.dttopa )
           --solo los procesados antes del vencimiento
       and trunc(p.dtprocesado)<=p.dttopa;
       RETURN 1;
EXCEPTION
  when others then
    return 0;
END verificaPABorrados;
   ...
   * %v 14/03/2022 - ChM - funcion para vencer ofertas en vtexprice que cierran en POS por eliminación de SAP
   * %v 21/03/2022 - ChM - agrego PA como columna en vtexprice y ajsuto borrar PA que eliminan de SAP
   * %v 02/06/2022 - IA: Ya no calculo precio con IVA, lo traigo de SAP. 
   * %v 16/08/2022 - ChM - Agrego Precio sin IVA para VTEX
   **********************************************************************************************************/
  PROCEDURE RefrescarPreciosVTEX (p_Fecha IN tblprecio.dtvigenciadesde%type) IS
      ...
      v_priceOFsinIVA            tblprecio.amprecio%type:=null;
      v_priceplsinIVA            tblprecio.amprecio%type:=null;
      v_dtfromof                 tblprecio.dtvigenciadesde%type:=null;
      v_dttoof                   tblprecio.dtvigenciahasta%type:=null;
      ...
      select p.cdarticulo,
             ...
             p.dtmodificacion,
             p.amprecio,
             p.amprecioiva precioconiva--PKG_CLD_DATOS.GETPrecioconIVA (p.cdarticulo,p.cdsucursal,p.amprecio) precioconiva ya no lo calculo IA 02/06/2022
        from tblprecio p,
             vtexsku   vs
  ...
  for P in
    ...
    loop
        ...
        select A.Amprecio,
               ...
               A.dtvigenciadesde,
               A.dtvigenciahasta
          into v_priceOFsinIVA, v_priceOF,v_dtfromof,v_dttoof
       ...
       --recupero solo la oferta ultima actulizada
       where  rownum = 1;
       --v_priceOF:=GETPrecioconIVA(p.cdarticulo,p.cdsucursal,v_priceOF); --ya no lo calculo porqe lo traigo de la tblprecio IA 02/06/2022
      exception
        when others then
         v_priceOFsinIVA:=null; 
         v_priceOF:=null;
      ...
      begin
        select A.Amprecio,
               A.amprecioiva-- IA 02/06/2022               
          into v_priceplsinIVA, v_pricepl
          from (select pre.amprecio,
                       pre.amprecioiva,-- IA 02/06/2022
                       pre.dtmodificacion
                  from tblprecio pre
                 where pre.cdsucursal = p.cdsucursal
                   and pre.id_canal = p.id_canal
    ...
    -- Carga o actualiza los precios lista de los artículos de vtex que se agregaron en la fecha del parámetro
     ...
     ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,r_precio(i).precioconiva,r_precio(i).amprecio,null,null,r_precio(i).dtmodificacion,'PL');
     END LOOP;
     ...
     FOR i IN 1 .. r_precio.COUNT LOOP
       ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,
                           r_precio(i).precioconiva,r_precio(i).amprecio,r_precio(i).dtvigenciadesde,r_precio(i).dtvigenciahasta,r_precio(i).dtmodificacion,'OF');
     END LOOP;
     ...
     CLOSE c_precio;
     FOR i IN 1 .. r_precio.COUNT LOOP
        ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,r_precio(i).precioconiva,r_precio(i).amprecio,r_precio(i).dtvigenciadesde,r_precio(i).dtvigenciahasta,r_precio(i).dtmodificacion,'PA');
     END LOOP;
     ...
     END LOOP;
      ...
      --funcion para vencer ofertas en vtexprice que cierran en POS por eliminación de SAP
      if verificaOfertasBorradas (p_Fecha) = 0 then
        pkg_control.GrabarMensaje(sys_guid(),null,sysdate,'error en BAJAS de OFERTAS VTEX','',0);
      end if;
     --funcion para vencer PA en vtexprice que cierran en POS por eliminación de SAP
      if verificaPABorrados (p_Fecha) = 0 then
        pkg_control.GrabarMensaje(sys_guid(),null,sysdate,'error en BAJAS de PA VTEX','',0);
      end if;
  commit;
  EXCEPTION
    WHEN OTHERS THEN
...
* %v 20/03/2022 - ChM Agrego and pre.id_canal = vsk.id_canal
* %v 05/04/2022 - ChM Agrego tapa 9999 para todas las tapas de distintas sucursales or trim(art.cdsucursal) = '9999')
* %v 28/04/2022 - ChM Carga los carruseles en VTEX según parametros 
* %v 31/08/2022 - ChM Ajusto elimino el  or con 9999 por nueva dinamica de carga de tapa
* %v 05/09/2022 - ChM cambio DISTRIBU para separar por canal VE y CO
*********************************************************************************************************/
...
BEGIN
  ...
  for colle in
   ...
   loop     
   --inserto los SKUs de los carruseles 
        if colle.id_tipo not in ('DISTRI','OF') then
           insert into VTEXCOLLECTIONSKU
                      (collectionid,skuid, refid,icprocesado,dtprocesado,observacion,orden)
                      select distinct
                             colle.collectionid,
                             vsk.skuid,
                             art.cdarticulo,
                             ...
                             null,
                             null,
                             --se conviene que en la carga desde excel, la descripcion define el orden numerico
                             to_number(regexp_substr(art.descripcion, '\d+')) orden
                       ...
                       where art.cdarticulo = vsk.refid
                         ...
                         and (art.cdsucursal = colle.cdsucursal /*or trim(art.cdsucursal) = '9999'*/)
                         --uso canal definido en TAPA para la carga ordenada de oferta y promoción
                         /* Ofertas y Promociones OPVE
                            Stockeate STVE
                            Carrusel - perfumería PEVE
                            Carrusel - limpieza LPVE
                            Carrusel - almacén ALVE*/
                         and TRIM(art.cdcanal) = colle.id_tipo||colle.id_canal
                         --se conviene que en la carga la descripcion define el orden
                    order by orden
                        ;
         end if;
        --inserto los SKUs de las tapas por sucursal y canal
        if colle.id_tipo = 'DISTRI' then
           insert into VTEXCOLLECTIONSKU
                      ...
                      select distinct
                       ...
                       where art.cdarticulo = vsk.refid
                         ...
                         and p_fecha between art.vigenciadesde and art.vigenciahasta
                         and (art.cdsucursal = colle.cdsucursal /*or trim(art.cdsucursal) = '9999'*/)                         
                         and art.cdcanal = colle.id_tipo||colle.id_canal
                        ;
         end if;
...
/****************************************************************************************************
* inactiva las promociones de VTEX que dejan de existir en POS tbl_promo por inactivación de usuario
* para promos que salen de algun canal.
* NOTA: no puedo inactivar grupos por sucursales que salen esto dañaria la logia de envio vtex en get_datos_vtex
* %v 15/02/2022 - ChM
* %v 20/03/2022 - ChM Agrego and p.isactive = 1
*****************************************************************************************************/
  FUNCTION desactivarpromoVTEX (P_FECHA      TBLARTICULO_TAPA.VIGENCIADESDE%TYPE
                                ) RETURN INTEGER IS

    BEGIN
 update vtexpromotion p
    set p.isactive = 0,
        p.icprocesado = 0,
        p.dtprocesado = null,
        p.dtupdate = sysdate
  where p_fecha between p.begindateutc and p.enddateutc
    and p.isactive = 1
   and p.id_promo_pos||p.id_canal not in
                   (
                   SELECT distinct
                          R.ID_PROMO||CANAL.ID_CANAL
                     FROM TBLPROMO                     R,
...
BEGIN
     ...
     END LOOP;
      --funcion para desactivar promociones que salen de POS
      if desactivarpromoVTEX(p_Fecha) = 0 then
          pkg_control.GrabarMensaje(sys_guid(),null,sysdate,'error en PROMOCIONES inactivas de VTEX','',0);
      end if;
     commit;
EXCEPTION
  WHEN OTHERS THEN
...
* %v 01/07/2021 - ChM
* %v 17/11/2021 - ChM - Agrego Canal por UV diferenciado por Canal
* %v 17/03/2022 - ChM - Ajusto minuv no sea cero por frescos
***************************************************************************************************/
FUNCTION MINSKU (p_id_promo TBLPROMO.ID_PROMO%type,
                 p_id_canal vtexsku.id_canal%type) RETURN integer
  IS
          ...
          from (
                select s.id_promo_pos,
                 ...
                 where p.uxb=-1
                   ...
                   and s.id_canal=p.id_canal
                   --solo prueba
                   --and p.cdpromo=187863
                  -- and p.cdsucursal='0014'
                 )A, vtexpromotion vp
...
* %v 09/02/2021 - ChM
* %v 29/03/2022 - ChM motivado a jira HD-68854 se deja sin direccion a clientes que el RAC no les asocie
*                     se elimina la opcion por defecto de direccion comercial activa
***************************************************************************************************/
...
/**************************************************************************************************
* Carga en una tabla global en memoria los datos de todos los clientes direcciones de VTEX a para PERSAT
* %v 24/06/2022 - ChM
***************************************************************************************************/
PROCEDURE CargarTablaPersatClientes IS

  v_Modulo    varchar2(100) := 'PKG_CLD_DATOS.CargarTablaPersatClientes';
  
    CURSOR c_clientes  IS
           select a.iddireccion_vtex id_cliente_dir,
                   c.cuit,
                   c.razonsocial,
                   c.email,
                   a.cdtipodireccion,
             ...
             where c.id_cuenta = a.id_cuenta
               and c.clientsid_vtex = a.clientsid_vtex                           
               and p.idpersona = c.idagent
                  --solo direcciones activas
               and a.icactive=1
               ...
               and c.icactive=1
                  --solo canal vendedor
               and c.id_canal = 'VE'
                --solo direcciones procesadas
               and a.icprocesado=1
           ;
    BEGIN
      --solo canales activos en el marketplace sucursal 9999
   
      g_l_persatclientes.delete();
      OPEN c_clientes;
     FETCH c_clientes  BULK COLLECT INTO g_l_persatclientes;      --cargo el cursor en la tabla en memoria
     CLOSE c_clientes;
...
EXCEPTION
  WHEN OTHERS THEN
    n_pkg_vitalpos_log_general.write(1,
                          'Modulo: ' || v_Modulo || '  Error: ' || SQLERRM);
END  CargarTablaPersatClientes;

/**************************************************************************************************
* Devuelve todos los datos de la tabla de PersatClientes en memoria
* %v 24/06/2022 - ChM
***************************************************************************************************/
FUNCTION PipePersatClientes RETURN t_PersatClientes_pipe
  PIPELINED IS
  i binary_integer := 0;
BEGIN
  i := g_l_PersatClientes.FIRST;
  while i is not null loop
    pipe row(g_l_PersatClientes(i));
    i := g_l_PersatClientes.NEXT(i);
  end loop;
  return;
EXCEPTION
  when others then
    null;
END PipePersatClientes;
  
  /**************************************************************************************************
* Carga clientes nuevos y actualiza los modificados de la tabla PERSATCLIENTES
* PERSAT TIPO: Cliente Activo, Baja Inactivo
* %v 24/06/2022 - ChM
* %v 12/09/2022 - ChM agrego bajas de direcciones que ya no existen en VTEXADDRESS
***************************************************************************************************/
PROCEDURE RefrescarPersatClientes IS
  v_Modulo              varchar2(100) := 'PKG_CLD_DATOS.RefrescarPersatClientes';

BEGIN

  g_l_persatclientes.delete();
  -- llena la tabla en memoria con los clientes direccines en AC
  CargarTablaPersatClientes();

    merge into persatclientes pc
    using (select *
             From Table(PipePersatClientes)) tpc
    on (pc.id_cliente_dir = tpc.id_cliente_dir )
    when not matched then -- altas
      insert
        (id_cliente_dir,
         cuit,
    ...
    when matched then -- modificaciones
       update
          set pc.cuit=tpc.cuit,
              pc.razonsocial=tpc.razonsocial,
              pc.email=tpc.email,
              pc.cdtipodireccion=tpc.cdtipodireccion,
              pc.sqdireccion=tpc.sqdireccion,
              pc.dscalle=tpc.dscalle,
              pc.dsnumcalle=tpc.dsnumcalle,
              pc.dslocalidad=tpc.dslocalidad,
              pc.dsprovincia=tpc.dsprovincia,
              --pc.pais=tpc.pais,
              --pc.latitud=tpc.latitud,
              --pc.longitud=tpc.longitud,
              pc.legajo_vendedor=tpc.legajo_vendedor,
              pc.icactive=tpc.icactive,              
              pc.dtupdate=sysdate,
              pc.icprocesado=0,
              pc.dtprocesado=null,
              pc.icnuevo=0
         where -- solo se actualizan si hubo algun cambio
              pc.cuit<>tpc.cuit
           or pc.razonsocial<>tpc.razonsocial
           or pc.email<>tpc.email
           or pc.cdtipodireccion<>tpc.cdtipodireccion
           or pc.sqdireccion<>tpc.sqdireccion
           or pc.dscalle<>tpc.dscalle
           or pc.dsnumcalle<>tpc.dsnumcalle
           or pc.dslocalidad<>tpc.dslocalidad
           or pc.dsprovincia<>tpc.dsprovincia
           -- pc.pais<>tpc.pais
            --pc.latitud<>tpc.latitud
            --pc.longitud<>tpc.longitud
           or pc.legajo_vendedor<>tpc.legajo_vendedor
           or pc.icactive<>tpc.icactive
             ;          
         update persatclientes oc
            set oc.icactive = 0,
                oc.icprocesado = 0, -- 0 para procesar en API de VTEX
                oc.dtprocesado = null,
                oc.observacion = null,               
                oc.dtupdate = sysdate,
                oc.icnuevo = 0,
                oc.razonsocial = substr(oc.razonsocial,1,45)||' '||oc.id_cliente_dir,                
                oc.cuit = oc.cuit||' '||substr(oc.id_cliente_dir,1,20)
            where oc.icactive = 1  -- solo doy de baja los activos
              and oc.cuit||oc.cdtipodireccion||oc.sqdireccion
                in (  select distinct c.cuit||a.cdtipodireccion||a.sqdireccion                            
                        from Vtexaddress     a,
                             VTEXCLIENTS     c,
                             carteraclientes ca,
                             personas        p,
                       ...
                       where c.id_cuenta = a.id_cuenta
                         and c.clientsid_vtex = a.clientsid_vtex
                         and e.cdcuit = c.cuit
                         and ca.identidad = e.identidad
                         and ca.idpersona = c.idagent
                         and p.idpersona = ca.idpersona
                            --solo asociacion de clientes activas
                         and ca.icactivo = 1
                            --solo direcciones inactivas
                            and (a.icactive=0
         ...
         --12/09/2022 - ChM agrego bajas de direcciones que ya no existen en VTEXADDRESS          
         update persatclientes oc
            set oc.icactive = 0,
                oc.icprocesado = 0, -- 0 para procesar en API de VTEX
                oc.dtprocesado = null,
                ...
                oc.icnuevo = 0,
                oc.razonsocial = substr(oc.razonsocial,1,45)||' '||oc.id_cliente_dir,
                oc.cuit = oc.cuit||' '||oc.id_cliente_dir
          -- select * from persatclientes oc        
          where oc.icactive = 1  -- solo doy de baja los activos            
              and not exists 
               (  select distinct a.iddireccion_vtex                   
                        from Vtexaddress     a,
                             VTEXCLIENTS     c
                            --solo canal vendedor
                         where c.id_cuenta = a.id_cuenta
                           and c.clientsid_vtex = a.clientsid_vtex
                           and c.id_canal = 'VE'  
                           and a.iddireccion_vtex=oc.id_cliente_dir                                                    
                      )
                     -- and oc.email='cying8545@gmail.com'
                      ;      
  commit;                   
EXCEPTION
  WHEN OTHERS THEN
