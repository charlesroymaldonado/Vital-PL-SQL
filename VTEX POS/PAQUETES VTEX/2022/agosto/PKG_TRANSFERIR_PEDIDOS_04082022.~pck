...
PROCEDURE InsertarPedidoPOS (p_pedidoid_vtex       IN  vtexorders.pedidoid_vtex%type,
  ...
  v_id_canal_pedido          vtexsellers.id_canal%type:=null;
       ...
       if v_id_canal ='VE' then   
         begin        
              select 'VE' 
               ...
               where cv.identidad = v_identidad
                 and cv.idviajante = v_idvendedor
                 and rownum = 1;     
               
              select 'TE' 
                into V_id_canal_pedido
                from CLIENTESTELEMARKETING ct
               where ct.IDENTIDAD = v_identidad     
                 and ct.IDPERSONA = v_idvendedor
                 and rownum = 1;          
           EXCEPTION
                 WHEN OTHERS THEN   
