...
PROCEDURE GetCat(Cur_Out Out Cursor_Type);
  ...
  PROCEDURE GetPromotion ( p_idcanal    In vtexsellers.id_canal%type,
                           Cur_Out      Out Cursor_Type);
  ...
  BEGIN
     OPEN cur_out FOR
          select vst.cdarticulo SKU_refid,
                 (vst.qtstock/vst.unitmultiplier) as qtstock
           from vtexstock   vst,
                vtexproduct vp
...
Comentarios:	*/
  ...
  * %v 10/05/2021 - ChM - Solo sucursales activas
  * %v 18/05/2021 - ChM - cambio parametro de p_cdsucursal por p_idcanal ahora las promo suben por canal
  ************************************************************************************************/
  ...
  BEGIN
    ...
    OPEN cur_out FOR
           SELECT DISTINCT
             ...
             vs.id_canal_vtex idsSalesChannel,
