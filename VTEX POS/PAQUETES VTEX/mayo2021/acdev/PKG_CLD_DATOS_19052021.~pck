...
PROCEDURE CargarStock IS
   ...
   v_unitmultiplier   vtexstock.unitmultiplier%type;
  CURSOR c_stock IS
            with stock as(
          select aa.cdalmacen,
                 aa.cdarticulo,
      ...
      ventas as (
         ...
         select s.cdalmacen,
                ...
                PKG_CLD_DATOS.GETMULTIPLICADOR (s.cdarticulo) unitmultiplier 
          from stock s
   ...
   OPEN c_stock;
     ...
     FOR i IN 1 .. r_stock.COUNT LOOP
       BEGIN
        ...
        if v_qtstock <> r_stock(i).qtstock or v_unitmultiplier<>r_stock(i).unitmultiplier then
           update vtexstock vs
              set vs.qtstock  = r_stock(i).qtstock,
                  vs.unitmultiplier = r_stock(i).unitmultiplier,
                  vs.dtupdate = sysdate,
                  vs.icprocesado = 0,
                  vs.dtprocesado=null,
                  vs.observacion=null
...
* %v 09/03/2021 - ChM
* %v 19/05/2021 - ChM agrego el tipo de colección CU para listado de ofertas por sucursal 
*********************************************************************************************************/
