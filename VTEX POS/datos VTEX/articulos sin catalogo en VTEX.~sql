     SELECT distinct --BB.CDARTICULO,
                  --   BB.NAME,
                     BB.DEPARTMENTID,
                     NVL((select 
                        distinct nvl(vc.departmentname,'') 
                            from vtexcatalog vc
                           where vc.departmentid = BB.DEPARTMENTID),'') department,
                     BB.dsdepartamento,         
                     BB.CATEGORYID,
                     NVL((select 
                        distinct NVL(vc.categoryname,'') 
                            from vtexcatalog vc
                           where vc.categoryid = BB.CATEGORYID),'') categoryID,
                     BB.dsuniverso,      
                     BB.SUBCATEGORYID,                     
                     NVL((select 
                        distinct NVL(vc.subcategoryname,'') 
                            from vtexcatalog vc
                            where  vc.subcategoryid = BB.SUBCATEGORYID),'') subcategoryID,                     
                     BB.dscategoria--, 
                   --  BB.BRANDID,
                    -- VB.NAME 
                 --    BB.UXB
                FROM (select distinct 
                      ...
                      --  VTEXBRAND     VB
                         -- validacion para listar solo articulos no catalogados en VTEX
                   WHERE TO_CHAR(BB.DEPARTMENTID||BB.CATEGORYID||BB.SUBCATEGORYID) NOT IN (SELECT TO_CHAR(VC.DEPARTMENTID||VC.CATEGORYID||VC.SUBCATEGORYID) 
                                                                                             FROM  VTEXCATALOG  VC)
                  --   AND VB.BRANDID = BB.BRANDID
                    /* --excluyo los no encontrados
                     AND BB.DEPARTMENTID<>-1
                     AND BB.CATEGORYID<> -1 
                     AND BB.SUBCATEGORYID<>-1
                     AND BB.BRANDID <> -1*/
                  order by  BB.DSDEPARTAMENTO,
                            BB.DEPARTMENTID,
                            BB.CATEGORYID,  
                            BB.SUBCATEGORYID;
                            
SELECT * FROM VTEXCATALOG ORDER BY 1;                            
