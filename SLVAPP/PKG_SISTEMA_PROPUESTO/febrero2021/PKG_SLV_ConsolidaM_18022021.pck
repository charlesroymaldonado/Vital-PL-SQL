...
CREATE OR REPLACE PACKAGE BODY PKG_SLV_ConsolidaM is
  ...
  * %v 09/12/2020 - LM   se agrega la fecha del pedido
  ***************************************************************************************************/
  ...
  PROCEDURE GetPedidosMultiCanal(p_DtHasta        IN DATE,
    ...
    v_idGrupo varchar2(40);
              
      --guardo en el cursor para agrupar
    cursor c_ped is
    select pp.idpersona, pp.identidad, pp.idcanal, trunc(pp.dtaplicacion) dtaplicacion, trunc(pp.dtentrega) dtentrega,
    pp.cdtipodireccion, pp.sqdireccion, nvl(pp.idcomisionista,'X') idcomisionista,pp.iceshijocomi, pp.nroorden
    from tbltmpslvpedagrupado pp
  ...
  BEGIN
    ...
    IF (v_idcomi IS NOT NULL) THEN
        --inserto los pedidos en a temporal
     insert into TBLTMPSLVPEDAGRUPADO (idtmppedagrup ,idpersona ,idcomisionista ,idcanal ,qtunidadmedidabase ,transid ,idgrupopedido ,
                                             cdtipodireccion ,sqdireccion ,dtaplicacion ,dtentrega ,identidad ,ammonto ,iceshijocomi ,nroorden )
      SELECT sys_guid(), --reparto
             p_idPersona,
             NULL, 
             pe.id_canal CANAL,
             (
              select nvl(sum(dp.qtunidadmedidabase),0) 
               from detallepedidos dp,
                    pedidos        pp
               where pp.transid = pe.transid
               and dp.idpedido = pp.idpedido
             ...
             sum(pe.ammonto),
             ( case 
               when INSTR(pe.transid,'-PGF')>0 then
                 1
               else
                 0
              end   
       ...
       WHERE pe.iddoctrx = do.iddoctrx
         ...
         AND pe.transid is not null --validacion para develop
       ...
       SELECT sys_guid(),  --comisionista
             ...
             nvl(o.dsobservacion ,'-') as nroorden
        FROM pedidos              pe,
             documentos           do,
    ...
    FOR r_ped in c_ped LOOP   --agrupo por cliente, direccion, fecha,canal, comisionista
        v_idGrupo:=sys_guid();
        update tbltmpslvpedagrupado pag set pag.idgrupopedido=v_idGrupo
        where pag.idpersona=r_ped.idpersona
        and nvl(pag.idcomisionista,'X')=r_ped.idcomisionista
        and pag.identidad=r_ped.identidad
        and pag.idcanal=r_ped.idcanal
        and trunc(pag.dtaplicacion)=trunc(r_ped.dtaplicacion)
        and trunc(pag.dtentrega)=trunc(r_ped.dtentrega)
        and pag.cdtipodireccion=r_ped.cdtipodireccion
        and pag.sqdireccion=r_ped.sqdireccion
        and pag.iceshijocomi=r_ped.iceshijocomi
        and pag.nroorden=r_ped.nroorden;
    END LOOP;
    
    OPEN p_cursor FOR
      SELECT pag.idgrupopedido TRANSID, --devuelve todos los pedidos de reparto
             NULL IDCOMISIONISTA,
             pag.idcanal CANAL,
             trunc(pag.dtaplicacion) DTPEDIDO,
             trunc(pag.dtentrega) DTENTREGA,
             e.CDCUIT CUIT,
             e.dsrazonsocial RAZONSOCIAL,
             de.dscalle || ' ' || de.dsnumero || ' (' ||
             trim(de.cdcodigopostal) || ') ' || lo.dslocalidad || ' - ' ||
             pro.dsprovincia DIRECCION,
             round(SUM(pag.ammonto), c_qtDecimales) MONTO,
             '-' COMISIONISTA,
             pag.nroorden ORDEN,
             pag.iceshijocomi FALTANTECOMI,
             sum(pag.qtunidadmedidabase) CantArt
        FROM tbltmpslvpedagrupado pag,
             direccionesentidades de,
             localidades          lo,
             provincias           pro, 
             entidades            e
       WHERE pag.idpersona=p_idPersona
         AND pag.idcanal<>'CO'
         AND pag.idcanal <> 'SA'
         AND pag.identidad = e.identidad
         AND pag.identidad = de.identidad
         AND pag.cdtipodireccion = de.cdtipodireccion
         AND pag.sqdireccion = de.sqdireccion
       ...
       GROUP BY pag.idgrupopedido,
                e.CDCUIT,
                trunc(pag.dtaplicacion),
                trunc(pag.dtentrega),
                pag.idcanal,
                e.dsrazonsocial,
                de.dscalle,
                ...
                pag.nroorden 
       UNION
        SELECT   peag.idgrupopedido TRANSID, --devuelve todos los pedidos de comi
                 peag.idcomisionista IDCOMISIONISTA,
                 peag.idcanal CANAL,
                 trunc(peag.dtaplicacion) DTPEDIDO,
                 trunc(peag.dtentrega) DTENTREGA,
                 e.CDCUIT CUIT,
                 e.dsrazonsocial RAZONSOCIAL,
                 de.dscalle || ' ' || de.dsnumero || ' (' ||
                 trim(de.cdcodigopostal) || ') ' || lo.dslocalidad || ' - ' ||
                 pro.dsprovincia DIRECCION,
                 round(SUM(peag.ammonto), 2) MONTO,
                 trim(ecomi.dsrazonsocial) || ' (' || trim(ecomi.cdcuit) || ')'  COMISIONISTA,
                 peag.nroorden orden,
                 peag.iceshijocomi FALTANTECOMI,
                 sum(peag.qtunidadmedidabase) CantArt
            FROM tbltmpslvpedagrupado peag, 
                 direccionesentidades de,
                 localidades          lo,
                 provincias           pro,
                 entidades            e,
                 entidades            eComi 
           WHERE peag.idpersona=p_idPersona
             AND peag.identidad = e.identidad
             AND peag.identidad = de.identidad
             AND peag.cdtipodireccion = de.cdtipodireccion
             AND peag.sqdireccion = de.sqdireccion
             and de.cdpais = pro.cdpais
             and de.cdprovincia = pro.cdprovincia
             and de.cdpais = lo.cdpais
             and de.cdprovincia = lo.cdprovincia
             AND de.cdlocalidad = lo.cdlocalidad
             AND peag.idcanal = 'CO'
             AND peag.idcomisionista=ecomi.identidad
           GROUP BY peag.idgrupopedido,
                    peag.idcomisionista,
                    ecomi.dsrazonsocial,
                    ecomi.cdcuit,
                    e.CDCUIT,
                    trunc(peag.dtaplicacion),
                    trunc(peag.dtentrega),
                    peag.idcanal,
                    e.dsrazonsocial,
                    de.dscalle,
                    de.dsnumero,
                    ...
                    peag.iceshijocomi,
                    peag.nroorden
          ORDER BY --dtentrega, direccion,
                    canal,comisionista,orden DESC;
  ...
  ***************************************************************************************************/
  PROCEDURE GetPedidosReparto(p_DtHasta IN DATE,
    ...
    cursor c_ped is
    select pp.idpersona, pp.identidad, pp.idcanal, trunc(pp.dtaplicacion) dtaplicacion, trunc(pp.dtentrega) dtentrega,
    pp.cdtipodireccion, pp.sqdireccion, nvl(pp.idcomisionista,'X') idcomisionista, pp.iceshijocomi
    from tbltmpslvpedagrupado pp
    where pp.idpersona=p_idPersona
    group by pp.idpersona, 
                trunc(pp.dtaplicacion),
  ...
  OPEN p_cursor FOR
      SELECT pag.idgrupopedido TRANSID, --devuelve todos los pedidos de reparto
       ...
       GROUP BY pag.idgrupopedido,
                ...
                pro.dsprovincia,
                pag.iceshijocomi
       ORDER BY trunc(pag.dtentrega), pro.dsprovincia, lo.dslocalidad DESC;
  
  --anterior agrupacion por transid
   /* OPEN p_cursor FOR
      SELECT pe.transid TRANSID, --devuelve todos los pedidos de reparto
             NULL IDCOMISIONISTA,
             pe.id_canal CANAL,
  ...
  * %v 21/01/2020 - ChM. Versión Inicial
  * %v 07/01/2020 - LM. se agrupa por cliente, fecha, direccion, canal
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    delete tbltmpslvpedagrupado where idpersona=p_idPersona;

    IF INSTR(p_idcanal, 'CO') <> 0 AND
       (INSTR(p_idcanal, 'VE') <> 0 OR INSTR(p_idcanal, 'TE') <> 0) THEN
      GetPedidosMultiCanal(v_dtHasta, p_IdComisionista,p_idPersona, p_Cursor);
    ELSE
      IF INSTR(p_idcanal, 'CO') = 0 THEN
        GetPedidosReparto(v_dtHasta,p_idPersona, p_Cursor);
      END IF;
      IF INSTR(p_idcanal, 'CO') <> 0 THEN
        GetPedidosComisionistas(p_idcomisionista,p_idPersona, p_Cursor);
      END IF;
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
...
/*      SELECT IDENT IdComisionista, CUIT || '- ' || RAZON RazonSocial
  ...
  *****************************************************************************************************/
  PROCEDURE GetPreVisualizarPedidos(p_QtBtoConsolidar IN  NUMBER,
    ...
    v_eshijo number:=0;
  BEGIN
    --valida si se mezclan pedidos hijos de comi con otros pedidos
    ...
    IF (p_TransId(1) IS NOT NULL and LENGTH(TRIM(p_TransId(1)))>1) THEN
      FOR i IN 1 .. p_TransId.Count LOOP
       ...
       --valida si se mezclan pedidos hijos de comi con otros pedidos
       select count(g.idtmppedagrup) into v_eshijo
       from tbltmpslvpedagrupado g
       where g.idgrupopedido=p_TransId(i)
       and g.iceshijocomi=1;
       if v_eshijo>0 then
         v_hijos:=v_hijos+1;
       end if;
       /*   IF INSTR(p_TransId(i),'-PGF') <> 0  THEN
            V_hijos:=V_hijos+1;
         END IF;*/
     ...
     insert into tbltmpslvConsolidadoM --inserta en la tmp los posibles consolidados Multicanal
          ...
          select sys_guid(),
                 ...
                 P.transid,
                 null
            from tbltmpslvpedagrupado p 
            where p.Idgrupopedido = p_TransId(i)
            and p.transid in (select ped.transid from pedidos ped where ped.transid=p.transid and ped.icestadosistema=c_pedi_liberado);
      ...
      END LOOP;
        --valida si se mezclan pedidos hijos de comi con otros pedidos
         IF V_hijos <> 0 and p_TransId.Count <> v_hijos   THEN
           p_Ok:=0;
           p_error:='Error no es posible consolidar pedidos de faltantes de Comisionista con otros pedidos!';
   	       ROLLBACK;
           RETURN;
         END IF;
    END IF;
  ...
  * %v 24/09/2020 - ChM  Elimino validación para permitir crear consolidadoM sin detalle por caso de sencibles
  * %v 17/12/2020 - LM   Filtro solo los pedidos en estado 2, por los pedidos repetidos desde venta movil
  * %v 18/02/2021 - ChM  Agrego manejo de excepcion de la funcion listagg
  *****************************************************************************************************/
  PROCEDURE SetConsolidadoMultiCanal(p_IdPersona        IN  personas.idpersona%type,
                                     p_IdConsolidadoM   OUT Tblslvconsolidadom.Idconsolidadom%type,
                                     p_Ok               OUT number,
    ...
    INSERT INTO tblslvconsolidadoMdet
      ...
      SELECT seq_consolidadomdet.nextval,
       ...
       WHERE trunc((cant / uxb), 0) >= v_qtbtoconsolidar --mayor a bultos a consolidar
         ...
         AND gs.cdsucursal = g_cdsucursal;
   --comento esta valdiación para permitir crear consolidadoM sin detalle por caso de sencibles 24/09/2020
    ...
    FOR PEDIDO IN (select d.identidadreal,
                    ...
                    where m.transid = p.transid
                     ...
                     -- and d.identidadreal = p.identidad
                      ...
                      and p.icestadosistema = c_pedi_liberado
                      and m.idcanal <> 'CO'
        ...
        INSERT INTO tblslvconsolidadopedidorel
             ...
             SELECT seq_consolidadopedidorel.nextval,
               ...
               FROM (SELECT SUBSTR(txt,
                           ...
                           INSTR(txt, ',', 1, level + 1) -
                           INSTR(txt, ',', 1, level) - 1) AS IDPEDIDO
                      FROM (SELECT replace(',' || PEDIDOCOMI.IDPEDIDO || ',', '''', '') AS txt
                              FROM dual)
                CONNECT BY level <=
                           LENGTH(txt) - LENGTH(REPLACE(txt, ',', '')) - 1) A;
  ...
  EXCEPTION
    WHEN OTHERS THEN      
      n_pkg_vitalpos_log_general.write(2,
                                       ...
                                       '  Error: ' || SQLERRM);
       p_Ok    := 0;   
       --Error por romper la función listagg
       --Error: ORA-01489: result of string concatenation is too long                              
      IF SQLCODE = '-01489' THEN 
        p_error := 'Imposible Realizar Consolidado. Seleccione menos pedidos e intente de nuevo.';
      ELSE
  ...
  * %v 30/11/2020 - ChM  Ajustó para no permitir generar faltante de CP si aún hay tareas abiertas
  *****************************************************************************************************/
  ...
  BEGIN
    ...
    select cp.idconsolidadopedido
     ...
     exception
      ...
      --validar que el pedido no tenga tareas sin finalizar
      if pkg_slv_consolidados.ContarTareasFinalizadas(p_IdconsolidadoPedido(i),c_TareaConsolidadoPedido)<>1 then
           p_Ok    := 0;
           p_error := 'Pedido '||p_IdconsolidadoPedido(i) ||' Tiene tareas sin finalizar.';
           RETURN;
      end if;    
