create materialized view TBLCTRLSTOCKARTCHM
on prebuilt table
     ...
     CantPickeada AS (  SELECT tablapick.cdarticulo, SUM (qtpick) qtpick
                          FROM (                            --- SLV Multicanal
                               SELECT td.cdarticulo,
                                         NVL (
                                            SUM (
                                               NVL (td.qtunidadmedidabasepicking,
                                                    0)),
                                            0)
                                            qtpick
                                    FROM slvapp.tblslvtarea               ta,
                                         slvapp.tblslvtareadet            td,
                                ...
                                SELECT rd.cdarticulo,
                                         NVL (
                                            SUM (
                                               NVL (rd.qtunidadmedidabasepicking,
                                                    0)),
                                            0)
                                            qtpick
                                    FROM slvapp.tblslvremito                re,
                                         slvapp.tblslvremitodet             rd,
                                         slvapp.tblslvpedfaltanterel        pf,
                                         (SELECT DISTINCT cp.idconsolidadopedido
                                            FROM slvapp.tblslvconsolidadopedido cp,
                                                 slvapp.
                                                 tblslvconsolidadopedidorel cpr,
                                                            ...
                                                            (SELECT 1
                                                              ...
                                                              WHERE docp.iddoctrx =
                                                                    ...
                                                                    AND de.
                                                                        cdprovincia =
                                                                           '23')) con
                                   WHERE ta.idconsolidadocomi =
                                            con.idconsolidadocomi
                                     and ta.idtarea = td.idtarea
                                GROUP BY td.cdarticulo
                                --- SLV Viejo
                                UNION ALL
                                    ...
                                    FROM tblslv_consolidado_pedido_det cpd,
                                         (SELECT DISTINCT cp.idconsolidado_pedido
                                            FROM tblslv_consolidado_pedido cp,
                                                 ...
                                                 pedidos p
                                           WHERE c.idconsolidado =
                                                    cp.idconsolidado
                                                 AND cp.idconsolidado_pedido =
                                                        cpr.idconsolidado_pedido
                                                 AND cpr.idpedido_pos =
                                                        p.idpedido
                                                 AND p.id_canal IN ('TE', 'VE')
                                                 AND c.fecha_consolidado >
                                                        SYSDATE - 15
                                                 AND cp.idestado <> 9
                                                 AND NOT EXISTS
                                                            (SELECT 1
                                                            ...
                                                            (SELECT 1
                                                              ...
                                                              WHERE docp.iddoctrx =
                                                                    ...
                                                                    AND de.
                                                                        cdprovincia =
                                                                           '23')) con
                                   WHERE cpd.idconsolidado_pedido =
                                            con.idconsolidado_pedido
