...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  *****************************************************************************************************/
  PROCEDURE SetLiberarArmador(p_IdArmador   IN   personas.idpersona%type,
   ...
   v_icgeneraremito           tblslvtipotarea.icgeneraremito%type;
   v_idremito                 tblslvremito.idremito%type;
   v_idubicacion              tblslvubicacionremito.idubicacionremito%type;
    BEGIN
      select count(*)
         into v_tarea
         from tblslvtarea ta,
  ...
  --recorre las tareas en curso para finalizar picking del detalle tarea
   ...
   loop
     ...
     if(v_tarea <> 0) then
         p_Ok:=0;
         p_error:='Imposible finalizar tarea '||to_char(curso.idtarea)||'artículo: '||curso.cdarticulo||' Comuniquese con Sistemas!';
         rollback;
         return;
     end if;
   end loop;
   ...
   loop
     ...
     if(v_tarea <> 0) then
         p_Ok:=0;
         p_error:='Imposible finalizar tarea: '||to_char(curso.idtarea)||' Comuniquese con Sistemas!';
         rollback;
         return;
     end if;
     ...
     if v_icgeneraremito = 1 then
       select re.idremito,
        ...
        if PKG_SLV_REMITOS.SetFinalizarRemito(v_idremito)=0 then
          ...
          if v_idubicacion <> 0 then
              PKG_SLV_REMITOS.SetRemitoUbicacion(v_idubicacion,v_idRemito,p_Ok,p_error);
              if p_ok = 0 then
                  n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       ' Error: ' || SQLERRM);               
