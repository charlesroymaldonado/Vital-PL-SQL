   ...
   v_modulo                           varchar2(100) := 'PKG_SLV_REPORTE.TotalArtComisionista';
    ...
    BEGIN
      ...
      OPEN p_Cursor FOR
               ...
               select rd.cdarticulo|| '- ' || da.vldescripcion Articulo ,
                      PKG_SLV_Articulo.SetFormatoArticulosCod(rd.cdarticulo,
                      --formato en piezas si es pesable  
                      decode(sum(nvl(rd.qtpiezaspicking,0)),0,sum(nvl(rd.qtunidadmedidabasepicking,0)),sum(nvl(rd.qtpiezaspicking,0))))  Cantidad, 
                      r.nrocarreta Carreta,
                      posapp.n_pkg_vitalpos_materiales.GetUxB(rd.cdarticulo) UXB  
                from  tblslvtarea t, 
                      tblslvremito r, 
                      tblslvremitodet rd, 
                      descripcionesarticulos da
