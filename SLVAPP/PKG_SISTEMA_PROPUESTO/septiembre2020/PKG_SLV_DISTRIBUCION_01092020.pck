  ...
  *****************************************************************************************************/
  ...
  insert into tblslvpordistribfaltantes
          (idporcdistribfaltantes,
           ...
           dtinsert)
    select SYS_GUID(),
           to_char(cp.idconsolidadopedido) idconsolidadopedido,
           pf.idpedfaltante,
    ...
    --trunc+1 las filas pares si aplica para ajustar las decimales
     If mod(v_Alterna,2)=0
        and (falt.qtunidadmedidabasepicking * falt.porcdist)-round(falt.qtunidadmedidabasepicking * falt.porcdist,0)<=0.5 then
         ...
         --valido si el redondeo sobre pasa lo solicitado ajusto a necesidad
 ...
 --Valida si todos los articulos se distribuyeron correctamente
   ValidaDistribucion(p_IdPedFaltante,c_TareaConsolidaPedidoFaltante,p_Ok,p_error);
    if p_Ok  = 0 then
         ...
         return;
    /*else
      --solo para prueba OJO agrego para poder ver el error en 
      --consolidadopedio sin generar remitos
      n_pkg_vitalpos_log_general.write(2,
                                      'Modulo: ' || v_modulo ||
                                      '  Detalle Error: Commit de prueba ejecutado' );
      commit;  */   
       end if;
