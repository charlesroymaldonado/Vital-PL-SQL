CREATE OR REPLACE PACKAGE PKG_SLV_TAREAS is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetArticulosXTarea(p_IdTarea       IN  tblslvtarea.idtarea%type,
                               p_TipoTarea     OUT tblslvtarea.cdtipo%type,
                               p_dtTarea       OUT tblslvtarea.dtinsert%type,                              
                               p_Idconsolidado OUT tblslvtarea.idconsolidadopedido%type,
                               p_DsSucursal    OUT sucursales.dssucursal%type,
                               p_Armador       OUT personas.dsnombre%type,                         
                               p_Cursor        OUT CURSOR_TYPE);
  ...
  v_modulo varchar2(100) := 'PKG_SLV_TAREAS.GetTareaAsigConsolidado';
    BEGIN
      OPEN p_Cursor FOR
        ...
        where ta.idpersonaarmador = p.idpersona
          and ta.cdestado = e.cdestado
          and case when p_tipoTarea = c_TareaConsolidadoMulti        and ta.idconsolidadom = p_idConsolidado then 1
                   when p_tipoTarea = c_TareaConsolidaMultiFaltante  and ta.idconsolidadom = p_idConsolidado then 1
                   when p_tipoTarea = c_TareaConsolidadoPedido       and ta.idconsolidadopedido = p_idConsolidado then 1 
                   when p_tipoTarea = c_TareaConsolidaPedidoFaltante and ta.idpedfaltante = p_idConsolidado then 1
                   when p_tipoTarea = c_TareaConsolidadoComi         and ta.idconsolidadocomi = p_idConsolidado then 1
                   when p_tipoTarea = c_TareaConsolidadoComiFaltante and ta.idconsolidadocomi = p_idConsolidado then 1       
              end = 1    
          and ta.cdtipo = p_tipoTarea
     order by ta.prioridad;
 /*    --TipoTarea 1,2 ConsolidadoM y ConsolidadoMFaltante
...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
  ...
  *****************************************************************************************************/
  ...
  END GetCarreta;
   ...
   /****************************************************************************************************
  * %v 09/05/2020 - ChM  Versión inicial GetArtBarraxTarea
  ...
  *****************************************************************************************************/
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      /* para validarlo renglon por renglon      
    --recorre para una idtarea los detalle tarea no finalizados
    for fintarea in (select det.cdarticulo, tip.icgeneraremito
                       from tblslvtarea     ta,
                            tblslvtareadet  det,
                            tblslvtipotarea tip
                      where ta.idtarea = p_IdTarea
    ...
    loop
      --actualiza los detalle tarea con valor cero indicando que quedan en faltante
      v_res := SetDetalleTarea(p_IdTarea,
                               fintarea.cdarticulo,
                               0,
                               'UN',
                               fintarea.icgeneraremito,
                               0);
      ...
      if v_res = -1 then
        p_Ok    := 0;
        p_error := 'Falla Actualizar Piking en cero. Comuniquese con sistemas!';
        rollback;
        return;
      end if;
      
      --Finaliza los articulos pendientes de la tarea
      if SetFindetalleTarea(p_IdTarea, fintarea.cdarticulo) <> 1 then
        p_Ok    := 0;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
    ...
    select ta.cdtipo,
           ...
           ta.idconsolidadom,
           ta.idconsolidadopedido,
           ta.idconsolidadocomi) consolidado,
           ta.dtinsert,
           upper(pe.dsnombre) || ' ' || upper(pe.dsapellido) Armador
      into p_TipoTarea,
           p_Idconsolidado,
