...
/*      SELECT IDENT IdComisionista, CUIT || '- ' || RAZON RazonSocial
  ...
  *****************************************************************************************************/
  FUNCTION VerificarBultosConsolidar(p_QtBtoConsolidar NUMBER,
   ...
   v_detalle     integer;
    BEGIN
         SELECT   trunc(A.cant / (posapp.n_pkg_vitalpos_materiales.GetUxB(A.cdarticulo)))    
          INTO v_detalle
  ...
  *****************************************************************************************************/
  ...
  BEGIN
     ...
     insert into tbltmpslvConsolidadoM --inserta en la tmp los posibles consolidados Multicanal
          ...
          select sys_guid(),
            ...
            from (select ped.id_canal canal,
                         ped.transid transid
                    from pedidos ped
                   where ped.transid = rpad(p_TransId(i),40,' ')
        ...
        IF SQL%ROWCOUNT = 0  THEN      --valida insert de la tabla tbltmpslvConsolidadoM
           ...
           RETURN;
        END IF;
        n_pkg_vitalpos_log_general.write(2,'GetPreVisualizarPedidos transid: ' || p_TransId(i) ||' - ' ||sysdate);
      END LOOP;
    END IF;
    ...
    IF (p_IdComisionista(1) IS NOT NULL and LENGTH(TRIM(p_IdComisionista(1)))>1) THEN
     ...
     insert into tbltmpslvConsolidadoM
        ...
        IF SQL%ROWCOUNT = 0  THEN
           ...
           RETURN;
        END IF;
        n_pkg_vitalpos_log_general.write(2,'GetPreVisualizarPedidos comiid: ' || p_IdComisionista(i) ||' - ' ||sysdate);
      END LOOP;
    END IF;
    IF (p_TransId(1) IS NOT NULL OR p_IdComisionista(1) IS NOT NULL) THEN
     ...
     n_pkg_vitalpos_log_general.write(2,'GetPreVisualizarPedidos antes de verificarBultos' || ' - ' ||sysdate);
     if VerificarBultosConsolidar(p_QtBtoConsolidar,p_idPersona) = 0 then
      ...
      n_pkg_vitalpos_log_general.write(2,'GetPreVisualizarPedidos despues de verificarBultos' || ' - ' ||sysdate);
      ...
      OPEN P_Cursor FOR --cursor de pedidos a consolidar multicanal
       SELECT gs.cdgrupo || ' - ' || gs.dsgruposector || ' (' ||
               ...
               PKG_SLV_Articulo.SetFormatoArticulosCod(A.COD,A.cant) cantidad,
               PKG_SLV_Articulo.SetFormatoArticulosCod(A.COD,PKG_SLV_Articulos.GetStockArticulos(A.COD)) stock,
         ...
         WHERE trunc(cant / posapp.n_pkg_vitalpos_materiales.GetUxB(A.COD), 0) >= 1 --mayor a bultos a consolidar
         ...
         COMMIT;
         n_pkg_vitalpos_log_general.write(2,'GetPreVisualizarPedidos despues de select de cursor' || ' - ' ||sysdate);
         p_Ok    := 1;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
   ...
   END IF;
    ...
    (idconsolidadom,qtconsolidado,idpersona,cdestado,dtinsert,dtupdate)
    VALUES
      (seq_consolidadom.nextval, v_QtBtoConsolidar, p_IdPersona, 1, SYSDATE, NULL);
      ...
      INSERT INTO tblslvconsolidadopedidorel
           ...
           SELECT seq_consolidadopedidorel.nextval,
             ...
             FROM (select p.idpedido
                    ...
                    where m.transid = p.transid
                      and d.iddoctrx = p.iddoctrx
                      and d.identidadreal = PEDIDO.identidadreal --valida identidad del cliente
                      and m.idpersona = p_idPersona
                      and m.idcomisionista is null
                      and m.idcanal <> 'CO'
  ...
  * %v 19/05/2020 - ChM  procedimiento para generar los faltantes de pedidos
 ...
 PROCEDURE SetConsolidadoPedidosFaltante(p_idpersona           IN  personas.idpersona%type,
    ...
    v_modulo       varchar2(100) := 'PKG_SLVConsolidaM.SetConsolidadoPedidosFaltante';
    v_error        varchar2(250);
  ...
  BEGIN
    FOR i IN 1 .. p_IdconsolidadoPedido.count LOOP
     --select para validar que el pedido no este facturado
      begin
      v_error := 'Falla validación con pedidos Facturados';
      select cp.idconsolidadopedido 
       ...
       where cp.idconsolidadopedido = prel.idconsolidadopedido
         ...
         and pe.idpedido = pcf.idpedido 
         and rownum=1;           
      if v_idpedido is not null then
          p_Ok    := 0;
          p_error := 'Error en consolidado pedido faltante. El pedido: '||
                      v_idpedido||' Ya está Facturado';
          return;
      end if;
      exception
        when no_data_found then
      ...
      if v_idpedido is not null then
          p_Ok    := 0;
          p_error := 'Error en consolidado pedido faltante. El pedido: '||
                      v_idpedido||' Ya és parte de un Consolidado Faltante';
          return;
       end if;
      exception
        when no_data_found then
    ...
    select cp.idconsolidadopedido 
     ...
     exception
        when no_data_found then
           p_Ok    := 0;
          p_error := 'Error en consolidado pedido faltante. El pedido: '||
                      v_idpedido||' no tiene artículos faltantes';
          return;
      end;  
    ...
    IF SQL%ROWCOUNT = 0  THEN      
        n_pkg_vitalpos_log_general.write(2,
                                       ...
                                       '  Detalle Error: ' || v_error);
   	    p_Ok:=0;
        p_error:='Error en consolidado pedido faltante. Comuniquese con Sistemas!';
        ROLLBACK;
        RETURN;
     END IF;
