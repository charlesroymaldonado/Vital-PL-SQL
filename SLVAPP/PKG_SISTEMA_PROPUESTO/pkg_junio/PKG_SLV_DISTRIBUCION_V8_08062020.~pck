...
CREATE OR REPLACE PACKAGE BODY PKG_SLV_DISTRIBUCION is
  ...
  *****************************************************************************************************/ 
  FUNCTION VerificaDiferenciasP(p_idPedido     tblslvconsolidadopedido.idconsolidadopedido%type)
                                return number is
    ...
    for cpedido in 
     ...
     loop
       begin
         ...
         select pc.qtunidadmedidabase
           into v_qtbase 
           from tblslvpedidoconformado pc
          where pc.idpedido = cpedido.idpedido
            and pc.cdarticulo = cpedido.cdarticulo
            and pc.sqdetallepedido = cpedido.sqdetallepedido;             
        exception
             ...
             insert into tblslvpedidoconformado
                ...
                left join observacionespedido          op
                    ...
                    where pe.idpedido = dpe.idpedido
                      ...
                      and pe.idpedido = cpedido.idpedido
                      and dpe.cdarticulo = cpedido.cdarticulo
                      and dpe.sqdetallepedido = cpedido.sqdetallepedido
                      and cp.idconsolidadopedido = cpedido.idpedido;
           IF SQL%ROWCOUNT = 0  THEN      
                n_pkg_vitalpos_log_general.write(2,
                                             'Modulo: ' || v_modulo ||
                                             '  Detalle Error: ' || v_error);               
       ...
       when others then
          ...
          if v_dif <> 0 then
            v_error:=' Error en update tblslvpedidoconformado';   
             update tblslvpedidoconformado pc
                set pc.qtunidadmedidabase = pc.qtunidadmedidabase+v_dif,
                    pc.dtupdate = sysdate
              where pc.idpedido = cpedido.idpedido
                and pc.cdarticulo = cpedido.cdarticulo
                and pc.sqdetallepedido = cpedido.sqdetallepedido;
