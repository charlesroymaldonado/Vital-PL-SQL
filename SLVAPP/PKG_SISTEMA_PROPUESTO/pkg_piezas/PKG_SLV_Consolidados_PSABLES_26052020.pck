   ...
   v_modulo varchar2(100) := 'PKG_SLV_Consolidados.GetArtPanelConsolidaPedido';
    ...
    BEGIN
      ...
      OPEN p_Cursor FOR
            select A.Sector,
                   A.articulo,
                   PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.base) Cantidad,
                   PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.picking) Cantidad_picking,
                   PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.diferencia) Diferencia,
                   PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.stock) stock,
                   A.uxb,
                   A.Ubicacion
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
