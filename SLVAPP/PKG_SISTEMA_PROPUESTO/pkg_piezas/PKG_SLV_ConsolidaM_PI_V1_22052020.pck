    ...
    select cp.idconsolidadopedido 
     ...
     where cp.idconsolidadopedido = prel.idconsolidadopedido
       ...
       and case 
             ...
             when cpd.qtpiezas<>0 and cpd.qtpiezas-nvl(cpd.qtpiezaspicking,0) <> 0 then 1
             --verifica los no pesable
             when cpd.qtpiezas = 0 and cpd.qtunidadesmedidabase-nvl(cpd.qtunidadmedidabasepicking,0) <> 0 then 1
           end = 1   
                ...
                (idpedfaltantedet,
                 ...
                 select seq_pedfaltantedet.nextval,
                   ...
                   from (select cpd.cdarticulo,                       
                                ...
                                sum(nvl(cpd.qtpiezas,0)-nvl(cpd.qtpiezaspicking,0)) piezas,
                                cpd.idgrupo_sector sector
                           from tblslvconsolidadopedido cp,
                                tblslvconsolidadopedidodet cpd
                          where cp.idconsolidadopedido = cpd.idconsolidadopedido
                            --valida insertar solo los articulos faltantes en su detalle y diferente de null 
                            and cpd.qtunidadesmedidabase-nvl(cpd.qtunidadmedidabasepicking,0) <> 0
                            and cpd.qtunidadmedidabasepicking is not null
                            --convierto a char en idconsolidadopedido para aplicar el artificio del arreglo
                            and to_char(cp.idconsolidadopedido) in 
                                (  select
