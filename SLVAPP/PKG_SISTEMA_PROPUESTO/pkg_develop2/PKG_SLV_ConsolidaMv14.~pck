create or replace package PKG_SLV_ConsolidaM is
  ...
  **********************************************************************************************************/
  ...
  PROCEDURE GetArticulosPanelConsolidadoM(p_idConsolidadoM  IN  Tblslvconsolidadom.Idconsolidadom%type,
                                          p_Cursor          OUT CURSOR_TYPE);  
                                          
  PROCEDURE GetArtFaltanteConsolidadoM(p_idConsolidadoM  IN  Tblslvconsolidadom.Idconsolidadom%type,
                                       p_Cursor          OUT CURSOR_TYPE);                                                                                                    
...
create or replace package body PKG_SLV_ConsolidaM is
  ...
  ****************************************************************************************************/
  ...
  c_FaltanteConsolidadoMulti CONSTANT tblslvtipotarea.cdtipo%type := 2;
  ...
  ***************************************************************************************************/
  ...
  PROCEDURE GetConsolidadoMC(p_DtDesde        IN DATE,
    ...
    v_modulo  varchar2(100) := 'PKG_SLVConsolidaM.GetConsolidadoMC';
    v_dtHasta date;
    v_dtDesde date;
    ...
    v_dtHasta := to_date(to_char(p_DtHasta, 'dd/mm/yyyy') || ' 23:59:59',
     ...
     OPEN p_Cursor FOR 
             Select m.idconsolidadom, 
                    ...
                    est.dsestado,
                    pkg_slv_ConsolidaM.SinAsigConsolidadoM(m.idconsolidadom) articulosSinAsignar,
                    pkg_slv_ConsolidaM.ConsolidadoMFaltante(m.idconsolidadom) tieneFaltante,
                    pkg_slv_ConsolidaM.SinAsigConsolidadoMFaltante(m.idconsolidadom) tieneFaltanteSinAsignar
  ...
  /****************************************************************************************************
  * %v 10/03/2020 - ChM  Versión inicial GetArticulosPanelConsolidadoM
  * %v 10/03/2020 - ChM  lista los articulos que conforman un IdConsolidadoM
  *****************************************************************************************************/
  PROCEDURE GetArticulosPanelConsolidadoM(p_idConsolidadoM  IN  Tblslvconsolidadom.Idconsolidadom%type,
                                          p_Cursor          OUT CURSOR_TYPE) IS
   
   v_modulo varchar2(100) := 'PKG_SLV_consolidaM.GetArticulosPanelConsolidadoM';
  
    BEGIN             
      OPEN p_Cursor FOR 
            select A.Sector,
                   A.articulo,
                   PKG_SLVArticulos.SetFormatoArticulos(A.cdarticulo,A.base) Cantidad, 
                   PKG_SLVArticulos.SetFormatoArticulos(A.cdarticulo,A.picking) Cantidad_picking,
                   PKG_SLVArticulos.SetFormatoArticulos(A.cdarticulo,A.diferencia) Diferencia,
                   A.uxb,
                   A.Ubicacion
    ...
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);                           
  END GetArticulosPanelConsolidadoM; 
  /****************************************************************************************************
  * %v 10/03/2020 - ChM  Versión inicial GetArtFaltanteConsolidadoM
  * %v 10/03/2020 - ChM  Muestra todos los artículos que entraron en el consolidado Multicanal con Faltante
  *****************************************************************************************************/
  PROCEDURE GetArtFaltanteConsolidadoM(p_idConsolidadoM  IN  Tblslvconsolidadom.Idconsolidadom%type,
                                             p_Cursor          OUT CURSOR_TYPE) IS
   
   v_modulo varchar2(100) := 'PKG_SLV_consolidaM.GetArtFaltanteConsolidadoM';
            ...
            select A.Sector,
              ...
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                     ...
                     where m.idconsolidadom = det.idconsolidadom
                       ...
                       and sec.cdsector = gs.cdsector
                       and art.cdarticulo = des.cdarticulo
                       --valida listar solo los ya pickiados
                       and det.qtunidadmedidabasepicking is not null
                       --valida listar solo los que tienen diferencia
    ...
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,
                                       'Modulo: ' || v_modulo ||
                                       '  Error: ' || SQLERRM);                           
  END GetArtFaltanteConsolidadoM; 
  /****************************************************************************************************
  ...
  ***************************************************************************************************/
  FUNCTION SinAsigConsolidadoM(p_idconsolidadom   tblslvconsolidadom.idconsolidadom%type)
                           return integer is
      v_modulo  varchar2(100) := 'PKG_SLVConsolidaM.SinAsigConsolidadoM';                    
      v_cont     integer;                     
  BEGIN  
  ...
  ***************************************************************************************************/
  FUNCTION SinAsigConsolidadoMFaltante(p_idconsolidadom   tblslvconsolidadom.idconsolidadom%type)
                           return integer is
      v_modulo  varchar2(100) := 'PKG_SLVConsolidaM.SinAsigConsolidadoMFaltante';                    
      v_cont     integer;                     
  BEGIN  
        Select count(m.idconsolidadom) 
          into v_cont    
              ...
              where m.idconsolidadom = det.idconsolidadom
                ...
                and det.cdarticulo not in(select td.cdarticulo  
                                           ...
                                           where ta.idtarea=td.idtarea
                                             and ta.idconsolidadom=m.idconsolidadom
                                             and ta.idpersona= m.idpersona
                                             and ta.cdtipo=c_FaltanteConsolidadoMulti);
      if v_cont <> 0 then
         return 1;
      end if;
         return 0;  
  ...
  ***************************************************************************************************/
  FUNCTION ConsolidadoMFaltante(p_idconsolidadom   tblslvconsolidadom.idconsolidadom%type)
                           return integer is
      v_modulo  varchar2(100) := 'PKG_SLVConsolidaM.ConsolidadoMFaltante';                    
      v_cont     integer;                     
  BEGIN  
        Select count(m.idconsolidadom) 
          into v_cont    
