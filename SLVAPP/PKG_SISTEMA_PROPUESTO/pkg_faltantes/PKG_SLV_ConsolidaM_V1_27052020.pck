...
/*      SELECT IDENT IdComisionista, CUIT || '- ' || RAZON RazonSocial
  ...
  *****************************************************************************************************/
  FUNCTION VerificarBultosConsolidar(p_QtBtoConsolidar NUMBER,
                                     ...
                                     RETURN INTEGER IS
   v_modulo varchar2(100) := 'PKG_SLV_ConsolidadoM.VerificarBultosConsolidar';  
   v_detalle     integer;
    BEGIN
        SELECT COUNT(*)
          INTO v_detalle
...
--                          ,gs.cdsector
     ...
     if VerificarBultosConsolidar(p_QtBtoConsolidar,p_idPersona) = 0 then
      ...
      OPEN P_Cursor FOR --cursor de pedidos a consolidar multicanal
        SELECT gs2.cdgrupo || ' - ' || gs2.dsgruposector || ' (' ||
               ...
               PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.cant) cantidad,
               PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,A.stock) stock,
               A.uxb,
               A.ubicacion
  ...
  BEGIN
    ...
    group by su.dssucursal,
      ...
      OPEN P_Cursor FOR
        SELECT A.COD,
               A.DESC_ART,                                
               PKG_SLVArticulos.SetFormatoArticulosCod(A.COD,
 ...
 PROCEDURE SetConsolidadoPedidosFaltante(p_idpersona           IN  personas.idpersona%type,
  ...
  /****************************************************************************************************
  * %v 27/05/2020 - ChM  Versión inicial calcula_porc
  * %v 27/05/2020 - ChM  calcula el procentaje de un articulo faltante a distribuir
  *****************************************************************************************************/ 
  PROCEDURE calcula_porc(p_idconsolidado integer,
                           p_cdart         detallepedidos.cdarticulo%type) is
                           
    v_modulo       varchar2(100) := 'PKG_SLVConsolidaM.calcula_porc';
    v_error        varchar2(250);
    p_total        number(10, 2);  
    
  BEGIN

    --busco el total del monto por cliente para calcular el porcentaje a aplicar en la distribucion
   -- p_total := get_base_distri(p_idconsolidado, p_cdart);

    --calculo el porcentaje y lo inserto en la tabla tblslvporcendistribfaltante
    insert into tblslvporcendistribfaltante
                  (idpedido,
                   idconsolidadom,
                   cdarticulo,
                   qtunidadmedidabase,
 ...
 PROCEDURE SetDistribucionPedidoFaltante(p_idpersona           IN  personas.idpersona%type,
    ...
    v_modulo       varchar2(100) := 'PKG_SLVConsolidaM.SetDistribucionPedidoFaltante';
    v_error        varchar2(250);
   -- v_idpedido     pedidos.idpedido%type := null;
