    ...
    BEGIN
      OPEN p_Cursor FOR
             Select m.idconsolidadom idconsolidado,
                    ...
                    PKG_SLVArticulos.SetFormatoArticulosCod(art.cdarticulo,
                    --formato en piezas si es pesable  
                    decode(det.qtpiezas,0,det.qtunidadmedidabase,det.qtpiezas)) Cantidad   
               from tblslvconsolidadom m,
                    tblslvconsolidadomdet det,
    ...
    BEGIN
      OPEN p_Cursor FOR
             Select cm.idconsolidadocomi idconsolidado,
                    ...
                    PKG_SLVArticulos.SetFormatoArticulosCod(art.cdarticulo,
                    --formato en piezas si es pesable  
                    decode(cdet.qtpiezas,0,cdet.qtunidadmedidabase,cdet.qtpiezas)) Cantidad                      
...
--    GetTareasByUserId(v_cur_tarea,p_idpersona);  -- OJOO utilizar para validar el permiso del usuario autentificado
    ...
    --devuelve la descripción del armador
     ...
     open p_cursor for
           select dta.cdarticulo Cdarticulo,
                  ...
                  PKG_SLVArticulos.GetCodigoDeBarra(dta.cdarticulo,
                  --valida pesables
       ...
       if tarea.icgeneraremito = 1 then
        open p_cursor for
           select dre.cdarticulo Cdarticulo,
                  ...
                  --valida pesables
                  decode(dre.qtpiezaspicking,0,
                  (nvl(dre.qtunidadmedidabasepicking,0)),
                  nvl(dre.qtpiezaspicking,0))) Cantidad
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona        IN   personas.idpersona%type,
   ...
   if length (p_cdarticulo)>1 and p_cantidad >= 0  then
      ...
      if p_cantidad > 0 then
         ...
         pkg_slvarticulos.GetValidaArticuloBarras(p_cdArticulo,p_cdBarras,v_cdunidad,v_cantidad_pes);
        else
  ...
  * %v 20/05/2020 - ChM  rellena al cabecera de tods los tipos genericos
  *****************************************************************************************************/
  ...
  * %v 20/05/2020 - ChM  rellena al cabecera del tipo c_TareaConsolidadoPedido
  *****************************************************************************************************/
