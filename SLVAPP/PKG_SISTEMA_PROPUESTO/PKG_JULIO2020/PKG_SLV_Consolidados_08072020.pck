  ...
  BEGIN
     ...
     OPEN p_Cursor FOR
           ...
           Distinct p.idconsolidadom,
                    ...
                    --activa la opción cerrar solo en estado C_EnCursoConsolidadoPedido
...
--consolidados comisionista
  ...
  *****************************************************************************************************/
  ...
  BEGIN
          begin
           ...
           exception
             when no_data_found then
               p_Ok    := 0;
               p_error := 'Consolidado '||p_idConsolidado||' no existe.';
               RETURN;
           end;
            --verifico si el pedido ya esta Finalizado
            if v_estado = C_FinalizadoConsolidadoM then
               p_Ok    := 0;
               p_error := 'Consolidado '||p_idConsolidado||' ya finalizado.';
               RETURN;
            end if;
          ...
          if v_tarea <> '_' then
              p_Ok:=0;
              p_error:=' No es posible finalizar el consolidado '||p_idConsolidado||
       ...
       exception
        ...
        begin
         ...
         if v_tarea <> '_' then
              ...
              ||p_idConsolidado||', tiene artículos sin asignar';
              return;
          end if;           
       exception
        when no_data_found then
  ...
  *****************************************************************************************************/
  ...
  BEGIN
          ...
          if v_ped is not null then
              p_Ok    := 0;
              p_error := 'El pedido '||p_idConsolidado||' pertenece al faltante '||v_ped|| ' sin distribuir';
              return;
          end if;
          exception
            when no_data_found then
          ...
          if v_tarea <> '_' then
              ...
              ||p_idConsolidado||', tiene tareas sin finalizar ('||v_tarea||')';
              return;
          end if;
       exception
        when no_data_found then
  ...
  *****************************************************************************************************/
  ...
  BEGIN
         ...
         exception
           when no_data_found then
             p_Ok    := 0;
             p_error := 'Pedido faltante '||p_idConsolidado||' no existe.';
             RETURN;
         end;
         --verifico si el faltante esta distribuido
          if v_ped = C_DistribFaltanteConsolidaPed then
             p_Ok    := 0;
             p_error := 'Pedido faltante '||p_idConsolidado||' ya distribuido.';
             RETURN;
          end if;
         ...
         if v_ped = C_FinalizaFaltaConsolidaPedido then
           p_Ok    := 0;
           p_error := 'Pedido faltante '||p_idConsolidado||' ya finalizado.';
           RETURN;
        end if;
