     ...
     if p_IdtareaManual is null then
       ...
       if p_IdtareaManual is not null then
         ...
         if v_cdtipotarea <> P_tipoTareaManual and P_tipoTareaManual is not null then
            p_Ok    := 0;
            p_error:='La Tarea '||p_IdtareaManual||' no corresponde al tipo '||P_tipoTareaManual||' enviado!';
            return;
         end if;
      end if;
     ...
     begin
        ...
        if tarea.cdmodoingreso = c_TareaHandHeld then
          if tarea.idtarea = p_IdtareaManual and p_IdtareaManual is not null then
             p_Ok    := 0;
             p_error:='La tarea: '||p_IdtareaManual||' esta definida para picking con HandHeld!';
             return;
          end if;
        end if;
  ...
  *****************************************************************************************************/
  ...
  BEGIN
   ...
   if v_cdmodoingreso <> p_cdmodoingreso  then
      p_Ok    := 0;
      p_error:='La Tarea: '||p_Idtarea||' no corresponde al tipo picking enviado!';
      return;
   end if;
   ...
   if v_cdmodoingreso <> c_TareaHandHeld  then
      if p_IdPersonaManual is null then
         p_Ok    := 0;
         p_error:='La Tarea: '||p_Idtarea||' está en modo ingreso MANUAL con IdPersona en NULL';
         return;
      end if;
   end if;
