...
****************************************************************************************************/
  ...
  *****************************************************************************************************/
  ...
  BEGIN
      ...
      --si desea finalizar (F) llama a SetControlarRemito y finaliza
      ...
      --si no lo encuentra continua el proceso (no error al usuario)
  ...
  END SetControl;
   /****************************************************************************************************
  * %v 27/07/2020 - ChM  Versión inicial SetReConteo
  * %v 27/07/2020 - ChM  recibe un idremito para recontar diferencias
  *****************************************************************************************************/
   PROCEDURE SetReConteo(p_idremito           IN  tblslvremito.idremito%type, 
    ...
    v_modulo           varchar2(100) := 'PKG_SLV_CONTROL.SetReConteo';
    v_estado           tblslvremito.cdestado%type:=null;
    v_idControl        tblslvcontrolremito.idcontrolremito%type;
    v_qtcontrol        tblslvcontrolremito.qtcontrol%type;         
    v_idConteo         tblslvconteo.idconteo%type;
  ...
  *****************************************************************************************************/
   PROCEDURE VerificaConteo(p_idControl          IN  tblslvconteo.idcontrolremito%type,
                            p_Ok                 OUT number,
                            p_error              OUT varchar2)
                            IS
  ...
  BEGIN
    --valida si existe el p_idControl en tblslvcontrolremito 
       ...
       exception                  
          when no_data_found then
              p_Ok    := 0;
              p_error := 'Control del Remito'||to_char(v_idremito)||' no existe.';
              return;
       end;
       ...
       exception                  
          when no_data_found then
              p_Ok    := 0;
              p_error := 'Conteo del remito '||to_char(v_idremito)||' no existe.';
              return;
       end;
      ...
      if v_estado in (C_Controlado, C_ControladoConError) then
         p_Ok    := 0;
         p_error := 'Remito '||to_char(v_idremito)||' ya Controlado.';
         RETURN;
      end if;  
   ...
   BEGIN
     --valida si existe el p_idremito en tblslvcontrolremito 
      ...
      if v_estado in (C_Controlado,C_ControladoConError) then
       ...
       --si desea finalizar (F) llama a VerificaConteo  OOOOOJOOOO VOY AQUI
      if p_cdBarras ='F' then
        VerificaConteo(v_idControl,p_Ok,p_error);
         if p_Ok = 0 then
            p_Ok    := 0;            
            return;          
         end if;         
     ...
     if v_cdunidad in ('KG','PZA') then
      ...
      -- si el articulo tiene diferencias 
      if v_diferencias <> 0 then
        -- intento actualizar en conteoDet
         if UpdateConteoDet(v_IdConteo,v_cdArticulo,v_qtbase,v_qtpiezas) = 0 then
           --sino actualiza inserta
            if InsertConteoDet(v_IdConteo,v_cdArticulo,v_qtbase,v_qtpiezas) = 0 then
                p_Ok    := 0;
                p_error := 'Error. Comuniquese con sistemas.';
                ROLLBACK;        
                RETURN;
