  ...
  PROCEDURE GetControl(p_idremito           IN  tblslvremito.idremito%type,
   ...
   BEGIN
    ...
    if v_estado = C_Controlado then
       p_Ok    := 0;
       p_error := 'Error Remito N° '||p_idremito||' Ya Controlado.';
       RETURN;
    end if;      
    p_Ok    := 1;
    p_error := '';
  ...
  BEGIN
     ...
     begin        
      ...
      when no_data_found then
        n_pkg_vitalpos_log_general.write(2,
                                        'Modulo: ' || v_modulo ||
                                        '  Control de Remito Sin detalle Piqueado ' || v_error);
        p_Ok    := 0;
   ...
   --busca las diferencias del control con el remito y las actualiza en el tblslvcontrolremitodet
       ...
       loop
         ...
         loop
            ...
            update tblslvcontrolremitodet crd
               ...
               set crd.qtdiferenciaunidadmbase = decode(remito.qtpiezas,0,nvl(crd.qtunidadmedidabasepicking,0)-remito.qtbase,0),
    ...
    --si no hay diferencias paso el estado de tblslvcontrolremito a controlado  
    v_band:=-1;
    select count (*)
      into v_band
      from tblslvcontrolremitodet    crd
     where crd.idcontrolremito = p_idControl
        -- verifica que no hay diferencias en el detalle 
       and crd.qtdiferenciaunidadmbase = 0 
       and crd.qtdiferenciapiezas = 0;
     --valida si no hay diferencias paso tblslvcontrolremito estado controlado  
     if v_band = 0  then
