  ...
  *****************************************************************************************************/
  ...
  PROCEDURE SetRegistrarPicking(p_IdPersona        IN   personas.idpersona%type,
   ...
   if length (p_cdarticulo)>1 and p_cantidad >= 0  then     
     ...
     --valida unidad de venta minima ChM 20/07/2020    
  ...
  BEGIN
   ...
   loop
     ...
     if nvl(p_idpersonamanual,'X')='X' then  --es picking hh
       PKG_SLV_TAREAS.SetRegistrarPicking(p_idpersona,v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, r_artbolsin.cdarticulo, p_idtarea,r_artbolsin.idpedido, p_Fintarea, p_ok, p_error); 
     else
       PKG_SLV_TAREAS.SetRegistrarPickingM(v_idRemito, p_NroCarreta, r_artbolsin.eancode, r_artbolsin.qtunidadpedido, p_IdTarea,p_IdPersonaManual, 1,r_artbolsin.idpedido, p_Fintarea, p_ok, p_error);
     end if;
     if (p_ok=0) then
       Rollback;
       Exit;
