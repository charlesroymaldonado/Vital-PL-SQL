 ...
 BEGIN
   ...
   --verifico si el faltante esta finalizado y se puede distribuir
    ...
    --select para validar que el pedido no esta en tblslvdistribucionpedfaltante
      ...
      if v_pedfal > 0 then
          p_Ok    := 0;
          p_error := 'Pedido Faltante'||p_IdPedFaltante||' ya distribuido';
          return;
      end if;
      exception
        when no_data_found then
   ...
   if P_OK = 0 then
    ...
    if v_pedfal = 0 then
       p_Ok    := 0;
       p_error := 'Pedido Faltante '||p_IdPedFaltante|| 'no tiene artículos a Distribuir.';
       RETURN;
    end if;   
 ...
 BEGIN
   ...
   --select para validar que el pedido no es parte de un consolidado faltante sin distribuir
      ...
      if v_pedfal is not null then
          p_Ok    := 0;
          p_error := 'El pedido '||p_IdPedidos||' pertenece al faltante '||v_pedfal|| 'sin distribuir';
          return;
      end if;
      exception
        when no_data_found then
   ...
   exception
    ...
    --select para validar que el pedido no esta en tblslvpedidoconformado
      ...
      if v_pedfal > 0 then
          p_Ok    := 0;
          p_error := 'Pedido '||p_IdPedidos||' ya distribuido';
          return;
      end if;
      exception
        when no_data_found then
   ...
   --verifico si el consolidado pedido no tiene picking
    if v_ped = 0 then
       p_Ok    := 0;
       p_error := 'Pedido '||p_IdPedidos|| ' no tiene artículos a facturar.';
       RETURN;
    end if; 
 ...
 *****************************************************************************************************/
 ...
 BEGIN
   ...
   exception
     when no_data_found then
       p_Ok    := 0;
       p_error := 'Pedido comisionista '||p_IdComi||' no existe.';
       RETURN;
   end;
    --verifico si el pedido comi ya esta distribuido
    if v_estado in (C_AfacturarConsolidadoComi,C_FacturadoConsolidadoComi) then
       p_Ok    := 0;
       p_error := 'Pedido comisionista '||p_IdComi||' ya distribuido.';
       RETURN;
    end if;
   --verifico si el pedido esta cerrado y se puede distribuir
    if v_estado <> C_FinalizadoConsolidadoComi  then
       p_Ok    := 0;
       p_error := 'Pedido comisionista '||p_IdComi||' no finalizado. No es posible Facturar.';
       RETURN;
    end if;
   ...
   --calcula los porcentajes que se aplicarán en la distribución de consolidados comi
    ...
    if v_ped = 0 then
       p_Ok    := 0;
       p_error := 'Pedido comisionista '||p_IdComi|| 'no tiene artículos a facturar.';
       RETURN;
    end if;
