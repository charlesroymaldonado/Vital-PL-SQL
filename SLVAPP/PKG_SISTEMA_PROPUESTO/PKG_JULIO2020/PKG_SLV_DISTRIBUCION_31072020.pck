 ...
 *****************************************************************************************************/
 ...
 BEGIN
  ...
  --verifica la distibucion de bultos para redondear decimales
  --OOOOOOOOOOOOOJJJJJJJJJJJJJOOOOOOOOOOOOOOOOOOOO
 -------------------------------------------------------------
  ...
  *****************************************************************************************************/
  ...
  BEGIN
   ...
   v_uxb:=PKG_SLV_ARTICULO.GetUXBArticulo(p_cdarticulo,'BTO');
    ...
    select sum(pd.diferencia)
      into v_diferencia
      from tblslvpruebadistribucion PD
     where pd.cdarticulo=p_cdarticulo
       --and pd.idpedfaltante = p_IdPedFaltante
       ; 
    ...
    for ajuste in
       (select pd.iddistribucionpedfaltante,
               pd.necesita 
          from tblslvpruebadistribucion pd
         where pd.cdarticulo = p_cdarticulo
          -- and pd.idpedfaltante = p_IdPedFaltante
    ...
    loop
      --rompe el ciclo sino hay nada que distribuir
      EXIT WHEN v_diferencia <= 0;
      --verifica si el articulo necesita (sino necesita no se puede ajustar nada el faltante completó el pedido)
      if ajuste.necesita > 0 then
          --verifica si la diferencia a distribuir alcanza para la necesidad
          if v_diferencia-ajuste.necesita>0 then
             v_cantidad:=ajuste.necesita;
             v_diferencia:=v_diferencia-ajuste.necesita;
          else
             v_cantidad:=v_diferencia;   
             v_diferencia:=0;
          end if;
          --actualiza la cantidad en el articulo del renglon de la distribución 
          v_error:='Error Actualizando tblslvpruebadistribucion';
          update tblslvpruebadistribucion pd
             set pd.unidades = v_cantidad                
           where pd.cdarticulo = p_cdarticulo
             and pd.iddistribucionpedfaltante = ajuste.iddistribucionpedfaltante
             -- and pd.idpedfaltante = p_IdPedFaltante
                  ;
          IF SQL%ROWCOUNT = 0 THEN
               n_pkg_vitalpos_log_general.write(2,
           ...
           --marca el ajuste del articulo para la nueva distribución 
            v_error:='Error Actualizando tblslvpruebadistribucion marca de ajuste';
          update tblslvpruebadistribucion pd
             set pd.nueva_distrib=1  
           where pd.cdarticulo = p_cdarticulo
            -- and pd.idpedfaltante = p_IdPedFaltante
                  ;
          IF SQL%ROWCOUNT = 0 THEN
