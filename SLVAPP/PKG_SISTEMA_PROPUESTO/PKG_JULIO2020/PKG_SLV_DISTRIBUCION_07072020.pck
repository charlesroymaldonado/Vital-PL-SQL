 ...
 BEGIN
   ...
   --verifico si el faltante esta finalizado y se puede distribuir
    ...
    --select para validar que el pedido no esta en tblslvdistribucionpedfaltante
      ...
      if v_pedfal > 0 then
          p_Ok    := 0;
          p_error := 'Error Pedido N°: '||p_IdPedFaltante||' ya distribuido';
          return;
      end if;
      exception
        when no_data_found then
 ...
 BEGIN
   ...
   exception
    ...
    --select para validar que el pedido no esta en tblslvpedidoconformado
      ...
      if v_pedfal > 0 then
          p_Ok    := 0;
          p_error := 'Error Pedido N°: '||p_IdPedidos||' ya distribuido';
          return;
      end if;
      exception
        when no_data_found then
 ...
 BEGIN
   ...
   --verifico si el pedido esta cerrado y se puede distribuir
    if v_estado <> C_FinalizadoConsolidadoComi  then
      ...
      if v_ped > 0 then
          p_Ok    := 0;
          p_error := 'Error Pedido Comisionista N°: '||p_IdComi||' ya distribuido';
          return;
      end if;
      exception
        when no_data_found then
