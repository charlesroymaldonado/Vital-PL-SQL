  ...
  BEGIN
    FOR i IN 1 .. p_IdconsolidadoPedido.count LOOP
     ...
     exception
      ...
      if v_idpedido is not null then
          p_Ok    := 0;
          p_error := 'Pedido '||v_idpedido||' ya es parte de un consolidado faltante.';
          return;
       end if;
      exception
        when no_data_found then
    ...
    select cp.idconsolidadopedido 
     ...
     exception
        when no_data_found then
           p_Ok    := 0;
          p_error := 'Pedido '||v_idpedido||' no tiene artículos faltantes';
          return;
      end;  
