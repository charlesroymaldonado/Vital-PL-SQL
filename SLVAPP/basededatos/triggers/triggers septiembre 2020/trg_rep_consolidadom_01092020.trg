CREATE OR REPLACE TRIGGER trg_rep_consolidadom
    after insert or update ON SLVAPP.TBLSLVCONSOLIDADOM  for each row
declare
   v_modulo          varchar2(100) := 'trg_rep_consolidadom';
   v_Accion          varchar2(3);
...
begin
 ...
 --  v_cdSucursal := getvlparametro('CDSucursal', 'General');
   ...
   values
   (seq_tblreplica.nextval, 'tblslvconsolidadom', v_Accion, :new.idconsolidadom, REPLICAS_GENERAL.get_estado_inicial, sysdate, :new.cdsucursal);
