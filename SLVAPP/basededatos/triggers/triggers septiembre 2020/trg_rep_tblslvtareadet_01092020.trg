CREATE OR REPLACE TRIGGER trg_rep_tblslvtareadet
    after insert or update on SLVAPP.tblslvtareadet for each row                                
declare
   v_modulo          varchar2(100) := 'trg_rep_tblslvtareadet';
   v_Accion          varchar2(3);
...
begin
   ...
   --elimino la condición por reportes de BW
 --  if :new.idconsolidadopedido is not null then
      ...
      values
        (seq_tblreplica.nextval, 'tblslvtareadet', v_Accion,:new.idtareadet, REPLICAS_GENERAL.get_estado_inicial, sysdate, :new.cdsucursal);
   -- end if;    
