CREATE OR REPLACE PACKAGE PKG_SLV_Consolidados is
 ...
 PROCEDURE SetEstadoConsolidadoFacturado  (p_DtDesde        IN DATE,
  ...
  FUNCTION   PickingTarea (p_idpedido        tblslvconsolidadom.idconsolidadom%type,
                           p_TipoTarea       tblslvtipotarea.cdtipo%type,
                           p_cdarticulo      tblslvtareadet.cdarticulo%type,
                           p_tipo_art        integer default 0)
                           RETURN NUMBER;

   FUNCTION ContarTareasFinalizadas(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,
                                    p_TipoTarea     tblslvtipotarea.cdtipo%type)
                                    return integer;
 ...
 PROCEDURE GetConsolidado  (p_DtDesde        IN DATE,
  ...
  FUNCTION TieneConsoFaltante(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type)
                              return integer;
  FUNCTION TareaConsolidadoArt(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,
                                p_cdarticulo    articulos.cdarticulo%type,
                                p_TipoTarea     tblslvtipotarea.cdtipo%type)
                              return varchar2;

  FUNCTION TieneBolsin(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,p_tipo tblslvtarea.cdtipo%type)
                              return varchar2;
...
CREATE OR REPLACE PACKAGE BODY PKG_SLV_Consolidados is
  ...
  BEGIN
      OPEN p_Cursor FOR
             Select m.idconsolidadom,
                    ...
                    est.dsestado,
                    --cantidad de SKU distintos que lo conforman
                    nvl((select count(cmd.cdarticulo)
                           from tblslvconsolidadomdet cmd
                          where cmd.idconsolidadom = m.idconsolidadom),0) SKUDistintos,
                    PKG_SLV_Consolidados.SinAsigConsolidadoM(m.idconsolidadom) articulosSinAsignar,
                    PKG_SLV_Consolidados.ConsolidadoMFaltante(m.idconsolidadom,p_TipoTarea,0) tieneFaltante,
              ...
              --and (p_estado=0 or m.cdestado=p_estado)
               AND
               (
                (p_estado is not null AND m.cdestado IN
                (SELECT Column_Value IDESTADO FROM TABLE(PKG_SLV_COMMON.SPLITPEDIDOSBYCOMA(CAST(p_estado AS VARCHAR2(4000)),',')))
                )
                OR
                (p_estado is null and m.cdestado = m.cdestado)
                )
  ...
  * %v 07/07/2020 - ChM  ajusto la activación o no de los botones del panel
  * %v 30/11/2020 - ChM  ajusto activar opción de faltantes si todas las tareas están finalizadas
  * %v 09/12/2020 - LM   se agrega el idFaltante si es que se encuentra en uno
  ***************************************************************************************************/
  ...
  BEGIN
     ...
     OPEN p_Cursor FOR
           ...
           Distinct p.idconsolidadom,
                    ...
                    --cantidad de SKU distintos que lo conforman
                    (select count(cpd.cdarticulo)
                       from tblslvconsolidadopedidodet cpd
                      where cpd.idconsolidadopedido = p.idconsolidadopedido) SKUDistintos,
                    PKG_SLV_Consolidados.SinAsigConsolidadoPedido(p.idconsolidadopedido)  articulosSinAsignar,
                    PKG_SLV_Consolidados.ConsolidadoPedidoFaltante(p.idconsolidadopedido, p_TipoTarea,1) tieneFaltante,
                    ...
                    -- sino evalua si tiene faltante y esta en curso
                    -- y si todas las tareas estan finalizadas 30/11/2020
                    case
                      when PKG_SLV_Consolidados.PedFaltanteConsolidadoPedido(p.idconsolidadopedido) = 1 then 0
                      else decode(PKG_SLV_Consolidados.ConsolidadoPedidoFaltante(p.idconsolidadopedido,p_TipoTarea,0),1,
                           decode(p.cdestado,C_EnCursoConsolidadoPedido,
                           decode(PKG_SLV_Consolidados.ContarTareasFinalizadas(p.idconsolidadopedido,p_TipoTarea),1,1,0),0),0)
                     end consFantante,
                     ...
                     -- sino 0
                     decode(PKG_SLV_Consolidados.ConsolidadoPedidoFaltante(p.idconsolidadopedido,p_TipoTarea,0),1
                    ...
                    --muestra el idFaltante si esta dentro de un faltante
                    (select nvl((select  pfr.idpedfaltante  from tblslvpedfaltanterel pfr
                     where pfr.idconsolidadopedido=p.idconsolidadopedido),0) from dual) IdFaltante,
                     PKG_SLV_CONSOLIDADOS.TieneBolsin(p.idconsolidadopedido,c_TareaConsolidadoPedido) tieneBolsin
              ...
              where est.cdestado = p.cdestado
                ...
                --and (p_estado=0 or p.cdestado=p_estado)
                AND
                 (
                  (p_estado is not null AND p.cdestado IN
                  (SELECT Column_Value IDESTADO FROM TABLE(PKG_SLV_COMMON.SPLITPEDIDOSBYCOMA(CAST(p_estado AS VARCHAR2(4000)),',')))
                  )
                  OR
                  (p_estado is null and p.cdestado = p.cdestado)
                  )
  ...
  BEGIN
     ...
     OPEN p_Cursor FOR
             Select p.idpedfaltante,
                    ...
                    est.dsestado,
                    --cantidad de SKU distintos que lo conforman
                    (select count(pfd.cdarticulo)
                       from tblslvpedfaltantedet pfd
                      where pfd.idpedfaltante = p.idpedfaltante) SKUDistintos,
                    ...
                    PKG_SLV_Consolidados.PedFaltante(p.idpedfaltante,p_TipoTarea,1)   tieneFaltante,
                    --desactiva la opción tieneFaltante si no tiene articulos faltantes o los estados
                    case
                      when PKG_SLV_Consolidados.PedFaltante(p.idpedfaltante,p_TipoTarea,0) = 0
                           or p.cdestado in
  ...
  BEGIN
     ...
     OPEN p_Cursor FOR
             Select distinct
                    ...
                    --cantidad de SKU distintos que lo conforman
                    (select count(ccd.cdarticulo)
                       from tblslvconsolidadocomidet ccd
                      where ccd.idconsolidadocomi = c.idconsolidadocomi) SKUDistintos,
                    ...
                    PKG_SLV_Consolidados.ConsolidadoComiFaltante(c.idconsolidadocomi,p_TipoTarea,1) tieneFaltante,
                    ...
                    case
                      when PKG_SLV_Consolidados.ConsolidadoComiFaltante(c.idconsolidadocomi,p_TipoTarea,0) = 0
                    ...
                    c.grupo zona,
                    PKG_SLV_CONSOLIDADOS.TieneBolsin(c.idconsolidadocomi,c_TareaConsolidadoComi) tieneBolsin
              ...
              where est.cdestado = c.cdestado
                ...
                --and (p_estado=0 or c.cdestado=p_estado)
                 AND
                 (
                  (p_estado is not null AND c.cdestado IN
                  (SELECT Column_Value IDESTADO FROM TABLE(PKG_SLV_COMMON.SPLITPEDIDOSBYCOMA(CAST(p_estado AS VARCHAR2(4000)),',')))
                  )
                  OR
                  (p_estado is null and c.cdestado = c.cdestado)
                  )
  ...
  * %v 16/11/2020 - LM   se agrega el filtro por estados//se cambia a multiestados
  *****************************************************************************************************/
  PROCEDURE GetConsolidado  (p_DtDesde        IN DATE,
                             p_DtHasta        IN DATE,
                             p_TipoTarea      IN tblslvtipotarea.cdtipo%type,
  ...
  BEGIN
    ...
    SetEstadoConsolidadoFacturado(v_dtDesde, v_dtHasta);
     ...
     if p_TipoTarea in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante) then
      GetConsolidadoMC(v_DtDesde,v_DtHasta,p_TipoTarea,p_estado,p_Cursor);
     end if;
     --TipoTarea 25 Consolidado pedido
     if p_TipoTarea = c_TareaConsolidadoPedido then
      GetConsolidadoPedido(v_DtDesde,v_DtHasta,p_TipoTarea,p_estado,p_Cursor);
     end if;
     ...
     if p_TipoTarea in(c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante) then
      GetConsolidadoPedidoFaltante(v_DtDesde,v_DtHasta,p_TipoTarea,p_estado,p_Cursor);
     end if;
    ...
    if p_TipoTarea in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
      GetConsolidadoComi(v_DtDesde,v_DtHasta,p_idcomi,p_TipoTarea,p_estado,p_Cursor);
    end if;
  ...
  * Agrego el estado 6/9  - anulado/entregado, para salvar los casos en los que crean la guia antes de actualizar el estado del SLVM
  * 14/06/2021 LM: se buscan 7 dias para atras, para tomar los consolidado que hayan quedado sin actualizar estados
  *****************************************************************************************************/
 PROCEDURE SetEstadoConsolidadoFacturado  (p_DtDesde        IN DATE,
         ...
         WHERE cpu.idconsolidadopedido IN
               (SELECT CP.Idconsolidadopedido
                 ...
                 WHERE PED.IDDOCTRX = DOCU.IDDOCTRX
                   ...
                   AND PED.ICESTADOSISTEMA IN (4,5,6,8,9)
                   AND TRUNC(cp.Dtinsert) BETWEEN TRUNC(NVL(p_DtDesde, SYSDATE)-7)
         ...
         WHERE ccu.idconsolidadocomi IN
               (SELECT distinct CP.Idconsolidadocomi
                 ...
                 WHERE PED.IDDOCTRX = DOCU.IDDOCTRX
                   ...
                   AND PED.ICESTADOSISTEMA IN (4,5,6,8,9)
                   AND TRUNC(CONS.Dtinsert) BETWEEN TRUNC(NVL(p_DtDesde, SYSDATE)-7)
                                                         AND TRUNC(NVL(p_DtHasta, SYSDATE)+1));

  END SetEstadoConsolidadoFacturado;

/****************************************************************************************************
  * %v 08/09/2020 - ChM  Versión inicial PickingTarea
  * %v 08/09/2020 - ChM  Obtener el picking de la tarea de un consolidado y articulo
  * %v 30/09/2020 - ChM Agrego validación de tarea desasignada and td.icdesasignar = 0
  * %v 16/12/2020 - LM  se corrige para que devuelva lo pickeado en las tareas de los faltantes de cons faltante y cons comi
  * %v 09/04/2021 - LM  se corrige para que devuelva lo pickeado en las tareas de los faltantes de cons multicanal
  *****************************************************************************************************/
  FUNCTION   PickingTarea (p_idpedido        tblslvconsolidadom.idconsolidadom%type,
                           p_TipoTarea       tblslvtipotarea.cdtipo%type,
                           p_cdarticulo      tblslvtareadet.cdarticulo%type,
    ...
    v_qtbase        tblslvtareadet.qtunidadmedidabase%type;
    v_qtpiezas      tblslvtareadet.qtpiezas%type;
    v_tareareporte  tblslvtipotarea.cdtipo%type;

  BEGIN
    --TipoTarea 10,20 ConsolidadoM y ConsolidadoMFaltante
     if p_TipoTarea in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante) then
         begin
           v_tareareporte:= c_TareaConsolidadoMulti;
         select sum(nvl(td.qtunidadmedidabasepicking,0)),
                sum(nvl(td.qtpiezaspicking,0))
           into v_qtbase,
                v_qtpiezas
           from tblslvtarea    ta,
                tblslvtareadet td
          where ta.idtarea = td.idtarea
            and ta.cdtipo in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante)
            and ta.idconsolidadom = p_idpedido
            and td.cdarticulo = p_cdarticulo
            and td.icdesasignar = 0 ;
       --valida pesables 0 contable 1 pesable
        if p_tipo_art = 0 then
           return nvl(v_qtbase,0);
         else
           return nvl(v_qtpiezas,0);
        end if;
         exception
           when others then
             return 0;
     ...
     if p_TipoTarea in( c_TareaConsolidadoPedido,c_ReporteFaltantePedido) then
       begin
         v_tareareporte:=c_TareaConsolidadoPedido;
         select sum(nvl(td.qtunidadmedidabasepicking,0)),
                sum(nvl(td.qtpiezaspicking,0))
           into v_qtbase,
                v_qtpiezas
     ...
     --TipoTarea 40,44 Faltantes Consolidado pedido
     if p_TipoTarea in(c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante,c_ReporteFaltaConsoFaltante) then

       v_tareareporte:=p_TipoTarea;
       if p_TipoTarea = c_ReporteFaltaConsoFaltante then
          v_tareareporte:= c_TareaConsolidaPedidoFaltante;
       end if;
        ...
        begin
         select sum(nvl(td.qtunidadmedidabasepicking,0)),
           ...
           from tblslvtarea    ta,
                tblslvtareadet td
          where ta.idtarea = td.idtarea
            and ta.cdtipo in(c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante,c_ReporteFaltaConsoFaltante)
            and ta.idpedfaltante = p_idpedido
            and td.cdarticulo = p_cdarticulo
            and td.icdesasignar = 0;
         --valida pesables
         if p_tipo_art = 0 then
    ...
    if p_TipoTarea in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
       begin
         v_tareareporte:=c_TareaConsolidadoComi;
         select sum(nvl(td.qtunidadmedidabasepicking,0)),
                sum(nvl(td.qtpiezaspicking,0))
           into v_qtbase,
                v_qtpiezas
           from tblslvtarea    ta,
                tblslvtareadet td
          where ta.idtarea = td.idtarea
            and ta.cdtipo in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante)
  ...
  /**************************************************************************************************
  * %v 30/11/2020 - ChM  función que devuelve 1 si el pedido tiene todas las tareas finalizadas
                         0 si tiene pendiente tareas por finalizar
  * %v 30/11/2020 - ChM. Versión Inicial
  ***************************************************************************************************/

   FUNCTION ContarTareasFinalizadas(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,
                                    p_TipoTarea     tblslvtipotarea.cdtipo%type)
                                    return integer IS
       v_conttarea    integer:=0;
       v_artSinAsig   integer:=0;
    BEGIN
        --TipoTarea 25 Consolidado pedido
        if p_TipoTarea in( c_TareaConsolidadoPedido,c_ReporteFaltantePedido) then
             --select para validar que el pedido no tenga tareas sin finalizar
             --cuento todas las tareas no finalizadas del CP
               select count (ta.idtarea)
                 into v_conttarea
                 from tblslvtarea ta
                where ta.idconsolidadopedido = to_number(p_Idconsolidado)
                  and ta.cdestado <> C_FinalizaTareaConsolidaPedido;
         end if;

         if nvl(v_conttarea,0) = 0 then
             if p_TipoTarea in( c_TareaConsolidadoPedido,c_ReporteFaltantePedido) then
               v_artSinAsig:= PKG_SLV_Consolidados.SinAsigConsolidadoPedido(p_idconsolidado);
             end if;
             if nvl(v_artSinAsig,0)=0 then
                 return 1;
             else
                 return 0;
             end if;
    ...
    BEGIN
      ...
      OPEN p_Cursor FOR
            select A.Sector,
                   ...
                   PKG_SLV_Articulo.SetFormatoArticulosCod(A.COD,
                   decode(A.basepza,0,(A.base-A.picking),(A.basepza-A.pickingpza))) Diferencia,
              ...
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                           ...
                           PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoM,p_TipoTarea, art.cdarticulo,0) picking,
                           PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoM,p_TipoTarea, art.cdarticulo,1) pickingpza,
               ...
               where --valida mostrar solo los faltantes cuando es tipo consolidado faltantes
                     case
                     --verifica si es pesable
                      when A.basepza<>0
                       and p_TipoTarea = c_TareaConsolidaMultiFaltante
                       and (A.basepza-A.pickingpza <> 0) then 1
                      --verifica los no pesable
                      when A.basepza = 0
                       and p_TipoTarea = c_TareaConsolidaMultiFaltante
                       and (A.base-A.picking <> 0)  then 1
  ...
  *****************************************************************************************************/
  PROCEDURE GetArtPanelConsolidaPedido(p_idConsolidadoPedido  IN  Tblslvconsolidadopedido.Idconsolidadopedido%type,
   ...
   V_tieneFaltante                     integer:=TieneConsoFaltante(p_idConsolidadoPedido);
      ...
      OPEN p_Cursor FOR
            select A.Sector,
              ...
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                           ...
                           det.qtpiezas basepza,
                           --verifica si es parte de un consofaltante recupera de tblslvconsolidadopedidodet
                           --sino recupera de tareas
                           case
                             when V_tieneFaltante = 0 then
                                  PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoPedido,p_TipoTarea, art.cdarticulo,0)
                                  else
                                  det.qtunidadmedidabasepicking
                            end picking,
                           case
                             when V_tieneFaltante = 0 then
                                  PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoPedido,p_TipoTarea, art.cdarticulo,1)
                             else
                                  det.qtpiezaspicking
                           end pickingpza,
                           PKG_SLV_Articulo.GetStockArticulos(art.cdarticulo) STOCK,
              ...
              where --valida mostrar solo los faltantes cuando es tipo consolidado faltantes
                     ...
                     --verifica si es pesable
                      when A.basepza<>0
                       ...
                       and (A.basepza-nvl(A.pickingpza,0) <> 0) then 1
                      --verifica los no pesable
                      when A.basepza = 0
                       and p_TipoTarea = c_ReporteFaltantePedido
                       and (A.base-nvl(A.picking,0) <> 0)  then 1
      ...
      OPEN p_Cursor FOR
            select A.Sector,
              ...
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                           ...
                           det.qtunidadmedidabase base,
                           det.qtpiezas basepza,
                           PKG_SLV_Consolidados.PickingTarea(p_idPedFaltante,p_TipoTarea, art.cdarticulo,0) picking,
                           PKG_SLV_Consolidados.PickingTarea(p_idPedFaltante,p_TipoTarea, art.cdarticulo,1) pickingpza,
                           PKG_SLV_Articulo.GetStockArticulos(art.cdarticulo) STOCK,
                           posapp.n_pkg_vitalpos_materiales.GetUxB(art.cdarticulo) UXB,
                           PKG_SLV_Articulo.GetUbicacionArticulos(ART.cdarticulo) UBICACION
                      from tblslvpedfaltante pf,
               ...
               where --valida mostrar solo los faltantes cuando es tipo consolidado faltantes
                     ...
                     --verifica si es pesable
                      ...
                      when A.basepza = 0
                       and p_TipoTarea in (c_ReporteFaltaConsoFaltante,c_TareaFaltanteConsolFaltante)
                       and (A.base-A.pickingpza <> 0)  then 1
    ...
    BEGIN
      ...
      OPEN p_Cursor FOR
            select A.Sector,
                   ...
                   A.uxb,
                   A.Ubicacion,
                   PKG_SLV_Consolidados.TareaConsolidadoArt(p_idConsolidadoComi,A.COD,p_TipoTarea) Tarea
              from (Select gs.cdgrupo||' - ' ||gs.dsgruposector||' ('||sec.dssector || ')' Sector,
                           art.cdarticulo || '- ' || des.vldescripcion Articulo,
                           art.cdarticulo COD,
                           det.qtunidadmedidabase base,
                           det.qtpiezas basepza,
                           PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoComi,p_TipoTarea, art.cdarticulo,0) picking,
                           PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoComi,p_TipoTarea, art.cdarticulo,1) pickingpza,
                           PKG_SLV_Articulo.GetStockArticulos(art.cdarticulo) STOCK,
                           posapp.n_pkg_vitalpos_materiales.GetUxB(art.cdarticulo) UXB,
                           PKG_SLV_Articulo.GetUbicacionArticulos(ART.cdarticulo) UBICACION
  ...
  *****************************************************************************************************/
  ...
  BEGIN
          ...
          if PKG_SLV_Consolidados.ConsolidadoPedidoFaltante(p_idConsolidado,c_TareaConsolidadoPedido,0)=1 and
  ...
  * %v 09/07/2020 - LM   Permite finalizar comi sin picking. afecta a calculo de faltantes si los dejan abiertos
  *****************************************************************************************************/
          ...
          distinct ta.cdmodoingreso
             ...
             where ta.dtinsert = (select max(ta2.dtinsert)
                                    from tblslvtarea ta2,
                                         tblslvtareadet td
                                   where ta2.idtarea = td.idtarea
                                     and td.cdarticulo = p_cdArticulo
                                     and ta2.idconsolidadom = v_idconsolidadom
                                     and td.icdesasignar = 0 )
...
/**************************************************************************************************
              ...
              where pf.idpedfaltante = detf.idpedfaltante
                ...
                and detf.cdarticulo not in(select td.cdarticulo
                                          ...
                                          --   and ta.idpersona= pf.idpersona  LM
                                             ...
                                             and ta.cdestado not in (C_FinalizadoTareaFaltConFalt, C_FinalizaTareaFaltaConsoliPed)
                                             and td.icdesasignar = 0 );
      if v_cont <> 0 then
         return 1;
      end if;
              ...
              where cm.idconsolidadocomi = det.idconsolidadocomi
                ...
                and det.cdarticulo not in(select td.cdarticulo
                                           ...
                                           where ta.idtarea=td.idtarea
                                            ...
                                            -- and ta.idpersona= cm.idpersona  LM
  ...
  * %v 15/09/2020 - LM  quito el filtro de persona que asigna contra tarea
  * %v 30/09/2020 - ChM Agrego validación de tarea desasignada and td.icdesasignar = 0
  * %v 15/07/2021 - LM  permite buscar varias vueltas de faltantes
  ***************************************************************************************************/
  FUNCTION SinAsigConsolidadoComiFaltante(p_idconsolidadoComi   tblslvconsolidadocomi.idconsolidadocomi%type)
                           return integer is
  ...
  * %v 10/03/2020 - ChM. Versión Inicial
  * %v 14/09/2020 - ChM  agrego bandera para buscar faltantes en la tarea o en el picking de la tabla
  *                      Así: P_icband 0 de la tabla 1 de la tarea
  *                      además agrego tipo tarea para usar la función pickingTarea
  ***************************************************************************************************/
  FUNCTION ConsolidadoMFaltante(p_idconsolidadom   tblslvconsolidadom.idconsolidadom%type,
                                p_TipoTarea        tblslvtipotarea.cdtipo%type,
                                P_icband           integer default 0)
        ...
        Select count(m.idconsolidadom)
          ...
          into v_cont
              ...
              where m.idconsolidadom = det.idconsolidadom
                ...
                --valida que exista diferencia entre picking y unidad base, estos son los faltantes
                and case
                    --verifica si es pesable 0 contable 1 pesable
                     when det.qtpiezas<>0
                      and det.qtpiezas-PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoM,p_TipoTarea, det.cdarticulo,1) <> 0 then 1
                     --verifica los no pesable
                     when det.qtpiezas = 0
                      and det.qtunidadmedidabase-PKG_SLV_Consolidados.PickingTarea(p_idConsolidadoM,p_TipoTarea, det.cdarticulo,0) <> 0 then 1
                    end = 1;
          if v_cont <> 0 then
           return 1;
        end if;
          ...
          into v_cont
              ...
              where p.idconsolidadopedido = det.idconsolidadopedido
                ...
                and case
                    --verifica si es pesable  0 contable 1 pesable
                     when det.qtpiezas<>0
                      and det.qtpiezas-PKG_SLV_Consolidados.PickingTarea(p_idconsolidadopedido ,p_TipoTarea, det.cdarticulo,1) <> 0 then 1
                     --verifica los no pesable
                     when det.qtpiezas = 0
                      and det.qtunidadesmedidabase-PKG_SLV_Consolidados.PickingTarea(p_idconsolidadopedido ,p_TipoTarea, det.cdarticulo,0) <> 0 then 1
                    end = 1;
          if v_cont <> 0 then
             return 1;
          end if;
          ...
          into v_cont
              ...
              where cm.idconsolidadocomi = det.idconsolidadocomi
                ...
                and case
                    --verifica si es pesable 0 contable 1 pesable
                     when det.qtpiezas<>0
                      and det.qtpiezas-PKG_SLV_Consolidados.PickingTarea(p_idconsolidadoComi ,p_TipoTarea, det.cdarticulo,1) <> 0 then 1
                     --verifica los no pesable
                     when det.qtpiezas = 0
                      and det.qtunidadmedidabase-PKG_SLV_Consolidados.PickingTarea(p_idconsolidadoComi ,p_TipoTarea, det.cdarticulo,0) <> 0 then 1
                    end = 1;
        if v_cont <> 0 then
           return 1;
        end if;
  ...
  ***************************************************************************************************/
          ...
          into v_cont
              ...
              where pf.idpedfaltante = det.idpedfaltante
                ...
                and case
                    --verifica si es pesable
                     when det.qtpiezas<>0
                      and det.qtpiezas-PKG_SLV_Consolidados.PickingTarea(p_idpedfaltante ,p_TipoTarea, det.cdarticulo,1) <> 0 then 1
                     --verifica los no pesable
                     when det.qtpiezas = 0
                      and det.qtunidadmedidabase-PKG_SLV_Consolidados.PickingTarea(p_idpedfaltante ,p_TipoTarea, det.cdarticulo,0) <> 0 then 1
                    end = 1;
          if v_cont <> 0 then
             return 1;
          end if;
  ...
  /**************************************************************************************************
  * %v 08/07/2020 - ChM  devuelve 1 si el consolidado pedido es parte de un ped faltante
  ...
  * %v 15/07/2021 - LM   se quita la tabla posconformado, no deberia estar
  ***************************************************************************************************/
...
/**************************************************************************************************
  * %v 11/03/2021 - LM  devuelve '-' si no tiene art de selectivos, 0 si aun no se creo el bolsin o el
  *                     nro del bolsin si ya estuviera creado
  ***************************************************************************************************/

   FUNCTION TieneBolsin(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,p_tipo tblslvtarea.cdtipo%type)
                              return varchar2 IS
        V_tiene     varchar2(10);
        v_tieneSensible integer:=0;
        v_idTareaAsignada  integer:=0;
    BEGIN
      v_tiene:='-';


     if p_tipo in (c_TareaConsolidadoMulti,c_TareaConsolidaMultiFaltante) then
      v_tiene:='-';
     end if;
     --TipoTarea 25 Consolidado pedido
     if p_tipo = c_TareaConsolidadoPedido then
           select count (cpd.idconsolidadopedidodet)
            into V_tieneSensible
            from tblslvconsolidadopedidodet cpd
           where cpd.idconsolidadopedido= p_idConsolidado
           and cpd.idgrupo_sector in (30,31);
           if v_tieneSensible>0 then
             v_tiene:='0';
              BEGIN
                  select distinct t.idtarea  into v_idTareaAsignada
                  from slvapp.tblslvtarea t , slvapp.tblslvtareadet td
                  where t.idconsolidadopedido=p_idconsolidado
                  and t.idtarea=td.idtarea
                  and td.idgrupo_sector in (30,31)
                  and rownum = 1 ;

                  if v_idTareaAsignada>0 then
                    BEGIN
                      select to_char(l.icllavero) into v_tiene
                      from posapp.llaverfcespec l
                      where l.idtarea=v_idTareaAsignada;
                    EXCEPTION
                      WHEN OTHERS THEN
                        v_tiene:='0';
                        return v_tiene;
     ...
     --TipoTarea 40,44 Faltantes Consolidado pedido
     if p_tipo in(c_TareaConsolidaPedidoFaltante,c_TareaFaltanteConsolFaltante) then
       v_tiene:='-';
       return v_tiene;
     end if;
    --TipoTarea 50,60 ConsolidadoComi y ConsolidadoComiFaltante
    if p_tipo in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
          select count (cpd.idconsolidadocomidet)
            into V_tieneSensible
            from slvapp.tblslvconsolidadocomidet cpd
           where cpd.idconsolidadocomi= p_idConsolidado
           and cpd.idgrupo_sector in (30,31);
           if v_tieneSensible>0 then
             v_tiene:='0';
              BEGIN
                  select distinct t.idtarea  into v_idTareaAsignada
                  from slvapp.tblslvtarea t , slvapp.tblslvtareadet td
                  where t.idconsolidadocomi=p_idconsolidado
                  and t.idtarea=td.idtarea
                  and td.idgrupo_sector in (30,31)
                  and rownum = 1 ;
 ...
 /**************************************************************************************************
  * %v 02/10/2020 - ChM  Procedimiento para devolver la ultima tarea a la que pertenece un
  *                       articulo de un consolidado por ahora solo para comisionista
  * %v 02/10/2020 - ChM. Versión Inicial
  ***************************************************************************************************/
   ...
   FUNCTION TareaConsolidadoArt(p_idconsolidado tblslvconsolidadopedido.idconsolidadopedido%type,
                                ...
                                p_TipoTarea     tblslvtipotarea.cdtipo%type)
                              return varchar2 IS
        V_tarea     varchar2(15):=0;
    BEGIN
        --TipoTarea 50,60 ConsolidadoComi y ConsolidadoComiFaltante
        if p_TipoTarea in (c_TareaConsolidadoComi,c_TareaConsolidadoComiFaltante) then
            select
          distinct ta.cdtipo||lpad(ta.idtarea,8,'0')
              into V_tarea
              from tblslvtarea ta,
                   tblslvtareadet td
             where ta.dtinsert = (select max(ta2.dtinsert)
                                   ...
                                   where ta2.idtarea = td.idtarea
                                     and td.cdarticulo = p_cdArticulo
                                     and ta2.idconsolidadocomi = p_idconsolidado
                                     and td.icdesasignar = 0 )
               and ta.idtarea = td.idtarea
               and td.cdarticulo = p_cdArticulo
