...
SELECT A.PEDIDO,
          ...
          decode(A.id_canal,'CO',
                (select distinct TRIM(NVL (e.dsrazonsocial, e.dsnombrefantasia))||' ('||TRIM(e.cdcuit)||')' razonsocial
                   from tblslvconsolidadopedido cp,tblslvconsolidadopedidorel prel ,entidades e, pedidos p
                  where cp.idconsolidadocomi=A.pedido
                    and cp.cdsucursal =A.cdsucursal
                    and cp.idconsolidadopedido = prel.idconsolidadopedido
                    and prel.idpedido = p.idpedido
                    and prel.cdsucursal = cp.cdsucursal
                    and p.idcomisionista=e.identidad),'-') NOMCOMISIONISTA,
          ...
          TO_CHAR (A.dtpedidoentregado, 'yyyymmdd') AS FECHAENTREGADO,
          TO_CHAR (A.dtpedidoentregado, 'HH24:MI:SS') AS HORAENTREGADO
     FROM (SELECT NVL2 (cp.idconsolidadocomi,
                  ...
                  (SELECT cm.dtinsert
                    ...
                    WHERE cm.idconsolidadom = cp.idconsolidadom
                     ...
                     (SELECT MAX (lp.dtmodif)
                     FROM tbllogestadopedidos lp
                    WHERE lp.icestadosistema = 9  --c_pedi_entregado 9;
                      AND lp.idpedido = pe.idpedido)
                     dtpedidoentregado
