...
select ta.idtarea,
              ...
              ta.dtfin,
              td.cdarticulo,
              TO_CHAR (td.dtupdate, 'yyyymmdd') AS FECHAFIN_PICK,
              TO_CHAR (td.dtupdate, 'HH24:MI:SS') AS HORAFIN_PICK,
              td.dtupdate,
              --precio promedio
              case
                when ta.idconsolidadocomi is not null then
                     (select round(avg(dp.ampreciounitario),2)
                       ...
                       where dp.icresppromo <> 1
                         ...
                         and prel.cdsucursal = ta.cdsucursal
                       ...
                       where dp.icresppromo <> 1
                         ...
                         and prel.idconsolidadopedido in (select cp2.idconsolidadopedido
                                                           ...
                                                           where cp2.idconsolidadopedido = frel.idconsolidadopedido
                                                             ...
                                                             and cp2.cdsucursal = ta.cdsucursal
                                                             and cp2.cdsucursal = frel.cdsucursal))    */
               else 0
              end PRECIO_PROMEDIO,
              -- cambiar este paquete el AC
              n_pkg_vitalpos_materiales.GetUxB(td.cdarticulo) VLUxB,
