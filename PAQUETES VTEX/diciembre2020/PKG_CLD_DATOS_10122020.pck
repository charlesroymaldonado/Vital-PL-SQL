CREATE OR REPLACE PACKAGE PKG_CLD_DATOS is
  ...
  TYPE PRODUCT IS RECORD (
        ...
        isactive      INTEGER:=1,
        icnuevo       INTEGER:=1,
        dtinsert      DATE:=SYSDATE,
        dtupdate      DATE,
        factor        INTEGER:=1,
        uxb           INTEGER:=1,
        observacion   VARCHAR2(4000),
        icprocesado   INTEGER:=0,
        dtprocesado   DATE);
  ...
  TYPE SKU IS RECORD (
        ...
        measurementunit VARCHAR2(20):='UN',
        dtupdate        DATE,
        icprocesado     INTEGER:=0,
        observacion     VARCHAR2(4000),
        dtinsert        DATE ,
  ...
  FUNCTION GETImpArt (p_cdarticulo articulos.cdarticulo%type,
...
/**************************************************************************************************
* devuelve el precio con impuesto del articulo que recibe
* %v 10/12/2020 - ChM 
***************************************************************************************************/
  FUNCTION GETImpArt (p_cdarticulo articulos.cdarticulo%type,
                      p_cdsucursal sucursales.cdsucursal%type,
...
--Buscar el IVA del artículo
   ...
   v_precioConIva := p_precio*(1+(v_PorcIva/100));
...
BEGIN
  ...
  for r_product in ( select distinct to_number(ar.cdarticulo) productID, 
    ...
    if r_product.vtaxunidad = 'BTO' and r_product.uxb <> 1 then
       r_product.factor:=null;
    end if;
    ...
    (cdalmacen,cdarticulo, cdsucursal, qtstock)
      ...
      ventas as (
         ...
         select s.cdalmacen,
                ...
                s.cdsucursal,
                case
                when (s.qtstock / n_pkg_vitalpos_materiales.getuxb(s.cdarticulo)) >= nvl (v.cantbtos, '0')
                then
                   s.qtstock
                else
                   0
...
END CargarStock;
  ...
  for P in         
    ...
    loop
       ...
       where  rownum = 1;
       v_priceOF:=pkg_cld_datos.GETIMPART(p.cdarticulo,p.cdsucursal,v_priceOF);
      exception
        when others then
         v_priceOF:=null;
         v_dtfromof:=null;
      ...
      begin
       ...
       where  rownum = 1;
        v_pricepl:=pkg_cld_datos.GETIMPART(p.cdarticulo,p.cdsucursal,v_pricepl);
      exception 
        when others then
          v_pricepl:=null;   
      end;
      ...
      using ( --lista los articulos(SKU) en VTEX con precio de lista agregados en la fecha del parametro
              select p.cdarticulo,
                     ...
                     p.id_canal,
                     p.amprecio,
                     pkg_cld_datos.GETIMPART(p.cdarticulo,p.cdsucursal,p.amprecio) impprecio
                from tblprecio p,
                     vtexsku   vs 
      ...
      when not matched then -- altas  de solo ofertas no debería existir nunca   
          ...
          values (lpc.cdsucursal,
                  ...
                  lpc.cdarticulo,                                
                  lpc.id_canal,
                  lpc.impprecio, --sino existe en VTEXPRICE se carga PL=OF
                  lpc.impprecio,
                  lpc.dtvigenciadesde,
                  lpc.dtvigenciahasta,
                  SYSDATE,
