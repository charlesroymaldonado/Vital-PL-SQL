...
PROCEDURE GetCat(Cur_Out Out Cursor_Type);
  ...
  PROCEDURE GetPromotionSKU (p_id_promo_pos In vtexpromotionsku.id_promo_pos%type,                         
                             Cur_Out        Out Cursor_Type);                           
...
create or replace package body PKG_GETDATOS_VTEX is
  ...
  ************************************************************************************************/
  ...
  BEGIN
    ...
    OPEN cur_out FOR
            SELECT DISTINCT
             ...
             --(ojo FALTA la conversión si está en BTO vp.unidadcond) BULTOS BUSCAR MENOR MENOR UXB
             DECODE(vp.type,1,null,'"quantity:"' || vp.valorcond ||' "percent:"'||vp.valoracc) percentualDiscountValueList1,
             ...
             (select distinct LISTAGG(vs2.id_canal_vtex, ', ') WITHIN GROUP (ORDER BY vs2.id_canal_vtex) 
              ...
              where vp2.id_promo_pos=vp.id_promo_pos
                ...
                vp.valorcond minimumQuantityBuyTogether,
                --Para tipo 7 = null Para tipo 1= los que van gratis LA RESTA ENTRE VALOR COND-VALOR ACC
                DECODE(vp.type,1,vp.valorcond-vp.valoracc,null) quantityToAffectBuyTogether,
  ...
  /**************************************************************************************************
* lista todos los SKU de una IDPROMO que recibe como parametro
* %v 30/12/2020 - ChM
***************************************************************************************************/
PROCEDURE GetPromotionSKU (p_id_promo_pos In  vtexpromotionsku.id_promo_pos%type,                         
                           Cur_Out        Out Cursor_Type) IS 
  v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.GetPromotionSKU';                           
  
BEGIN
 OPEN cur_out FOR
            select vps.skuid
              from Vtexpromotionsku vps
             where vps.id_promo_pos=p_id_promo_pos;

EXCEPTION
  WHEN OTHERS THEN
      pkg_log_general.write(1,
                            'Modulo: ' || v_Modulo || '  Error: ' ||
                            SQLERRM||' promocion id: '|| p_id_promo_pos);
      return;
  ...
  BEGIN
     ...
     IF (p_refId(1) IS NOT NULL and LENGTH(TRIM(p_refId(1)))>1) THEN
       FOR i IN 1 .. p_refId.Count LOOP
          ...
          v_icprocesado:=to_number(substr(p_refId(i),74,1));
          v_cdsucursal:=rpad((trim(substr(p_refId(i),76,4))),8,' ');
          v_id_canal:=upper(trim(substr(p_refId(i),81,2)));
          v_observacion:= substr(p_refId(i),81,3900);
         --actualiza a procesado el articulo del arreglo 
         begin
