CREATE OR REPLACE PACKAGE PKG_CLD_DATOS_CANAL is
 ...
 FUNCTION GETMULTIPLICADOR (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE,
  ...
  FUNCTION MINSKU (p_id_promo TBLPROMO.ID_PROMO%type) RETURN integer;
...
end PKG_CLD_DATOS_CANAL;
/
CREATE OR REPLACE PACKAGE BODY PKG_CLD_DATOS_CANAL is
   ...
   CargarTablaproduct();    
        ...
        insert into vtexproduct
          select tvp.productid,
                 ...
                 tvp.dtprocesado,
                 tvp.variedad,
                 tvp.id_canal From Table(Pipeproducts) tvp;
  commit;
...
PROCEDURE CargarTablaSKU IS
  ...
  CURSOR c_sku IS
      select distinct
             ...
             PKG_CLD_DATOS_CANAL.GETMULTIPLICADOR(vp.refid) unitmultiplier,
             'UN' measurementunit,
             null dtupdate,
             0 icprocesado,
             null observacion,
...
PROCEDURE ins_upd_vtexprice ( p_cdarticulo       IN  articulos.cdarticulo%type,
  ...
  IS
    ...
    v_dtupdate     vtexprice.dtupdate%type;
...
END ins_upd_vtexprice;
  ...
  PROCEDURE RefrescarPreciosVTEX (p_Fecha IN tblprecio.dtvigenciadesde%type) IS
      ...
      select p.cdarticulo,
             ...
             p.dtmodificacion,
             p.amprecio,
             PKG_CLD_DATOS_CANAL.GETPrecioconIVA (p.cdarticulo,p.cdsucursal,p.amprecio) precioconiva
        from tblprecio p,
             vtexsku   vs
           ...
           insert into VTEXCOLLECTIONSKU
                      ...
                      select colle.collectionid,
                       ...
                       where art.cdarticulo = vsk.refid
                         ...
                         --elimino cnal para cargar la tapa dos veces una por cada canal VE y CO
           ...
           insert into VTEXCOLLECTIONSKU
                      ...
                      select distinct
                       ...
                       where pre.refid = vsk.refid
                         ...
                         and case
                             when colle.cdsucursal='9999    ' AND PKG_CLD_DATOS_CANAL.RevisaOfertaPromo(pre.refid,pre.id_canal,P_FECHA,'OF') = 1 then 1
                      ...
                      select distinct
                       ...
                       where vp.id_promo_pos = vps.id_promo_pos
                         ...
                         and vp.icprocesado = 1
                          --todas las sucursales si es para la 9999
                         and case
                             when colle.cdsucursal='9999    ' AND PKG_CLD_DATOS_CANAL.RevisaOfertaPromo(vps.refid,vp.id_canal,P_FECHA,'PR') = 1 then 1
...
PROCEDURE RefrescarPromos (p_Fecha IN tblprecio.dtvigenciadesde%type) IS
 ...
 CURSOR c_promo IS
         SELECT distinct
                ...
                PKG_CLD_DATOS_CANAL.LeyendasPromoCucarda(r.id_promo) cucarda,
                PKG_CLD_DATOS_CANAL.FNLeyendaPromoCorta(r.id_promo) leyenda,
                case
                  when PKG_CLD_DATOS_CANAL.cantSKU(R.ID_PROMO) >=100 then -2
                  when VP1.NOMBRE = 'Bulto' then
                       PKG_CLD_DATOS_CANAL.revisarmultipleUxB (R.ID_PROMO)
                  else
                       1
                   end UxB,
                MINSKU(R.ID_PROMO) MINUV                     
           FROM TBLPROMO                     R,
 ...
 cursor c_Clients is
      (select
       distinct
                ...
                null observacion,
                'CL'  AGENT,
                PKG_CLD_DATOS_CANAL.GetAgente(e.identidad,tc.cdsucursal) idagent,
                PKG_CLIENTE_CENTRAL.GetHabilitBebidaAlcoholica(e.identidad) ICALCOHOL,
                PKG_CLD_DATOS_CANAL.GetCanal(e.identidad,tc.cdsucursal) id_canal,
