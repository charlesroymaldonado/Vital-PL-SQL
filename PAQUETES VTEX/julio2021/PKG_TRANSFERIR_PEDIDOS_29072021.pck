...
FUNCTION GETMULTIPLICADOR (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE) RETURN INTEGER;
...
***************************************************************************************************/
  FUNCTION GETMULTIPLICADOR (P_CDARTICULO ARTICULOS.CDARTICULO%TYPE) RETURN INTEGER IS
...
BEGIN
  ...
  /*o	Texto del 1 + 40  el idCuenta  (cliente)
    ...
    o	Texto del 250 + 100  observaciones del pedido
  */
   ...
   if trim(substr(p_cabecera,82,10))='pickup' then
    ...
    IF (p_detalle(1) IS NOT NULL and LENGTH(TRIM(p_detalle(1)))>1) THEN
       FOR i IN 1 .. p_detalle.Count LOOP
          ...
          v_quantity:=v_quantity*GETMULTIPLICADOR(v_cdarticulo);
   ...
   v_dtentrega:=v_fechapedido+2;     
    ...
    v_observacion:= trim(substr(p_cabecera,250,100));    
 ...
 --inserto datos en documentos con tipo de comprobante PEDI
  ...
  values
  (p_pedidoid_vtex   , v_identidad     , v_idpersonaresponsable , null        , v_iddoctrx      , v_qtmateriales ,v_dsreferencia      ,
       ...
       FOR i IN 1 .. p_detalle.Count LOOP
        ...
        --  p_detalle(1):='0158777 4669                100                                           0';
          ...
          begin
            ...
            exception 
              when others then
               Setvtexorders (p_pedidoid_vtex,null,2,'Error al intentar recuperar promoción artículo: '||v_cdarticulo||'cdpromo '||v_idpromo_vtex);
               rollback;
               RETURN;                       
          end; 
          ...
          begin
            ...
            exception 
              when others then
                Setvtexorders (p_pedidoid_vtex,null,2,'Error al intentar recuperar descripción del artículo: '||v_cdarticulo);                           
               rollback;  
               RETURN;               
          end;
