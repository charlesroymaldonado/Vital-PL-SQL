...
create or replace package body PKG_GETDATOS_VTEX is
  ...
  BEGIN
    ...
    OPEN cur_out FOR
           SELECT DISTINCT
             ...
             --(ojo FALTA la conversión si está en BTO vp.unidadcond) BULTOS BUSCAR MENOR MENOR UXB [{1, 20}]
             DECODE(vp.type,1,null,'[{' ||vp.valorcond||', ' ||vp.valoracc||'}]') percentualDiscountValueList,
  ...
  ********************************************************************************************************/
  ...
  PROCEDURE SetPedidosVtex (p_pedidoid_vtex  IN  vtexpedidos.pedidoid_vtex%type) IS 

    v_modulo        varchar2(100) := 'PKG_GetDatos_VTEX.SetPedidosVtex';

  BEGIN
     insert into vtexpedidos 
                 (pedidoid_vtex,
                 idpedido_pos,
                 icprocesado,
                 dtprocesado,
