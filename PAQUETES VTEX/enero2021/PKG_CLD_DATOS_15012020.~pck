  ...
  CURSOR c_stock IS
          ...
          select aa.cdalmacen,
           ...
           where aa.cdarticulo = vs.refid
             ...
             and vse.icactivo = 1                  --solo sucursales activas
       ...
       BEGIN
          select *
           ...
           if(promo.name <> r_promo(i).nombre or promo.type <> r_promo(i).tipo or 
              ...
              promo.dscucarda <> r_promo(i).cucarda or promo.dsleyendacorta <> r_promo(i).leyenda) then
...
BEGIN
  null;
/*--Tomo los datos del temporal del pedido
  select *
    into r
    from tblcc_cabecera tc
   where tc.idtransaccion_cc = p_idtransaccion_cc;

  --calculo los totales del documento
    select sum(td.amlineasiniva)
      into v_amnetodocumento
      from tblcc_detalle td
     where td.idtransaccion_cc = r.idtransaccion_cc;

  \*if r.identidad <> r.identidadreal then
    select sum(td.amlineaconiva)
      into v_amdocumento
      from tblcc_detalle td
     where td.idtransaccion_cc = r.idtransaccion_cc;
    else
    v_amdocumento := v_amnetodocumento;
  end if;*\

 --Traigo el cuit del cliente
  select e.cdcuit
    into v_cdcuit
    from entidades e
   where e.identidad = r.identidadreal;

  if r.icconsumidorfinal = 1 then
    v_dsreferencia := '[' || trim(v_cdcuit) || ']'||p_dsreferencia;
  else
    v_dsreferencia := NULL;
  end if;
  ...
  VALUES
      (sys_guid()         , NULL                  , NULL                             , r.cdsucursal   , r.identidad      , 'PEDI'        ,
  ...
  --Traigo el nombre y apellido de la persona responsable
  select p.dsnombre || ' ' || p.dsapellido
    into v_dspersona
    from personas p
   where p.idpersona = r.idpersonaresponsable;

  --Averiguo la SituaciÃ³n de IVA
  if r.icconsumidorfinal = 1 then
    v_cdsituacioniva := '2';
  else
    v_cdsituacioniva := '1';
  ...
  --MPASSIOTTI - 15/05/2017 - Migracion CC - Se cambia valor por defecto icestadosistema, de 0 a -1
  --Inserto finalmente el registro cabecera en la tabla pedidos (uso el mismo id del de la transacciÃ³n de telemarketing)
  insert into pedidos
  ...
  (v_idpedido        , r.identidad     , r.idpersonaresponsable, v_dspersona, v_iddoctrx      , v_qtmateriales, null             ,
   ...
   r.idvendedor      , r.sqdireccion   , v_amnetodocumento     , null       , r.idcomisionista, r.id_canal    , v_idpedido       ,
  ...
  EXCEPTION
    WHEN OTHERS THEN
       n_pkg_vitalpos_log_general.write(1,
                            ...
                            SQLERRM);
      p_Ok:=0;
      p_error:='Error no es posible Insertar Pedido POS';
   	  ROLLBACK;
      RETURN;  
