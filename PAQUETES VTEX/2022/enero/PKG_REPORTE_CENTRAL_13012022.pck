...
SELECT su.cdsucursal,
                ...
                ad.idsobre,
                ta.dtinicioalivio dtcargaalivio,
   ...
   ******************************************************************************************************************/
   ...
   BEGIN
      ...
      SELECT re.cdregion,
               ...
               de.cdtipodireccion,
               de.sqdireccion,
               de.dscalle || ' ' || de.dsnumero || ' (' || TRIM(de.cdcodigopostal) || ') ' || lo.dslocalidad Direccion,
...
select distinct ec.dsestado, su.dssucursal Sucursal,
    ...
    v_idReporte := PKG_REPORTE_CENTRAL.SetSucursalesSeleccionadas(p_sucursales);
      ...
      open p_cur_out for
            SELECT   v.idpersona,
             ...
             and   (cv.dthasta = trunc(last_day(sysdate)) -- solo las relaciones vigentes
                    or cv.dthasta = (select max(dthasta) from clientesviajantesvendedores))
             and sr.cdsucursal = su.cdsucursal
             and sr.idreporte = v_idReporte
             group by v.idpersona,su.cdsucursal,su.dssucursal,v.cdlegajo,v.dsapellido || ', ' || v.dsnombre
             order by count(*) desc;
      PKG_REPORTE_CENTRAL.CleanSucursalesSeleccionadas(v_idReporte);
  ...
  ***************************************************************************************************/
  ...
  BEGIN
      ...
      open p_cur_out for
      with VitalDigital as (
                   select cu.identidad, ep.vlaplicacion, ep.mail, cu.nombrecuenta
                   from tblentidadaplicacion        ep, 
                        tblcuenta                   cu
                   where ep.vlaplicacion in('Vital Digital')
                   and   ep.icactivo=1
      ...
      TiendaOnline as (
       ...
       SELECT  e.cdcuit,
                      ...
                      nvl(aa.nombrecuenta,'---') Cuenta_VD,
                      nvl(aa.vlaplicacion,'---') aplicacion_VD,
                      nvl(aa.mail,'---') Mail_VD
                      from clientesviajantesvendedores cv,
                           entidades                   e,
                           personas                    v,
                           ...
                           TiendaOnline                tio
                   where cv.identidad = e.identidad
                   and   cv.idviajante = p_idpersona
                   and   cv.idviajante = v.idpersona
                   and   s.cdsucursal = cv.cdsucursal
                   and   e.identidad=aa.identidad(+)  
  ...
  * %v 10/12/2021 - Iaquilano - Cambia la lógica por movimientos desde el 1 del mes en lugar de saldos acumulados
  ***************************************************************************************************/
  PROCEDURE GetCuentaAnticipoCliente (p_sucursales IN  VARCHAR2,
                                p_fechahasta in  date,
                                p_cur_out    OUT cursor_type) IS
 ...
 select s.dssucursal,
   ...
   from (select *
           from (select e.cdcuit,
                        e.dsrazonsocial,
                        i.cdsucursal,
                        pkg_ingreso_central.GetDescIngreso(ci.cdconfingreso,
                                                           i.cdsucursal) tipo,
                        nvl(a.vlmultiplicador *
                            pkg_ingreso_central.GetImporteNoAplicado(i.idingreso,
                                                                     to_date(trunc(mc.dtmovimiento))),
                            0) monto,
                   ...
                   from tblingreso       i,
                        ...
                        tblcuenta        c,
                        entidades        e
                  where mc.dtmovimiento between
                        to_date('01/' || to_char(P_fechahasta, 'mm/yyyy'),
                                'dd/mm/yyyy') and to_date(P_fechahasta + 1)
                    and i.idingreso = mc.idingreso
                    and i.cdsucursal in
                        (select s.cdsucursal
                           from sucursales s, tbltmp_sucursales_reporte sr
                  ...
                  (select 1
                    ...
                    and not
                         (ci.cdforma = '4' /*PB*/
                         or (ci.cdmedio = '5' and ci.cdaccion in ('3', '2') /*Cheq.Rechazado*/
                         ))
                    and a.cdaccion = ci.cdaccion
         ...
         select *
           from (select e.cdcuit,
                   ...
                   from tblcobranza    c,
                        ...
                        tblcuenta      c,
                        entidades      e
                  where c.dtimputado between
                        to_date('01/' || to_char(P_fechahasta, 'mm/yyyy'),
                                'dd/mm/yyyy') and to_date(P_fechahasta + 1) --Fecha de aplicación
         ...
         select *
           from (select e.cdcuit,
                  ...
                  where c.dtimputado between
                    ...
                    and ci.cdconfingreso = i.cdconfingreso
                    and ci.cdmedio <> '9' --Prestamo Comisionista
                    and not (ci.cdforma = '4' /*PB*/
                         or (ci.cdmedio = '5' and
                         ci.cdaccion in ('3', '2', '6') /*Cheq.Rechazado*/
                         ))
                    and mc.idcuenta = c.idcuenta
                    and c.identidad = e.identidad)
          where monto <> 0
         ...
         select *
           from (select e.cdcuit,
                        ...
                        nvl(pkg_core_documento.GetDeudaDocumento(d.iddoctrx,
                                                                 to_date(trunc(dtdocumento)),
                                                                 1),
                            0) monto,
                        'NC que al final del día tienen saldo'
                   from documentos d, tblcuenta c, entidades e
                  where d.dtdocumento between
                        to_date('01/' || to_char(P_fechahasta, 'mm/yyyy'),
                                'dd/mm/yyyy') and to_date(P_fechahasta + 1) --Fecha de la NC
...
/*****************************************************************************************************************
  ...
  ******************************************************************************************************************/
  PROCEDURE GetReporteControlNC (p_fechaDesde    IN  DATE,
    ...
    v_Modulo VARCHAR2(100) := 'PKG_REPORTE.GetReporteControlNC';
    v_idReporte             VARCHAR2(40) := '';
  BEGIN
    v_idReporte := SetSucursalesSeleccionadas(p_sucursales);
    OPEN p_cur_out for
     ...
     SELECT     su.dssucursal,
          ...
          WHERE     d.cdcomprobante   LIKE       'NC%'
            ...
            AND   ( p_cf = 1 AND trim(e.identidad) in ( trim(Getvlparametro('IdCfReparto','General')),
                                                  trim(GetVlparametro('CdConsFinal','General')) )    or p_cf        is null or p_cf = 0 )
