...
--  Procedure RefrescarClientesApp;
  ...
  FUNCTION promoSKU (p_id_promo       TBLPROMO.ID_PROMO%type,
            p_id_promo_hija  VTEXPROMOTIONSKU.ID_PROMO_HIJA%type)  RETURN integer;
                        ...
                        update vtexpromotion vp
                         ...
                         where vp.id_promo_pos = r_promo(i).id_promo
                           ...
                           --borrar e insertar los SKU de la Promoción
                            if(promoSKU (r_promo(i).id_promo,0)=1)then
                        n_pkg_vitalpos_log_general.write(2, 'Modulo: '||v_modulo||'  Error: ' || SQLERRM
...
* %v 29/12/2020 - ChM
* %v 14/01/2022 - ChM ajusto actualizar solo promo padres en este proceso
***************************************************************************************************/
FUNCTION promoSKU (p_id_promo       TBLPROMO.ID_PROMO%type,
                   p_id_promo_hija  VTEXPROMOTIONSKU.ID_PROMO_HIJA%type)  RETURN integer
  IS
...
BEGIN
   delete vtexpromotionSKU vps where vps.id_promo_pos = p_id_promo and vps.id_promo_hija=p_id_promo_hija;
...
PROCEDURE promoMultipleUXB (p_Fecha IN tblprecio.dtvigenciadesde%type) IS
 ...
 CURSOR c_promo_cond IS
        --LISTO LAS PROMOCIONES HIJAS QUE SE CREAN O ACTULIZAN
        select distinct vp.*,(A.valorcond*A.UXB)/A.uv condnueva
          from (
                select s.*,               
                       p.cdsucursal,
                       p.id_canal,
 ...
 --busco solo padres para verificar si cambio algo de la cabecera              
 /*CURSOR c_promo_padre IS           
            select *             
              from vtexpromotion vp
   ...
   TYPE lv_vtexpromo is table of c_promo_padre%rowtype;
        r_promo_p      lv_vtexpromo;    */  
...
/*
     OPEN c_promo_padre;
     FETCH c_promo_padre  BULK COLLECT INTO r_promo_p;      --cargo el cursor en la tabla en memoria
     CLOSE c_promo_padre;*/
