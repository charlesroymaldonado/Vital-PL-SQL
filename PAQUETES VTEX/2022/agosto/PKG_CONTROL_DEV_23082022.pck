CREATE OR REPLACE PACKAGE PKG_CONTROL IS
  ...
  ***************************************************************************************************/
  ...
  PROCEDURE ControlarPROMOPOSVTEX (p_fecha      IN  tblpromo.vigencia_desde%type,  
...
END ControlarVTEXpromo;
 ...
 /************************************************************************************************************
  * Informar sobre las promocioens que no suben por multiple UxB o mas 100 skus o de tipo 1 para canales VE y CO
  * %v 21/12/2021 ChM: v1.0
  ************************************************************************************************************/
PROCEDURE ControlarPROMOPOSVTEX (p_fecha      IN  tblpromo.vigencia_desde%type,  
                                 p_cur_out    OUT cursor_type)
IS
   v_modulo               varchar2(100) := 'PKG_CONTROL.ControlarPROMOPOSVTEX';
...
BEGIN
    ...
    --promociones que no suben por multiple UxB o mas 100 skus
         ...
         distinct p.cdpromo,
                  p.name,
                  p.id_canal,
                  decode(p.uxb,-1,'Multiple UxB',-2,'Más de 100 Skus') error
        ...
        distinct p.cdpromo cd_promo,
                 p.nombre,
                 c.id_canal,
                 'Promoción tipo 1' error
            from tblpromo                 p,                   
                 tblpromo_canal           c                  
           where p.id_promo = c.id_promo
             and p.id_promo_tipo=1                              
             and c.id_canal in ('VE','CO')
             and p_fecha between p.vigencia_desde and p.vigencia_hasta
        order by id_canal
           ;    
EXCEPTION
  WHEN OTHERS THEN   
   n_pkg_vitalpos_log_general.write(2, 'Modulo: '||v_modulo||'  Error: ' || SQLERRM);
END ControlarPROMOPOSVTEX;
    ...
    OPEN p_cur_out FOR
      select count(*) as cant, 'VKP1' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
       ...
       where a.knumh = k.knumh
         and a.datab >= to_char(trunc(sysdate), 'yyyymmddhh24miss') 
