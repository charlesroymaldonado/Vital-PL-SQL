CREATE OR REPLACE PACKAGE PKG_CONTROL IS
  ...
  ***************************************************************************************************/
  ...
  PROCEDURE Control_Monitor_interfaces(p_cur_out OUT cursor_type);

  PROCEDURE Control_Log_interfaces(p_cur_out OUT cursor_type);

  PROCEDURE Control_Precios_SAP(p_cur_out OUT cursor_type);
PROCEDURE ControlVentasCAR(p_cdsucursal IN sucursales.cdsucursal%type,
                             p_fecha      IN date,
                             p_ok         OUT integer);
                             
PROCEDURE Control_Precios_SAP_POS(p_cur_out OUT cursor_type);  

PROCEDURE Control_Baja_Ofertas(p_Fecha      IN date,
...
/***************************************************************************************************
* function GetConsumidorFinalNR
* Dado una sucursal devuelve el idEntidad del CF No Residente desconocido
*
* %v 12/07/2022 - Iaquilano
***************************************************************************************************/
function GetConsumidorFinalNR(p_servidor in sucursales.servidor%type)
return entidades.identidad%type
is
   v_Result               entidades.identidad%type;
...
begin
   ...
   EXECUTE IMMEDIATE 'select GetVlparametro@' || p_servidor || '  (''CdCFNoResidente'', ''General'') as CF from dual ' INTO v_Result;

   return v_Result;

exception when others then
   return null;
end GetConsumidorFinalNR;
...
/***************************************************************************************************
* procedure ControlarCFNR
* Controla que la información del CF No residente desconocido sea consistente
*
* %v 12/07/2022 - Iaquilano
***************************************************************************************************/
procedure ControlarCFNR
...
BEGIN
   ...
   loop
      if pkg_replica_suc.GetActiva(v_RegTmp.cdSucursal) = 1 then
         ...
         v_idEntidad := GetConsumidorFinalNR(v_RegTmp.servidor);
         ...
         if v_amSaldo <> 0 then
            ...
            GrabarMensaje(sys_guid(), null, sysdate, 'CF No Residente de sucursal ' || upper(v_RegTmp.Servidor) || ' con saldo', v_dsMensaje, 0);
         end if;
    ...
    p_result:=0;
      select count(*)
       ...
       where v_fecha between vp.begindateutc and vp.enddateutc
         ...
         and vp.icrevisadopos is null
         --   valida o incluir solo promos con SKUs ChM 15/06/2022
...
END ControlarVTEXBrand;
  ...
  * %v 23/08/2022 ChM - Excluyo el control de colecciones vacias acordado do APW
  ************************************************************************************************************/
  ...
  --Alerta de colecciones vacías en VTEX
   /* for coll in(
    select vc.collectionid,
     ...
     from vtexcollection vc where VC.ID_CANAL<>'TA')
    loop
      if coll.cant=0 then
...
END ControlarVTEXOrders;
  ...
  * %v 15/04/2021 ChM: v1.0
  * &v 05/08/2022 ChM - agrego no validar clientes sin direcciones           
  ************************************************************************************************************/
...
BEGIN
  ...
  --ChM 14/04/2022 no se controla más en HELP los sin dirección
   /* for clients in
      (
        select vc.id_cuenta,
               (select count(*)
  ...
  --PASAR CORREO para resolver URGENTE el agente
 /* select count(*)
    into p_result
    from vtexclients c
   where c.icactive=1
     and c.icprocesado<>1
     and c.clientsid_vtex<>'1';
    if  p_result<>0 then
         return;
      end if;*/
   --clientes activos en vtex y INACTIVOS pos que no suben ACTUALIZACION por tener canal NO
   --BORRAR en MD y Luego en POS inactivos no son necesarios en la BD
  /* select count(*)
    into p_result
    from vtexclients c
   where c.icactive=0
...
END ControlarVTEXClients;
 ...
 /************************************************************************************************************
  * Retorna cursor con el semaforo de las interfaces SAP
  * %v 10/09/2021 - IAquilano
  ************************************************************************************************************/
  PROCEDURE Control_Monitor_interfaces(p_cur_out    OUT cursor_type) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.Control_Monitor_interfaces';

  BEGIN

 OPEN p_cur_out FOR
 select * from  tblmonitor_interfaces;

  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,'Modulo: ' || v_modulo || '  Error: ' ||SQLERRM);

  END Control_Monitor_interfaces;

 /************************************************************************************************************
  * Cursor para monitoreo de replicas de Precios desde SAP
  * %v  21/09/2021 - IAquilano
  ************************************************************************************************************/
  PROCEDURE Control_Precios_SAP(p_cur_out OUT cursor_type) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.Control_Precios_SAP';

  BEGIN

    OPEN p_cur_out FOR
      select count(*) as cant, 'VKP1' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
        from sapint.a071 a, sapint.konp k
       where a.knumh = k.knumh
         and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
         and a.kschl = 'VKP1'--Precio Lista
      union all
      select count(*)as cant, 'ZOF2' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
        from sapint.a071 a, sapint.konp k
       where a.knumh = k.knumh
         and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
         and a.kschl = 'ZOF2'--Precio Oferta
      union all
      select count(*)as cant, 'ZFAC' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
        from sapint.a071 a, sapint.konp k
       where a.knumh = k.knumh
         and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
         and a.kschl = 'ZFAC'--Factor
      union all
      select count(*)as cant, 'ZFA1' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
        from sapint.a071 a, sapint.konp k
       where a.knumh = k.knumh
         and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
         and a.kschl = 'ZFA1'--Recargo
      union all
      select count(*)as cant, 'ZKA2' as condicion, max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
        from sapint.a071 a, sapint.konp k
       where a.knumh = k.knumh
         and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
         and a.kschl = 'ZKA2'--Precio Acercamiento
      ;
  ...
  END Control_Precios_SAP;

  /************************************************************************************************************
  * Control de precios procesados SAP a TBLPRECIO
  * %v  07/04/2022 - IAquilano
  ************************************************************************************************************/
  PROCEDURE Control_Precios_SAP_POS(p_cur_out OUT cursor_type) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.Control_Precios_SAP_POS';

  BEGIN

    OPEN p_cur_out FOR
       select *
  from (with SAP as (select count(*) as cant,
                            'VKP1' as condicion,
                            max(to_date(decode(a.insert_ts, '0', '00010101000000', a.insert_ts), 'yyyymmddhh24miss')) as max_insert
                       from sapint.a071 a, sapint.konp k
                      where a.knumh = k.knumh
                        and to_date(a.datab,'yyyymmddhh24miss') >= trunc(sysdate)
                        and a.kschl = 'VKP1' --Precio Lista
                        and a.dt_procesado <> '0'--que no esten procesados
                     ), PO as (select count(*) as cant,
                                      'PRECIO LISTA' as condicion,
                                      max(tp.dtmodificacion) as max_insert
                                 from tblprecio tp
                                where tp.dtvigenciadesde >= trunc(sysdate)
                                  and tp.id_precio_tipo = 'PL')
         select case
                  when p.cant < s.cant then
                   '0'
                  else
                   '1'
                end as V_OK,
           ...
           from (with SAP as (select count(*) as cant,
                               ...
                               where a.knumh = k.knumh
                                 ...
                                 and a.dt_procesado <> '0'--que no esten procesados
                                               ...
                                               'PRECIO OFERTA' as condicion,
                                               max(tp.dtmodificacion) as max_insert
                                          from tblprecio tp
                                         where tp.dtvigenciadesde >= trunc(sysdate)
                                           and tp.id_precio_tipo = 'OF')
                  select case
                           when p.cant < s.cant then
                            '0'
                           else
                    ...
                    from (with SAP as (select count(*) as cant,
                                        ...
                                        where a.knumh = k.knumh
                                          ...
                                          and a.dt_procesado <> '0'--que no esten procesados
                                                        ...
                                                        'PRECIO ACERCAMIENTO' as condicion,
                                                        max(tp.dtmodificacion) as max_insert
                                                   from tblprecio tp
                                                  where tp.dtvigenciadesde >= trunc(sysdate)
                                                    and tp.id_precio_tipo = 'PA')
                           select case
                                    when p.cant < s.cant then
                                     '0'
                                    else
                                  ...
                                  s.condicion,
                                  p.max_insert
                             from po p, sap s --Precio Acercamiento
                           ) t2
  ...
  END Control_Precios_SAP_POS;
  /************************************************************************************************************
  * Retorna cursor con el semaforo de las interfaces SAP
  * %10/09/2021 - IAquilano
  ************************************************************************************************************/
  PROCEDURE Control_Log_interfaces(p_cur_out    OUT cursor_type) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.Control_Log_interfaces';

  BEGIN

 OPEN p_cur_out FOR
 select * from tbllog_interfaces;

  EXCEPTION
    WHEN OTHERS THEN
      n_pkg_vitalpos_log_general.write(2,'Modulo: ' || v_modulo || '  Error: ' ||SQLERRM);

  END Control_Log_interfaces;
  ...
  ************************************************************************************************************/
  ...
  Procedure Control_Replicas_CAR_BKP(p_cdsucursal IN sucursales.cdsucursal%TYPE,
                                 p_fecha      IN date,
  ...
  Begin
    ...
    if pkg_replica_suc.GetActiva(p_cdsucursal) = 1 then

      v_query := ('select decode(nvl(sum(cant), ''0''), 0, ''OK'', ''NO OK'')
      from (select count(*) as cant
              from cierressucursal@' || v_sucursal || ' tc
             where tc.dtcierre = ''' || p_fecha || '''
               and not exists
             (select 1
                      from cierressucursal tcc
                     where tcc.cdsucursal = tc.cdsucursal
                       and tcc.dtcierre = tc.dtcierre
                       and tcc.icestadocierre = tc.icestadocierre)
          union all
          select count(*) as cant
      from tblcobranza@' || v_sucursal || ' tc
     where tc.dtimputado BETWEEN ''' || p_fecha || ''' AND ''' ||
                 v_dthasta || '''
       and not exists
     (select 1 from tblcobranza tcc where tc.idcobranza = tcc.idcobranza)
    union all
    select count(*)
  from tblcuenta@' || v_sucursal || ' tc
  where dtalta between ''' || p_fecha || ''' AND ''' ||
                 v_dthasta || '''
  and not exists(select *
  from tblcuenta tcc where tc.idcuenta = tcc.idcuenta)
  union all
  select count(*)
  from entidades@' || v_sucursal || '  e
  where dtalta between ''' || p_fecha || ''' AND ''' || v_dthasta || '''
  and not exists(select 1 from entidades ee where ee.identidad = e.identidad )
  union all
      select count(*)
        from tblcheque@' || v_sucursal ||
                 ' tch
       where tch.idingreso in
             (select ti.idingreso
                from tblingreso@' || v_sucursal || ' ti
               where dtingreso BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || ''')
         and not exists
       (select 1 from tblcheque tcc where tcc.idingreso = tch.idingreso)
    union all
    select count(*)
      from TBLDIFERENCIACAJA@' || v_sucursal || ' tc
     where dtcierrecaja BETWEEN ''' || p_fecha || ''' AND ''' ||
                 v_dthasta || '''
       and not exists
     (select 1
              from TBLDIFERENCIACAJA tcc
             where tcc.iddiferenciacaja = tc.iddiferenciacaja)
    union all
    select count(*)
      from TBLDOCUMENTO_SALIDA@' || v_sucursal || ' ts
     where dtsalida BETWEEN ''' || p_fecha || ''' AND ''' ||
                 v_dthasta || '''
       and not exists
     (select 1
              from TBLDOCUMENTO_SALIDA tss
             where tss.idfacturasalida = ts.idfacturasalida)
    union all
      select count(*)
      from TBLELECTRONICO@' || v_sucursal || ' te
      where te.idingreso in (select ti.idingreso
                from tblingreso@' || v_sucursal || ' ti
               where dtingreso BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || ''')
      and not exists(select 1 from TBLELECTRONICO tee where te.idingreso = tee.idingreso)
    union all
      select count(*)
        from TBLMOVCUENTA@' || v_sucursal || ' tm
        where tm.dtmovimiento between ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
        and not exists(select 1 from tblmovcuenta tmm where tmm.idmovcuenta = tm.idmovcuenta)
    union all
      select count(*)
        from TBLDOCUMENTODEUDA@' || v_sucursal || ' tm
        where (dtestadoinicio between ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || ''' or dtestadofin between ''' ||
                 p_fecha || ''' AND ''' || v_dthasta || ''')
        and not exists(select 1 from TBLDOCUMENTODEUDA tmm where tmm.iddocumentodeuda = tm.iddocumentodeuda)
    union all
            select count(*) as cant
              from detallemovmateriales@' || v_sucursal ||
                 ' dd,
                   documentos@' || v_sucursal || ' d
             where d.idmovmateriales = dd.idmovmateriales
               and d.dtdocumento BETWEEN  ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1
            ...
            union all
            select count(*) as cant
              from movmateriales@' || v_sucursal ||
                 ' dd, documentos@' || v_sucursal || ' d
             where d.idmovmateriales = dd.idmovmateriales
               and d.dtdocumento BETWEEN  ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
            ...
            select count(*) as cant
             ...
             where d.dtdocumento BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
            ...
            select count(*) as cant
             ...
             where idmovmateriales in
                   (select idmovmateriales
                     ...
                     where dtdocumento between  ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || ''')
               and not exists
             (select 1
            ...
            union all
            select count(*) as cant
              from tblimpdocumento@' || v_sucursal ||
                 ' tip
             where idmovmateriales in
                   (select idmovmateriales
                      from documentos@' || v_sucursal || '
            ...
            select count(*) as cant
             ...
             where ts.dtlote BETWEEN  ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1
            ...
            select count(*) as cant
             ...
             where dtoperacion BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
            ...
            select count(*) as cant
             ...
             where dtestado BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1
            ...
            select count(*) as cant
             ...
             where dtmovimiento BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
            ...
            select count(*) as cant
             ...
             where tc.dttransaccion between ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1
            ...
            select count(*) as cant
             ...
             where dtlote BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1 from tblclsube tss where tss.idclsube = ts.idclsube)
            union all
            select count(*) as cant
              from tblcierrelote@' || v_sucursal ||
                 ' tcs
             where dtcierrelote BETWEEN ''' || p_fecha ||
                 ''' AND ''' || v_dthasta || '''
               and not exists
             (select 1
  ...
  ************************************************************************************************************/
  ...
  Begin
    ...
    if pkg_replica_suc.GetActiva(p_cdsucursal) = 1 then
      ...
      v_query := ('select decode(count(*),''0'', ''OK'', ''NO OK'')
                     from tblreplica@' || v_sucursal || ' T
                    where upper(vlnombretabla) in (''DIRECCIONESENTIDADES'',''DOCUMENTOS'',''ENTIDADES'',
                                                    ''TBLCHEQUE'',
                                                    ''TBLCIERRELOTESALON'',
                                                    ''TBLCOBRANZA'',
                                                    ''TBLDESTINOCAUDAL'',
                                                    ...
                                                    ''TBLDOCUMENTO_SALIDA'',
                                                    ''TBLELECTRONICO'',
                                                    ''TBLIMPDOCUMENTODETALLE'',
                                                    ''TBLMOVCAJA'',
                                                    ''TBLMOVCUENTA'',
                                                    ''TBLTARJETA'',
                                                    ''DETALLEMOVMATERIALES'',
                                                    ''TBLCLCONTRACARGO'',
                                                    ''TBLINTERDEPOSITO'',
                                                    ''TBLPOSNETBANCO'',
                                                    ''TBLPOSNET_TRANSMITIDO'',
                                                    ''TBLDOCUMENTODEUDA'',
                                                    ''MOVMATERIALES'',
                                                    ''TBLCIERRELOTE'',
                                                    ''TBLCLSUBE'',
                                                    ...
                                                    ''CIERRESSUCURSAL'',
                                                    ''TBLTESORO'',
                                                    ''TBLDOCUMENTO_CONTROL'',
                                                    ''TBLCLNOTADECREDITO'',
                                                    ''PEDIDOS'')
                    and cdestado = ''1  ''
                    and t.timestamp between ''' || p_fecha || ''' AND ''' || v_dthasta || '''
                    ');
  ...
  /************************************************************************************************************
  * Retorna estado de proceso de CAR por sucursal
  * %21/10/2021 - IAquilano
  ************************************************************************************************************/
  PROCEDURE ControlVentasCAR(p_cdsucursal IN sucursales.cdsucursal%type,
                             p_fecha      IN date,
                             p_ok         OUT integer) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.ControlVentasCAR';
  ...
  BEGIN
    ...
    Begin
      ...
      select 1
        into p_ok
        from sapint.tblcar_sucprocesadas tp
       where tp.cdsucursal = p_cdsucursal
         and tp.dtprocesada = p_fecha;
  ...
  END ControlVentasCAR;

/************************************************************************************************************
  * Retorna cursor con las ofertas vencidas por cantidad definida
  * %10/08/2022 - ChM
  ************************************************************************************************************/
  PROCEDURE Control_Baja_Ofertas(p_Fecha      IN date,
                                 p_cur_out    OUT cursor_type) IS

    v_modulo varchar2(100) := 'PKG_CONTROL.Control_Baja_Ofertas';
 ...
 OPEN p_cur_out FOR
            select o.cdsucursal,
                   ...
                   a.cdarticulo          articulo,
                   a.vldescripcion       descripcion,
                   o.id_canal,
                   o.qtunidades          Limite_unidades,
                   o.dtvigenciadesde     oferta_desde,
                   o.dtvigenciahasta     oferta_hasta,
                   o.qtunidades_vendidas,
                   o.dtprocesado         fecha_cierre_oferta
              from ofertasbajas o, descripcionesarticulos a, sucursales s
             where o.cdarticulo = a.cdarticulo
               and o.cdsucursal = s.cdsucursal
                  --solo vencidas para el dia del parametro
               and o.dtprocesado = p_Fecha
               and o.icprocesado = 1;
