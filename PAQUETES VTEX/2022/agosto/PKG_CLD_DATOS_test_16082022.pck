...
BEGIN
  ...
  -- if p_dtmodificacion > v_dtupdate then
         if p_id_precio_tipo='PL' and v_pricepl <> p_precio then
             update vtexprice vp
          ...
          if p_id_precio_tipo='OF'  then
            if v_dtfromof<>p_dtvigenciadesde or v_dttoof<>p_dtvigenciahasta or p_precio<>v_priceof then
                 update vtexprice vp
          ...
          if p_id_precio_tipo='PA'  then
            if v_dtfrompa<>p_dtvigenciadesde or v_dttopa<>p_dtvigenciahasta or p_precio<>v_pricepa then
                 update vtexprice vp
...
END verificaPABorrados;
  ...
  for P in
    ...
    loop
      ...
      begin
        select A.amprecioiva-- IA 02/06/2022
          into v_pricepl
          from (select pre.amprecioiva,-- IA 02/06/2022
                       pre.dtmodificacion
                  from tblprecio pre
                 where pre.cdsucursal = p.cdsucursal
                   and pre.id_canal = p.id_canal
    ...
    -- Carga o actualiza los precios lista de los artículos de vtex que se agregaron en la fecha del parámetro
     ...
     ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,r_precio(i).precioconiva,null,null,r_precio(i).dtmodificacion,'PL');
     END LOOP;
     ...
     FOR i IN 1 .. r_precio.COUNT LOOP
       ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,
                           r_precio(i).precioconiva,r_precio(i).dtvigenciadesde,r_precio(i).dtvigenciahasta,r_precio(i).dtmodificacion,'OF');
     END LOOP;
     ...
     CLOSE c_precio;
     FOR i IN 1 .. r_precio.COUNT LOOP
        ins_upd_vtexprice ( r_precio(i).cdarticulo,r_precio(i).cdsucursal,r_precio(i).id_canal,r_precio(i).precioconiva,r_precio(i).dtvigenciadesde,r_precio(i).dtvigenciahasta,r_precio(i).dtmodificacion,'PA');
     END LOOP;
