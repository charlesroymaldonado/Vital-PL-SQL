...
begin
   ...
   for v_regDetalle in (select *
                        ...
                        where d.idpedido = p_idPedido
                         ...
                         and case
                              when v_RegPedido.icorigen = 4 and d.icresppromo=0 then 1
                              when v_RegPedido.icorigen = 4 and d.icresppromo=1 then 0
                              when v_RegPedido.icorigen <> 4 then 1
                             end = 1     
                              )
   loop
     
      -- ChM los pedidos de VTEX no repetan promociones enteras                    
      if v_RegDetalle.Cdpromo is not null and v_RegPedido.icorigen <> 4 then --Es parte de una PROMOCION
