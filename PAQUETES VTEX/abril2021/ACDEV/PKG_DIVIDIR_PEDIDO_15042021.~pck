...
*
* %v 04/01/2016 - MarianoL
* %v 08/04/2021 - ChM ajsuto para dividir promociones solo para pedidos de icorigen VTEX
***************************************************************************************************/
procedure Dividir(p_idPedido in  pedidos.idpedido%type,
                  p_amLimite in  number,
                  p_ok       out integer,
...
begin
   ...
   for v_regDetalle in (select *
                        ...
                        where d.idpedido = p_idPedido
                         ...
                         and case
                              when nvl(v_RegPedido.icorigen,-1) = 4 and d.icresppromo=0 then 1
                              when nvl(v_RegPedido.icorigen,-1) = 4 and d.icresppromo=1 then 0
                              when nvl(v_RegPedido.icorigen,-1) <> 4 then 1
                             end = 1     
                              )
   loop
     
      -- ChM los pedidos de VTEX no repetan promociones enteras                    
      if v_RegDetalle.Cdpromo is not null and nvl(v_RegPedido.icorigen,-1) <> 4 then --Es parte de una PROMOCION
