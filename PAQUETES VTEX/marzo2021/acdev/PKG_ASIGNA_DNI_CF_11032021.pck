create or replace package PKG_ASIGNA_DNI_CF is
  ...
  **********************************************************************************************************/
  ...
  --tabla en memoria para la distribución del detalle pedido
   ...
   DISTRIB        PEDIDO_DIS;
   DISTRIB_PROMO  PEDIDO_DIS;
   
   FUNCTION AsignaDNICF(p_cdcuit          entidades.cdcuit%TYPE,
...
create or replace package body PKG_ASIGNA_DNI_CF is
  ...
  * %v 29/06/2020 - ChM crea la tabla temporal de los artículos disponibles de los pedidos a asignar DNI
  *****************************************************************************************************/
   FUNCTION TempDistrib(p_cdcuit          entidades.cdcuit%TYPE,
    ...
    CURSOR DET (p_cdcuit          entidades.cdcuit%TYPE,
          ...
          SELECT LISTAGG(dp.idpedido, ', ')
                 WITHIN GROUP (ORDER BY dp.idpedido) idpedido,
                 dp.cdarticulo,
                 NULL cdpromo,
        ...
        GROUP BY dp.cdarticulo, op.dsobservacion              
        ORDER BY 2;   
    
    CURSOR DET_PROMO (p_cdcuit          entidades.cdcuit%TYPE,
                      p_dtPedido        pedidos.dtaplicacion%TYPE,
                      p_cdTipoDireccion pedidos.cdtipodireccion%TYPE,
                      p_sqDireccion     pedidos.sqdireccion%TYPE)IS
          ...
          SELECT dp.idpedido idpedido,
                 dp.cdarticulo,
                 dp.cdpromo,
                 dp.qtunidadmedidabase qtbase,
                 dp.qtpiezas qtpiezas,
                 dp.ampreciounitario preciounitario, 
                 dp.vluxb,
                 op.dsobservacion,
                 0 bandera                                 
            FROM pedidos                 pe
    ...
    BEGIN
     ...
     CLOSE DET;
     OPEN DET(p_cdcuit,p_dtPedido,p_cdTipoDireccion,p_sqDireccion);
     FETCH DET_PROMO BULK COLLECT INTO DISTRIB_PROMO;      --cargo el cursor en la tabla en memoria
     CLOSE DET_PROMO;
     RETURN 1;
  EXCEPTION
      WHEN OTHERS THEN
  ...
  ************************************************************************************************************/
  FUNCTION  AsignaDNICF(p_cdcuit          entidades.cdcuit%TYPE,
    ...
    v_Modulo    VARCHAR2(100) := 'PKG_ASIGNA_DNI_CF.AsignaDNICF';
    v_dtDesde   date;
    v_dtHasta   date;
