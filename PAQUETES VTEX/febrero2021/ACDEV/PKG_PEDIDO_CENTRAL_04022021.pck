...
CREATE OR REPLACE PACKAGE BODY PKG_PEDIDO_CENTRAL IS
  ...
  * %v 04/02/2021 - ChM agrego error de no cambiar estado 19 
  ************************************************************************************************************/
  ...
  BEGIN
    ...
    FOR v_RegPed IN c_RegPed LOOP
      ...
      --Si el estado es 19 no permite el cambio de estado ChM
      IF v_RegPed.Icestadosistema = 19 THEN
        v_band19:=1;
        EXIT WHEN v_band19=1;
      END IF;
    ...
    if v_band19 = 1 then
       p_ok    := 0;
       p_error := '  Error: el estado 19 DNI CF no disponible. No se puede cambiar, agregue DNI de CF al cliente';
      end if;
    p_ok    := 1;
    p_error := '';
