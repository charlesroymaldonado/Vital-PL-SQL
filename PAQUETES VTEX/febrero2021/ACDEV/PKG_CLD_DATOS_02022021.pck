...
* %v 22/01/2021 - ChM
***************************************************************************************************/
PROCEDURE SetVtexPedidos (p_pedidoid_vtex    IN     vtexpedidos.pedidoid_vtex%type,
                          p_idpedido_pos     IN     vtexpedidos.idpedido_pos%type, 
                          p_icprocesado      IN     vtexpedidos.icprocesado%type,
                          p_observacion      IN     vtexpedidos.observacion%type) is
...
--  p_cabecera:='CLH237C1BBB794844E8CE05000CB3C00415C        comisionista@unet.edu.vecomisionista@unet.edu.vecomisionista05500                          4B7C2D073AA4304EE053100000CEA5C6        12       1   pruebacomi'
  ...
  --recupero el cdsucursal y el ID_canal según el affiliateId
   ...
   exception 
     when others then
         SetVtexPedidos (p_pedidoid_vtex,null,2,'No existe la sucursal o no esta activa');
           RETURN;  
   end;
   ...
   begin
    ...
    exception 
         when others then
             SetVtexPedidos (p_pedidoid_vtex,null,2,'No se encuentra la cuenta del cliente: '||trim(substr(p_cabecera,5,40)));              
             RETURN;  
   end; 
   ...
   begin
    ...
    exception 
         when others then
             SetVtexPedidos (p_pedidoid_vtex,null,2,'No se encuentra CUIT del cliente: '||v_identidadReal);        
               RETURN;          
   end; 
  ...
  --divido por 100 por que vtex envia los dos decimales en los ultimos 2 digitos
   ...
   begin
    ...
    exception 
         when others then
             SetVtexPedidos (p_pedidoid_vtex,null,2,'No existe ID vendedor o comisionista: '||substr(p_cabecera,136,40)||' canal: '||v_id_canal);        
               RETURN;  
   end;   
    ...
    if v_qtmateriales = 0 then 
       SetVtexPedidos (p_pedidoid_vtex,null,2,'Pedido sin articulos: '||trim(substr(p_cabecera,5,45)));         
               RETURN;  
    end if;
 ...
 --inserto datos en documentos con tipo de comprobante PEDI
  ...
  IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
   	 SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar documento del pedido VTEX'||p_pedidoid_vtex);
     rollback;  
     RETURN;       
    END IF;
  ...
  (sys_guid()        , v_identidad     , NULL                   , null        , v_iddoctrx      , v_qtmateriales ,v_dsreferencia      ,
   ...
   IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
   	 SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar idpedido POs del pedido VTEX'||p_pedidoid_vtex);
     rollback; 
     RETURN;         
    END IF;
    ...
    IF (p_detalle(1) IS NOT NULL and LENGTH(TRIM(p_detalle(1)))>1) THEN
       FOR i IN 1 .. p_detalle.Count LOOP
        ...
        --  p_detalle(1):='0158777 4669                100                                           0';
          ...
          --si la división es exacta y vluxb > 1 se pasa BTO sino UN
          ...
          begin
            ...
            exception 
              when others then
               SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al intentar recuperar promocion articulo: '||v_cdarticulo);
               rollback;
               RETURN;                       
          end; 
          ...
          begin
            ...
            exception 
              when others then
                SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al intentar recuperar descripcion del articulo: '||v_cdarticulo);                           
               rollback;  
               RETURN;               
          end;
          ...
          (v_idpedido         , i               , v_cdunidadmedida , v_cdarticulo    , v_vlcantidad  , v_quantity          , v_qtpiezas   ,
           ...
           IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
             SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar detalle del pedido VTEX'||p_pedidoid_vtex||'cdarticulo: '||v_cdarticulo);                          
             rollback;  
             RETURN;   
            END IF;           
     ...
     ELSE     
     SetVtexPedidos (p_pedidoid_vtex,null,2,'No existen articulos en el pedido: '||v_idpedido);     
     rollback;  
     RETURN;  
     END IF;
  ...
  IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
   	 SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar tx_pedidos_insert del pedido VTEX'||p_pedidoid_vtex);
     rollback;  
     RETURN;   
    END IF;
  ...
  if length(v_observacion)>1 then
    ...
    values
     ...
     IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
   	 SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar observaciones del pedido VTEX'||p_pedidoid_vtex);                 
     rollback;  
     RETURN;  
    END IF;           
  end if;             
  ...
  --si es CF y ammomto superior al limite inserto para dividir pedido
    if v_cdsituacioniva = 2 and v_ammonto > v_limitedividepedido then
       INSERT INTO tx_pedidos_particionar
        ...
        IF  SQL%ROWCOUNT = 0  THEN      --valida insert 
           SetVtexPedidos (p_pedidoid_vtex,null,2,'Error al insertar tx_pedidos_particionar del pedido VTEX'||p_pedidoid_vtex);
           rollback;   
           RETURN;  
        END IF;      
    end if;
  SetVtexPedidos (p_pedidoid_vtex,v_idpedido,1,'Insertado en POS Correctamente!');                       
  COMMIT;   
  EXCEPTION
    WHEN OTHERS THEN
      SetVtexPedidos (p_pedidoid_vtex,null,2, 'Modulo: ' || v_Modulo || '  Error: ' ||SQLERRM);                  
   	  ROLLBACK;
      RETURN;  
...
FUNCTION DividirPedidos return integer is
  ...
  v_Modulo               varchar2(100) := 'PKG_CLD_DATOS.DividirPedidos';
  v_icorigen             pedidos.icorigen%type:=4;--0-Normal 1-Especificos 2-viejos sin identificar  3-Salon 4-Ecommerce 
  v_ok                   integer;
