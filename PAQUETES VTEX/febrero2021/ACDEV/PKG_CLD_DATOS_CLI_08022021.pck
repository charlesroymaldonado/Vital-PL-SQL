CREATE OR REPLACE PACKAGE PKG_CLD_DATOS_CLI is
...
end PKG_CLD_DATOS_CLI;
/
CREATE OR REPLACE PACKAGE BODY PKG_CLD_DATOS_CLI is
...
FUNCTION GetAgente ( p_identidad    in entidades.identidad%type) RETURN clientescomisionistas.idcomisionista%type IS
     ...
     v_id          clientescomisionistas.idcomisionista%type;
          ...
          select *
           ...
           if(clients.email <> g_l_Clients(i).email and clients.clientsid_vtex<>1) then
             --inactiva el existente
              ...
              --inserta el nuevo registro por el cambio de email en VTEX  OOJOOO REVISAR DIRECCIONES
                  ...
                  or clients.email <> g_l_Clients(i).email or clients.icalcohol <> g_l_Clients(i).icalcohol) then              
                    update vtexclients vc
                       set vc.razonsocial=g_l_Clients(i).razonsocial,
                           ...
                           vc.observacion=null
                     where vc.id_cuenta = clients.id_cuenta;                
                       --BORRAR E INSERTAR TODAS DIRECCIONES ASOCIADAS AL CLIENTE QUE SE MODIFIQUE POR BAJA  ICACTIVE = 0              
               end if; 
           end if;           
      	EXCEPTION
          --si no lo encuentra 
               ...
               if(clients.id_cuenta <> g_l_Clients(i).id_cuenta) then              
                    update vtexclients vc
                       set vc.id_cuenta=g_l_Clients(i).id_cuenta,
                           ...
                           vc.observacion=null
                     where vc.id_cuenta = clients.id_cuenta; 
                     --OOOOJJJJJJJOOOOOOOOOOFALTA AQUI ACTUALIZAR LAS DIRECCIONES ASOCIADAS A ESTA CUENTA DEL CLIENTE 
               end if; 
                EXCEPTION
