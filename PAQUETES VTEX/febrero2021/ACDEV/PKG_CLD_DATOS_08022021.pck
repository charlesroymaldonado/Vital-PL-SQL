...
PROCEDURE Setvtexorders (p_pedidoid_vtex    IN     vtexorders.pedidoid_vtex%type,
  ...
  v_Modulo varchar2(100) := 'PKG_CLD_DATOS.SETvtexorders';
 ...
 cursor c_Clients is
      (select 
       distinct 
                ...
                null dtprocesado,
                null observacion,
                12345 agente, --OJO PENDIENTE FUNCION QUE RECUPERA ID VENDEDOR O COMI
                'CL' tipoagente               
           from tblentidadaplicacion ta,
                entidades            e,
                tblcuenta            tc               
  ...
  CargarTablaClients;
      ...
      FOR i IN 1 .. g_l_Clients.COUNT LOOP
          ...
          select *
           ...
           --si lo encuentra verifica si algo cambio en el cliente
           if(clients.razonsocial <> g_l_Clients(i).razonsocial or clients.email <> g_l_Clients(i).email or clients.cdsucursal <> g_l_Clients(i).cdsucursal
              or clients.icactive <> g_l_Clients(i).icactive or clients.agent<> g_l_Clients(i).agent) then
                ...
                update vtexclients vc
                 ...
                 where vc.id_cuenta = clients.id_cuenta;                
                   --borrar e insertar todas direccioens asocaidas al cliente que se modifique
                 /*   if(promoSKU (r_promo(i).id_promo)=1)then
                n_pkg_vitalpos_log_general.write(2, 'Modulo: '||v_modulo||'  Error: ' || SQLERRM
                                                  ||' error al cargar articulos a la Promoción error: '||r_promo(i).id_promo);                                                 
              end if;*/
          ...
          --si no lo encuentra 
          WHEN NO_DATA_FOUND THEN
          --busca por email si este existe
          select *
            into clients
            from vtexclients vc
