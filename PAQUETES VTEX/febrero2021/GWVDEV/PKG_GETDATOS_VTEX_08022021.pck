...
create or replace package body PKG_GETDATOS_VTEX is
  ...
  ********************************************************************************************************/
  ...
  PROCEDURE SetPedidosVtex (p_pedidoid_vtex  IN  vtexorders.pedidoid_vtex%type) IS 
  ...
  BEGIN
     ...
     IF (p_refId(1) IS NOT NULL and LENGTH(TRIM(p_refId(1)))>1) THEN
       FOR i IN 1 .. p_refId.Count LOOP
          ...
          --el caracter 82 + 1  el estado puede ser : 1 procesado sin error, 2 procesado con error
          --separa del arreglo 83 + 3900 observacion  
          v_id_cuenta:=to_number(substr(p_refId(i),1,40));          
          v_clientsid_vtex:=to_number(substr(p_refId(i),41,40)); 
          v_icprocesado:=to_number(substr(p_refId(i),82,1));
