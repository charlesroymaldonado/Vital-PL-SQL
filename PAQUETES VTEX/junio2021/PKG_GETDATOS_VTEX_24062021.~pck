...
PROCEDURE GetCat(Cur_Out Out Cursor_Type);
  ...
  PROCEDURE GetPromotion ( /*p_cdsucursal In sucursales.cdsucursal%type,
                           p_idcanal    In vtexsellers.id_canal%type,*/
                           Cur_Out      Out Cursor_Type);
...
--             vo.valorcond*vo.uxb cantminima,
             ...
             case
                when (INSTR(vo.dsleyendacorta,'BULTOS')<>0 and INSTR(vo.dsleyendacorta,'cada')<> 0) then
                    'cada '||vo.valorcond*vp.uxb||'  UNIDADES'
  ...
  BEGIN
    ...
    OPEN cur_out FOR
        SELECT A.id_promo_pos,
               ...
               LISTAGG(A.id_canal_vtex, ', ') WITHIN GROUP (ORDER BY A.id_canal_vtex) idsSalesChannel,
         ...
         SELECT DISTINCT
                     ...
                     DECODE(vp.type,1,null,'[{' ||
                        --divido por el minimo UV para convertir multiplicador
                        ceil((vp.valorcond*vp.uxb)/
                           (select min(vsk.unitmultiplier)
                              from Vtexpromotionsku vps,
                                   vtexsku          vsk
                             where vps.id_promo_pos = vp.id_promo_pos
                               and vps.refid = vsk.refid))||', ' ||vp.valoracc||'}]') percentualDiscountValueList,
                        ...
                        ceil((vp.valorcond*vp.uxb)/
                           (select min(vsk.unitmultiplier)
                              from Vtexpromotionsku vps,
                                   vtexsku          vsk
                             where vps.id_promo_pos = vp.id_promo_pos
                               and vps.refid = vsk.refid)) minimumQuantityBuyTogether,
